...
%
%\subsection{Peptide Spectral Library Search}
...
%In this paper we have shown that SLGF is more sensitive than SpectraST. There are classes of spectra, e.g. high $DotBias$, not identified by SpectraST that SLGF is now able to identify. Not only is this significant, but in figure~\ref{SearchAbundance} we can see that SLGF gains significantly over spectra in mid to high abundance spectra.

%We have departed from the null hypothesis of many existing works, i.e. query spectra come from random compounds, and instead examine the effect of instrument measurement variability (and other sources of variability) on replicates of library spectra. Using such an approach we are able to better assess SSMs and assess the significance for single SSM scores. It can be seen in our results that indeed such an approach is able to provide $\sim$12\% increase in sensitivity over state of the art library search tools even with our models being partially incomplete (especially for very low abundance spectra).

%Another limitation of many current spectral library search tools is how intimately intertwined they are with proteomics data. The Spectral Library Generating Function method presented here makes no assumptions on compounds being peptides, and proteomics data was only used for evaluation of the method. Thus, our results are very promising that SLGF is very effective in terms of increasing search sensitivity and we are hopeful for its applicability outside of proteomics.
