...
//
///////////////////////////////////////////////////////////////////////////////
void PlotContig::getDirectAbruijn(void)
{
...
void PlotContig::alignSpectra(void)
{
  ...
  // setp 4: set limits according to parent mass plus offset
  for(int i = 0 ; i < m_offsets.size() ; i++) {
    int currentStar = m_offsets[i].id;
    Spectrum &sp = (*m_star)[currentStar];
    m_offsets[i].endingMass = m_offsets[i].offset + sp.parentMass;
    m_offsets[i].tolerance = m_peakMassTol;
  }


  // step 5: sort spectra from lower offset to greater
  sort(m_offsets.begin(), m_offsets.end());

  // step 6: get minimun offset
  double minOffset = m_offsets[0].offset;

  // step 7: set the deNovo sequence offset to be equal to the minimun offset, if negative
  if(minOffset < 0.0) {
...
void PlotContig::calcultateLimits(void)
{
  ...
  // check reference sequence mass limit
  if(m_referenceIntervals.size()) {
    double aux = m_deNovoOffset + m_referenceOffset + m_referenceIntervals[m_referenceIntervals.size()-1];
    if(aux > m_mzUpperLimit)
      m_mzUpperLimit = aux;
  }

  // check homolog sequence mass limit
  if(m_homologIntervals.size()) {
    double aux = m_deNovoOffset + m_homologOffset + m_homologIntervals[m_homologIntervals.size()-1];
    if(aux > m_mzUpperLimit)
      m_mzUpperLimit = aux;
  }

  // check denovo sequence mass limit
  if(m_deNovoIntervals.size()) {
    double aux = m_deNovoOffset + m_deNovoIntervals[m_deNovoIntervals.size()-1];
    if(aux > m_mzUpperLimit)
      m_mzUpperLimit = aux;
  }

  // check user sequence mass limit
  if(m_userIntervals.size()) {
    double aux = m_deNovoOffset + m_userOffset + m_userIntervals[m_userIntervals.size()-1];
    if(aux > m_mzUpperLimit)
      m_mzUpperLimit = aux;
  }
...
void PlotContig::initDeNovo(void)
{
  ...
  //    if(abData[i].first[j] == m_stars[0])
  //      m_breaks.push_back(abData[i].second[j]);
...
//
///////////////////////////////////////////////////////////////////////////////
void PlotContig::initMassIntervals(vector<double> &massIntervals, string &masses)
{
  //cout << "----------------------" << endl;

  if(massIntervals.size())
    return;

  AAJumps jumps(1);
  vector<float> aux;
  jumps.getPRMMasses(masses, aux);

  //cout << aux.size() << endl;

  massIntervals.clear();
  massIntervals.push_back(0.0);
  for(int i = 0 ; i < aux.size() ; i++)
    massIntervals.push_back( (double)aux[i] );
}
///////////////////////////////////////////////////////////////////////////////
//
///////////////////////////////////////////////////////////////////////////////
...
void PlotContig::drawSequences(void)
{
  ...
  if(m_denovo.length()) {
    ...
    offsetPrevious = 0.0;
    ContigSequence seq(m_rendererObject, &m_deNovoIntervals2, previous, &m_denovo, offset, offsetPrevious, "de Novo:", coord, coordPrev, drawLabels, m_mzLowerLimit, m_mzUpperLimit, "#A00000", m_allDots);
    seq.setPeakMassTol(m_peakMassTol);
    seq.useIndividualPeakMassTol(m_useIndividualPeakMassTol);
  ...
  if(m_homolog.length()) {
    ...
    ContigSequence seq(m_rendererObject, &m_homologIntervals, previous, &m_homolog, offset, offsetPrevious, "Homolog:", coord, coordPrev, drawLabels, m_mzLowerLimit, m_mzUpperLimit, "#A00000", m_allDots);
    seq.setPeakMassTol(m_peakMassTol);
    seq.useIndividualPeakMassTol(m_useIndividualPeakMassTol);
    seq.setZoom(m_rendererImage.zoom);
    seq.setImageDimensions(m_rendererImage.imageSizeX, m_rendererImage.imageSizeY);
  ...
  if(m_reference.length()) {
    ...
    ContigSequence seq(m_rendererObject, &m_referenceIntervals, previous, &m_reference, offset, offsetPrevious, "Reference:", coord, coordPrev, drawLabels, m_mzLowerLimit, m_mzUpperLimit, "#A00000", m_allDots);
    seq.setPeakMassTol(m_peakMassTol);
    seq.useIndividualPeakMassTol(m_useIndividualPeakMassTol);
    seq.setZoom(m_rendererImage.zoom);
    seq.setImageDimensions(m_rendererImage.imageSizeX, m_rendererImage.imageSizeY);
  ...
  if(m_user.length()) {
    ...
    previous = &m_deNovoIntervals2;
    coordPrev = ntp + disp;
    ContigSequence seq(m_rendererObject, &m_userIntervals, previous, &m_user, offset, offsetPrevious, "User:", coord, coordPrev, drawLabels, m_mzLowerLimit, m_mzUpperLimit, "#4040F0", m_allDots, bend);
    seq.setPeakMassTol(m_peakMassTol);
    seq.useIndividualPeakMassTol(m_useIndividualPeakMassTol);
    seq.setZoom(m_rendererImage.zoom);
    seq.setImageDimensions(m_rendererImage.imageSizeX, m_rendererImage.imageSizeY);
...
bool PlotContig::checkContigItem(int spectrum, double mass)
{
  ...
  for(int i = 0 ; i < abData.size() ; i++)
    for(int j = 0 ; j < abData[i].first.size() ; j++)
      if(abData[i].first[j] == spectrum)
        if(fabs(abData[i].second[j] - mass) <= m_peakMassTol)
          return true;
...
bool PlotContig::checkContigItemAtPosition(int spectrum, int position, float &value)
{
  ...
  if(abData.size() <= position)
    return false;

  // get the node IDs list
  const vector<int> &nodeIds = abData[position].first;

  // go thru the
  for(int j = 0 ; j < nodeIds.size() ; j++)
    if(nodeIds[j] == spectrum) {
      value = abData[position].second[j];
      return true;
    }
...
void PlotContig::drawVerticalLines(void)
{
  ...
  vector<pair<double, double> > topPoints;
  ...
  //double disp3 = (m_mzUpperLimit - m_mzLowerLimit) / ((double)m_rendererImage.imageSizeX - nxp) * 7.0; //  npw = 1.0 / nx
  double disp3 = (m_mzUpperLimit - m_mzLowerLimit) / ((double)m_rendererImage.imageSizeX - nxp * (double)m_rendererImage.imageSizeX) * 7.0;
  double dispY = (double)m_rendererImage.fontHeight / (double)m_rendererImage.imageSizeY / m_rendererImage.zoom * 0.5;
  ...
  for(int i = 0 ; i < m_deNovoIntervals.size() ; i++) {
    ...
    topPoints.push_back(make_pair<double,double>(x2, y2));
  }
  ...
  //for(int i = m_offsets.size()-1 ; i >=0  ; i-- ) {
    ...
    for(int j = 0 ; j < m_deNovoIntervals.size() ; j++ ) {
      ...
      if(checkContigItemAtPosition(id, j, value)) {
        ...
        double y2 = 1.0 / (double)m_stars.size() * (double)position * nys + nyp  + dispY;
        ...
        x1 = x2 - disp3;
        y1 = y2 = 1.0 / (double)m_stars.size() * (double)position * nys + nyp  + dispY;
...
void PlotContig::drawSpectrum(int spectrumIndex, int spectrumCount)
{
  ...
  double sx = fabs(mzUpperLimit) / denom;
  double sy = (1.0 / nps) * nys;


  //double disp3 = (m_mzUpperLimit - m_mzLowerLimit) * 0.005;
  double disp3 = (m_mzUpperLimit - m_mzLowerLimit) / ((double)m_rendererImage.imageSizeX - nxp * (double)m_rendererImage.imageSizeX);
  double factor = (m_mzUpperLimit + disp3) / mzUpperLimit;

  //cout << "m_mzUpperLimit: " << m_mzUpperLimit << endl;
  //cout << "mzUpperLimit: " << mzUpperLimit << endl;
  ...
  // draw lead dotted line
  if(spectrumCount + 1 - m_offsets.size())
    drawLine(1, 0,
             nxp,  RENDERER_COORD_SCREEN,
             oy, RENDERER_COORD_SCREEN,
             ox, RENDERER_COORD_SCREEN,
             oy, RENDERER_COORD_SCREEN,
             "#b0b0b0");
  ...
  // draw the spectrum
  ContigSpectrum  cs(m_rendererObject, m_contigIndex, &abData, sp, &m_deNovoIntervals, spectrumIndex);
  cs.useMinimunIntentity(m_useMinimunIntensity);
  cs.setLineThickness(m_rendererImage.lineThickness);
  cs.setPeakMassTol(m_peakMassTol);
  cs.useIndividualPeakMassTol(m_useIndividualPeakMassTol);
