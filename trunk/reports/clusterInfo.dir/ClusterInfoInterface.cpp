...
ClusterInfoInterface::ClusterInfoInterface() : outdir("."), outFileName(DEFAULT_AUTFILE_NAME),
  ...
  outSummaryFilename(DEFAULT_SUMMARY_FILEMAME),
  projectdir(".")
{
}
...
///////////////////////////////////////////////////////////////////////////////
int ClusterInfoInterface::writeCsvFile(vector<string> *inputFiles, vector<vector<vector<int> > > *scanNumbers, string &outFileName, vector<SpecSet> &specs)
{
  // data holder
  ...
  ofstream file(outFileName.c_str(), ios::out | ios::binary);
    // if error, say so
  if(!file.is_open()) {
    cerr << "ERROR: ClusterInfo: could not open file for writing: " << outFileName << endl;
    return -1;
  }
...
///////////////////////////////////////////////////////////////////////////////
int ClusterInfoInterface::WriteClusterSummary(std::map<string, string> &groups, string groupName, vector<string> *inputFiles)
{
  ...
  // add headers
  vector<string> row;
  row.push_back("cluster index");
  row.push_back("number of spectra");
  row.push_back("parent mass");
  row.push_back("precursor charge");
  row.push_back("precursor mass");
  row.push_back("sum(precursor intensity)");
  // add group names
  int groupNameSize = groupName.size();
  for(map<std::string, std::string>::iterator it = groups.begin(); it != groups.end(); it++)
    row.push_back(it->first.substr(groupNameSize));
  // store headers
  data.push_back(row);
  ...
  // open file to write to
  ofstream file(outSummaryFilename.c_str(), ios::out | ios::binary);
    // if error, say so
  if(!file.is_open()) {
    cerr << "ERROR: ClusterInfo: could not open summary file for writing: " << outSummaryFilename << endl;
    return -1;
  }
...
int ClusterInfoInterface::processOptions(int argc, char **argv)
{
  ...
  listOptions.push_back(CommandLineParser::Option("-cluster-spectra", "CLUSTER_SPECTRA",  true));
  ...
  listOptions.push_back(CommandLineParser::Option("-out-summary-file","OUT_SUMMARY_FILE", true));
  ...
  for(int i = 0 ; i < inputFiles.size() ; i++) {
    ...
    if(spec.loadPklBin(inputFiles[i].c_str()) <= 0) {
  ...
  if(specs_ms.loadPklBin(specs_msFilename.c_str()) <= 0)
    cerr << "Unable to load file: " << specs_msFilename << endl;
...
int ClusterInfoInterface::help(ostream & sout)
{
  ...
  sout << "  --cluster-spectra FILE      Cluster spectra file (defaults to 'spectra/specs_ms.pklbin')\n";
  ...
  sout << "  --out-summary-file FILE     Output filename for the summary file (defaults to 'cluster_summary.tsv')\n";
  sout << '\n';
