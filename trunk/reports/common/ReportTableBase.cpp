...
int ReportTableBase::saveTable(char separator)
{
  // get filename with full path
  string fn = composeFileName(m_projectDir, m_tableFilename);
  ...
  if(m_colHeadings.size()) {
    ...
    for(int j = 0 ; j < m_colHeadings.size() ; j++) {
      if(j) line += separator;
      line += m_colHeadings[j];
    }
    file << line << endl;
  }

  writeTable(file, separator);
...
////////////////////////////////////////////////////////////////////////////////
int ReportTableBase::writeTable(ostream &outstream, char separator, char endline)
{
