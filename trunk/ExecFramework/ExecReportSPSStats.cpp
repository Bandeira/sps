...
namespace specnets
{
  ...
  bool ExecReportSPSStats::loadInputData(void)
  {
    ...
    {
      if (!ExecMergeConvert::loadSpecset(m_params.getValue("INPUT_CONTIGS"),
                                         m_contigs))
      {
    ...
    {
      if (!ExecMergeConvert::loadSpecset(m_params.getValue("INPUT_STARS"),
                                         m_stars))
      {
    ...
    if (minDBmp > 0)
    {
      ...
      for (int i = 0; i < m_contigs->size(); i++)
      {
        ...
        int lastIdx = -2;
        bool haveMP = false;
        for (unsigned int j = 0; j < (*m_overlaps)[i].size(); j++)
        {
          int nextIdx = floatToInt((*m_overlaps)[i][j][0]);
    ...
    for (int i = 0; i < m_specIDs->size(); i++)
    {
      ...
      else
      {
        ...
        while (init || j != lastSpecIdx)
        {
          ...
          if ((*m_stars)[j].scan == (*m_specIDs)[i]->m_scanNum)
          {
            specIdx = j;
            lastSpecIdx = j;
            break;
          }
          j = (j == m_stars->size() - 1) ? 0 : j + 1;
        }
        if (specIdx < 0)
        {
          ERROR_MSG("Failed to locate scan " << (*m_specIDs)[i]->m_scanNum);
          abort();
        }
