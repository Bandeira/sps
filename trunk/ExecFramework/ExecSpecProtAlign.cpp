...
namespace specnets
{
  ...
  bool ExecSpecProtAlign::invoke(void)
  {
   ...
   // More readable code if we use a reference instead of pointer
    ...
    bool alignall = (bool) m_params.exists("ALIGN_ALL") ? ((int) m_params.getValueInt("ALIGN_ALL") ? 1 : 0) : 0;
    DEBUG_VAR(alignall);

    bool resetEndpoints = (bool) m_params.exists("RESET_ENDPOINTS") ? ((int) m_params.getValueInt("RESET_ENDPOINTS") ? 1 : 0) : 0;
    ...
    for (specIdx = startIdx; specIdx <= endIdx; specIdx++) {
      ...
      cSpec.psmList.remove_if(LessThanPredicate(bestScore * thresholdPercent));
      cSpecRev.psmList.remove_if(LessThanPredicate(bestScore * thresholdPercent));
      if (DEBUG_SPECPROTALIGN) DEBUG_VAR(cSpec.psmList.size());
      if (DEBUG_SPECPROTALIGN) DEBUG_VAR(cSpecRev.psmList.size());
