...
namespace specnets
{
                                          ...
                                          int abundanceIndex)
  {
    ...
    if (organismIndex != -1)
    {
      currMatch->m_organism.push_back(line[organismIndex]);
    }
    ...
    {
      currMatch->m_compound_name.push_back(line[compoundNameIndex]);
    }
    
    
    if (smilesIndex != -1)
    {
      currMatch->m_smiles.push_back(line[smilesIndex]);
    }
    
    if (inchiIndex != -1)
    {
      currMatch->m_InChI.push_back(line[inchiIndex]);
    }
    
    if (inchiAuxIndex != -1)
    {
      currMatch->m_InChI_Aux.push_back(line[inchiAuxIndex]);
    }
    
    if (libmetadataIndex != -1)
    {
      currMatch->m_library_metadata  = (line[libmetadataIndex]);
    }
    
    if (sharedpeakIndex != -1)
    {
      if (!sscanf(line[sharedpeakIndex].c_str(), "%d", &(currMatch->m_shared_peaks)))
      {
                                           ...
                                           bool includeHeader)
  {
    ...
    {
      ofs
          << "#Scan#\tSpectrumFile\tAnnotation\tOrigAnnotation\tProtein\tdbIndex\tnumMods\tmatchOrientation\tstartMass\tCharge\tMQScore\tp-value\tisDecoy\tStrictEnvelopeScore\tUnstrictEvelopeScore\tCompoundName\tOrganism\tFileScanUniqueID\tFDR\tLibraryName\tmzErrorPPM\tLibMetaData\tSmiles\tInchi\tLibSearchSharedPeaks\tAbundance"
          << endl;
    }
