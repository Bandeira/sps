...
namespace specnets
{
  ...
  public:
    ...
    void setDbMatch(const string & protein, const int dbIndex, const float startMass);
     ...
     *
     */
    void getAnnotationFromMatchedPeaks(Spectrum & dbSpec, const string & dbSeqStr, string & annotation);
     ...
     */
    bool getModifications(vector<float> &modifications) const;
    ...
    string m_library_metadata;

    /**
     *  Below is meta data regarding the spectrum used for loading spectra
     *  into CCMS for creating spectral libraries
     */
    vector<string> m_submission_metadata;
    vector<string> m_organism;
    vector<string> m_compound_name;
    vector<string> m_smiles;
    vector<string> m_InChI;
    vector<string> m_InChI_Aux;
