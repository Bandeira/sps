...
void PrmGraph::create_nodes()
{
	...
	for (i=0; i<strong_frags_lists.size(); i++)
	{
		...
		for (q=0; q<strong_peak_idxs.size(); q++)
		{
			...
			node.org_peak_mass = peaks[p_idx].mass;
			node.orig_peak_offset = ft.offset;
			node.orig_peak_orient = ft.orientation;
			node.orig_peak_charge = ft.charge;
		...
		for (f=0; f<strong_frags_in_combos.size(); f++)
		{
			...
			for (c=0; c<combos.size(); c++)
			{
				...
				if (combo.frag_inten_idxs[0] == strong_frag_idx)
				{
					...
					node.org_peak_mass = peaks[i].mass;
					node.orig_peak_offset = strong_frag.offset;
					node.orig_peak_orient = strong_frag.orientation;
					node.orig_peak_charge = strong_frag.charge;
...
void PrmGraph::print_only_scores() const
{
	...
	for (i=0; i<nodes.size(); i++)
	{
		if (nodes[i].score>NEG_INF && nodes[i].score !=0 )
			cout << nodes[i].mass << " " << nodes[i].score << " " << nodes[i].org_peak_mass << "," << nodes[i].orig_peak_orient << "," << nodes[i].orig_peak_charge << "," << nodes[i].orig_peak_offset << endl;
	}
}
