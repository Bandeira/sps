									 ...
									 vector<vector<float> > *inp_weights)
{
	...
	max_model_charge = (inp_weights ? inp_weights->size()-1 : 3);
	int charge;
	...
	if (this->pmc_rank_mass_thresholds.size() == 0)
	{
	...
	// create SQS models
	this->sqs_models.resize(max_model_charge+1);
	for (charge =0; charge<=max_model_charge; charge++)
	{
		sqs_models[charge].resize(max_model_charge+1);
		int j;
	...
	{
		int charge1,charge2;
		for (charge1=2; charge1<=max_model_charge; charge1++)
		{
				...
				for (size_idx=0; size_idx<=num_sizes; size_idx++)
				{
					...
					if (ds.class_weights[0]<0.0001 || ds.class_weights[1]<0.0001)
					{
						cout << "Warning: insufficient number of samples, not trianing model for charge " << charge1 <<
							" vs charge " << charge2<< " (size " << size_idx << ")" << endl;
						continue;
					}
	...
	init_sqs_correct_factors(max_model_charge,sqs_mass_thresholds.size());
	...
	for (size_idx=0; size_idx<=num_sizes; size_idx++)
	{
		...
		float p_thresh = 0.05;
		int d;
		for (d=0; d<=max_model_charge; d++)
		{
			vector<int> counts;
			vector<int> max_counts;
			counts.resize(max_model_charge+1,0);
			max_counts.resize(max_model_charge+1,0);
...
bool PMCSQS_Scorer::read_sqs_models(Config *_config, char *file)
{
	...
	while (in_stream.getline(buff,128))
	{
		...
		if (charge1<1 || charge2<0 || charge1>=num_sqs_charges || charge2>=charge1 || size_idx<0)
		{
	...
	if (max_model_charge>0 && max_model_charge >=num_sqs_charges)
	{
		max_model_charge = num_sqs_charges - 1;
	}
	else
		max_model_charge = num_sqs_charges - 1;
