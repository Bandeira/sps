...
void Model::write_model()
{
	...
	if (edge_model.get_ind_was_initialized())
	{
...
//	config.print_all_regional_fragment_relationships();
	...
	cout << endl << "STAGE 3: Train SQS models" << endl;
	cout <<         "*************************" << endl << endl;
	if (start_stage>3)
	{
		cout << endl << "Already done." << endl;
	...
	else
	{
		...
		if (! neg_sqs_list)
		{
			cout << "Error: must supply negative (bad) spectra for SQS training (use the -neg_spec_list option)." << endl;
			exit(1);
		}
	...
	cout << endl << "STAGE 4: Train PMCR models" << endl;
	cout <<         "**************************" << endl << endl;
	if (start_stage>4)
	{
		cout << endl << "Already done." << endl;
	...
	else
	{
		...
		{
			cout << "No need for parent mass correction model, PM is accurate enough (tolerance is " <<
				config.get_pm_tolerance() << ")" << endl;
		}
	}
	...
	cout << endl << "STAGE 5: Train breakage score models" << endl;
	cout <<         "************************************" <<endl;
	cout << endl;
	if (start_stage>5)
	{
	...
	{
		if (! this->comp_assigner.get_ind_was_initialized())
		{
			cout << "Error: no peptide composition assigner was read, cannot continue training!" << endl;
			cout << "You can probably get around this by adding the line \"#COMP_ASSIGNER LTQ_COMP/IT_TRYP\" to the file " <<
				this->get_model_name() << ".txt" << endl;
			exit(1);
		}
