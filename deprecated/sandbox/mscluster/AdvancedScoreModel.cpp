...
void AdvancedScoreModel::read_cum_seq_prob_models(const char *name, bool silent_ind)
{
	...
	{
		CumulativeSeqProbModel *csp_model = (CumulativeSeqProbModel *)cumulative_seq_prob_models[i];
		char name_buff[64];

		sprintf(name_buff,"%s_CSP_%d.txt",name,i);
		
		string path = config.get_resource_dir() + "/" +dir_name + "/" + string(name_buff);
		...
		csp_model->read_model(&config,ifs);
		ifs.close();

		cumulative_seq_prob_models[i] = (void *)csp_model;
						...
						int charge, int size_idx, int region_idx)
{
	...
	// train models
	for (c=1; c<regional_breakage_score_models.size(); c++)
	{
