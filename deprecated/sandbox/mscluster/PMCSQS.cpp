													 ...
													 vector<PmcSqsChargeRes>& res)
{
	...
	if (res.size()<=max_model_charge)
		res.resize(max_model_charge+1);

	
	vector<float> min_comp_probs;		// the minimal probability when used in a comparison model
	min_comp_probs.resize(max_model_charge+1,2.0);
	min_comp_probs[0]=0;

	if (ind_initialized_sqs)
	{
		const int max_sqs_charge = ( sqs_models.size()<max_model_charge ?
			sqs_models.size() : max_model_charge);

		fill_fval_vector_with_SQS(bs, sqs_sam);
	
		int charge;
		for (charge=1; charge<=max_sqs_charge; charge++)
		{
	...
	const int max_pmc_charge = (pmc_rank_models.size() < max_model_charge ? 
		pmc_rank_models.size() : max_model_charge);
												   ...
												   mass_t increment)
{
	...
	for (charge=1; charge<=max_model_charge; charge++)
	{
		...
		float bias = 0;
		if (pmc_charge_mz_biases.size()>0 && pmc_charge_mz_biases[charge].size()>size_idx)
