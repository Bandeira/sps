...
{ 
	...
	while (fgets(buff,256,inp_file))
	{
		...
		if (! strncmp(buff,"denovoMode",10))
		{
			ind_denovo_mode = true;
			continue;
		}
	...
	perform_denovo_on_list_of_files(model, files,  tag_length,
		file_start_idx, num_solutions, min_length, max_length, true, 
