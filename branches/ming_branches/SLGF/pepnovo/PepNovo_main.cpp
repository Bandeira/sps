...
const string build_name = "20090907"; 
...
//	printf("-large_db_model   - if rescoring searches of large databases (e.g., 300MB+), this flag can later scoring to give better results.\n");
	...
	printf("\nParameters for MS-Blast:\n");
	printf(  "------------------------\n");
	printf("-msb_query_size <X> - max size of MSB query allowed (default X=150000).\n");
	...
	printf("-msb_min_score  <X> - the minimal MS-Blast score to be included in the query, default X=3.0 .\n");
//	printf("-msb_query_condense - takes a list of _full.txt files and creates a single _query.txt file (list should be given with -list flag).\n");
...
{ 
	...
	bool got_msb_query_condense  = false;
	bool got_msb			 = false;
	...
	int	  msb_query_size = 150000;
	int	  msb_num_solutions = 7;
	float msb_min_score = 3.0;

	seedRandom(112233);
	strcpy(tag_suffix,"tags");
	strcpy(msb_query_name,"MSBlast");
	...
	while (i<argc)
	{
		...
		else if (! strcmp(argv[i],"-msb_query_condense"))
		{
			got_msb_query_condense = true;
		}
	...
	cout << "Copyright 2009, The Regents of the University of California. All Rights Reserved." << endl;
	...
	if (got_msb)
	{
		...
		msbc.generateMsBlastSequences(msb_query_name, list_vector, &model, min_filter_prob, 
									  msb_num_solutions, indOutputCumulativeProbs);
		msbc.generateMsBlastQuery(msb_query_name, msb_query_size, msb_min_score);
		return 0;
	}
