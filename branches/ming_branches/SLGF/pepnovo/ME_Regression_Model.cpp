...
void ME_Regression_Model::print_ds_probs(const ME_Regression_DataSet &ds) const
{
    ...
    if (pr_pos.size()<10)
    {
        printf("#pos samples %lu.\n",(unsigned long)pr_pos.size());
    }
    else
    {
        double av=0;
        for (i=0; i<pr_pos.size(); i++)
            av+=pr_pos[i];
        av /= pr_pos.size();
        printf("#pos samples %lu, avg prob = %.3f\n", (unsigned long)pr_pos.size(),av);
    ...
    if (pr_neg.size()<10)
    {
        printf("#neg samples %lu.\n", (unsigned long)pr_neg.size());
    }
    else
    {
        double av=0;
        for (i=0; i<pr_neg.size(); i++)
            av+=pr_neg[i];
        av /= pr_neg.size();
        printf("#neg samples %lu, avg prob = %.3f\n", (unsigned long)pr_neg.size(),av);
