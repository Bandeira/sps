...
public:
	...
	{
		pmcsqs.compute_sqs_cum_stats_for_ided(&config_, list);
	}

	void compute_sqs_cum_stats_for_crap(char *list)
	{
		pmcsqs.compute_sqs_cum_stats_for_crap(&config_, list);
	}
	...
	float get_sqs_only(const Config *config, const BasicSpectrum& bs, int *max_charge=NULL)
	{
