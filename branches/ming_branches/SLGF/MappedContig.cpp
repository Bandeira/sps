...
namespace specnets
{
                             ...
                             float peakTol)
  {
    ...
    ((Spectrum &) (*this)) = (*parentContigs)[idx];
    ...
    if (vertMapped) {
      ...
      int lastMapped = floatToInt((*overlaps)[index][(*overlaps)[index].size()-1][1]);
  ...
  float MappedContig::getPercBVerts(void)
  {
    ...
    for (int i = 0; i < length; i++) {
      if (abruijnVerts[i].annotated) {
        denominator += 1.0;
        numerator
            += (abruijnVerts[i].annotation.find("b") != string::npos) ? 1.0 : 0;
      }
    }
  ...
  float MappedContig::getPercYVerts(void)
  {
    ...
    for (int i = 0; i < length; i++) {
      if (abruijnVerts[i].annotated) {
        denominator += 1.0;
        numerator
            += (abruijnVerts[i].annotation.find("y") != string::npos) ? 1.0 : 0;
      }
    }
    return (numerator / denominator) * 100.0;
  }
  ...
  /**
   * @return % of gaps that match label. if _label == 0, denominator is
   *   # of all gaps. if _label > 0, denominator is # of annotated gaps
   */
  float MappedContig::getPercGaps(short _label)
  {
    float numerator = 0;
    float denominator = 0;
