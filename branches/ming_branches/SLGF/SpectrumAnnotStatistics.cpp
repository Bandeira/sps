...
namespace specnets
{
                                                           ...
                                                           string &ionNames)
  {
    ...
    {
      if ((*spectrum)[i][0] >= AAJumps::minAAmass
          && (*spectrum)[i][0] <= (peptide_mass - 37))
	...
	for (int psmIdx = 0; psmIdx < psmSet.size(); psmIdx++) {
		...
		for (unsigned i = 0; i < spectrum->size(); i++) {
			if ((*spectrum)[i][0] >= AAJumps::minAAmass && (*spectrum)[i][0]
					<= (peptide_mass - 37)) { // ignore small masses or masses larger than we expect.
				total_intensity += (*spectrum)[i][1]; // add to total intensity regardless of annotation
				const ftIonFragment* curr_frag = psm.m_peakAnnotations[i].first;
				if (curr_frag != NULL) { // make sure peak is annotated
					if (ionNames.compare("all") == 0) { // include all identified peaks
						identified_intensity += (*spectrum)[i][1];
					} else {
						for (int j = 0; j < ion_types.size(); j++) {
							if (ion_types[j].compare(curr_frag->name) == 0) { // include only listed peaks
								identified_intensity += (*spectrum)[i][1];
							}
						}
					}
