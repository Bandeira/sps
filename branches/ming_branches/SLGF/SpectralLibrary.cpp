...
namespace specnets
{
    ...
    int SpectralLibrary::search_target_decoy_specset(SpectralLibrary &decoy, 
        ...
        for(int query_idx = 0; query_idx < searchable_spectra.size(); query_idx++){
            ...
            //searchable_spectra[query_idx].setResolution(1.0005, true);
            ...
            if(thread_id == 0){
                //cout<<"Searching Scan:\t"<<searchable_spectra[query_idx].scan<<"\tAND Index\t"<<query_idx*8<<"\tof\t"<<searchable_spectra.size()<<endl;
            }
            else{
            ...
            int target_decoy_search = this->search_target_decoy(decoy, 
    ...
    int SpectralLibrary::search_target_decoy_specset(SpectralLibrary &decoy, 
        ...
        for(int query_idx = 0; query_idx < searchable_spectra.size(); query_idx++){
            ...
            if(thread_id == 0){
                //cout<<"Searching Scan:\t"<<searchable_spectra[query_idx].scan<<"\tAND Index\t"<<searched_count*8<<"\tof\t"<<searchable_spectra.size()<<endl;
            }
            else{
                //cout<<"Searching Scan OTHER THREAD:\t"<<searchable_spectra[query_idx].scan<<"\tAND Index\t"<<query_idx<<"\tof\t"<<searchable_spectra.size()<<endl;
            }
    ...
    /*! \brief Finds spectrum that best represents each peptide
        Warning, this will break for same peptides with multiple charges
     */
    ...
    /*! \brief Removes spectra from the library that have too few peaks
     */
    void SpectralLibrary::filter_simple_spectra(   MS2ScoringModel &model,
                                                        vector<string> &ionsToExtract,
                                                        string allIons){
        SpectralLibrary temp_lib;
        //Filtering based on simplicity of spectrum
        for(int spectrum_idx = 0; spectrum_idx < specs.size(); spectrum_idx++){
            if(spectrum_signal_peak_count(model, ionsToExtract, allIons, specs[spectrum_idx].psmList.front(), 0.05) > 4)
                temp_lib.specs.push_back(specs[spectrum_idx]);
        }
        specs.clear();
        specs.insert(specs.begin(), temp_lib.specs.begin(), temp_lib.specs.end());
    ...
    int SpectralLibrary::remove_low_complexity_spectra(SpectralLibrary &decoy, MS2ScoringModel &model, vector<string> &ionsToExtract,string allIons){
        if(specs.size() != decoy.size()){
            return 0;
        }
        
        SpectralLibrary temp_lib_target;
        SpectralLibrary temp_lib_decoy;
        float intensity_thres = 0.12;
        int peak_count = 5;
        
        for(int spectrum_idx = 0; spectrum_idx < specs.size(); spectrum_idx++){
            if(spectrum_signal_peak_count(model, ionsToExtract, allIons, specs[spectrum_idx].psmList.front(), intensity_thres) > peak_count){
                temp_lib_target.specs.push_back(specs[spectrum_idx]);
                temp_lib_decoy.specs.push_back(decoy[spectrum_idx]);
            }
        }
        specs.clear();
        decoy.specs.clear();
        ...
        specs.insert(specs.begin(), temp_lib_target.specs.begin(), temp_lib_target.specs.end());
        decoy.specs.insert(decoy.specs.begin(), temp_lib_decoy.specs.begin(), temp_lib_decoy.specs.end());
        temp_lib_target.specs.clear();
        temp_lib_decoy.specs.clear();
        
        for(int spectrum_idx = 0; spectrum_idx < specs.size(); spectrum_idx++){
            specs[spectrum_idx].psmList.front()->m_spectrum = &(specs[spectrum_idx]);
            decoy.specs[spectrum_idx].psmList.front()->m_spectrum = &(decoy.specs[spectrum_idx]);
        }
        
        DEBUG_MSG("FILTERING LOW COMPLEXITY SPECTRA\t"<<specs.size());
    ...
    SpectralLibrary SpectralLibrary::create_decoy_spectral_library(MS2ScoringModel model, vector<string> ions_to_extract, string allIons){
        ...
        for(int library_idx = 0; library_idx < specs.size(); library_idx++){
            ...
            //Seeding this annotation
            int seed = hashpeptide(annotation, specs[library_idx].parentCharge);
            srand(seed);
            ...
            for(int random_retries = 0 ; random_retries < 50; random_retries++){
                ...
                float percent_sim = percent_sequence_similarity(annotation, random_annotation);
                //DEBUG_MSG(random_annotation<<"\t"<<annotation<<"\t"<<percent_sim);
                
                if(percent_sequence_similarity(annotation, random_annotation) > 0.6){
                    //cout<<random_annotation<<"\t"<<annotation<<endl;
                    continue;
                }
                ...
                if(binary_search(library_peptides.begin(), library_peptides.end(), search_random_annotation)){
                    ...
                    //DEBUG_MSG(annotation_orig_stripped<<endl<<random_annotation<<endl);
            ...
            DEBUG_MSG(decoy_library.size()<<"\t"<<annotation_orig_stripped<<"\t"<<random_annotation<<"\t"<<percent_sequence_similarity(annotation_orig_stripped, random_annotation)<<endl);
            ...
            decoy_psm->SLGF_distribution = specs[library_idx].psmList.front()->SLGF_distribution;
    ...
    void SpectralLibraryGFSearch::train_library_montecarlo(MS2ScoringModel model, vector<string> ions_to_extract, string allIons, SpectralLibrary &library){
        Spectrum test_spectrum;
        //test_spectrum.peakList.resize(4);
        psmPtr psm(new PeptideSpectrumMatch());
        psm->m_annotation = "*.AAAA.*";
        psm->m_spectrum = &test_spectrum;
        test_spectrum.psmList.push_back(psm);
        
        test_spectrum.insertPeak(90.055, 30, 1);
        test_spectrum.insertPeak(161.055, 30, 1);
        test_spectrum.insertPeak(232.055, 30, 1);
        test_spectrum.insertPeak(303.055, 30, 1);
        
        //test_spectrum.peakList[0][0] = 90.055; test_spectrum.peakList[0][1] = 30;
        //test_spectrum.peakList[1][0] = 161.055; test_spectrum.peakList[1][1] = 30;
        //test_spectrum.peakList[2][0] = 232.055; test_spectrum.peakList[2][1] = 30;
        //test_spectrum.peakList[3][0] = 303.055; test_spectrum.peakList[3][1] = 30;
...
#pragma omp parallel for num_threads(6)
        ...
        //for(int spec_idx = 0; spec_idx < 100; spec_idx++){
            ...
            montecarlo_spectrum(this->specs[spec_idx], model, ions_to_extract, allIons, band_mean, band_variance,  insertion_probability, deletion_probability, this->specs[spec_idx].psmList.front()->SLGF_distribution, histograms, 4);
        }
    }
	...
	SpectralLibrary temp;
        //train_library_montecarlo(model, ions_to_extract, allIons, temp);
	train_library_dynamicprogramming(model, ions_to_extract, allIons, temp);
        
        return;
        DEBUG_MSG("DEBUG SLGF Generation");
        vector<float> debug_intensities;
        vector<pair<float, float> > debug_SLGF_out;
        vector<float> band_mean;
        vector<float> band_variance;
        vector<pair<float, float> > insertion_probability;
        
        get_band_meanvariance(band_mean, band_variance, histograms);
        
        debug_intensities.push_back(1.f);
        debug_intensities.push_back(1.f);
        debug_intensities.push_back(1.f);
        debug_intensities.push_back(1.f);
        debug_intensities.push_back(1.f);
        dynamicprogramming_ionvector(debug_intensities, 
                                     band_mean, 
                                     band_variance,
                                     insertion_probability,
                                     deletion_probability,
	...
	DEBUG_MSG("START Training");
        ...
        for ( it=same_annotation_clusters.begin() ; it != same_annotation_clusters.end(); it++ ){
            ...
            for(int cluster_spec_idx1 = 0; cluster_spec_idx1 < (*it).second.size(); cluster_spec_idx1++){
                for(int cluster_spec_idx2 = 0; cluster_spec_idx2 < (*it).second.size(); cluster_spec_idx2++){
                    if(cluster_spec_idx1 == cluster_spec_idx2) continue;
                    
                    psmPtr training_library_psm1 = (*it).second[cluster_spec_idx1]->psmList.front();
                    psmPtr training_library_psm2 = (*it).second[cluster_spec_idx2]->psmList.front();
                    
                    float dot_product = spectrum_similarity_nosqrt(training_library_psm1, training_library_psm2, create_deliminated_aminoacids(library_annotation).size(), model, ions_to_extract, allIons);
                    
                    DEBUG_MSG("QUADRATICSIM\t"<<dot_product<<"\t"<<training_library_psm1->m_spectrumFile<<"\t"<<training_library_psm1->m_spectrum->scan<<"\t"<<training_library_psm2->m_spectrumFile<<"\t"<<training_library_psm2->m_spectrum->scan);
