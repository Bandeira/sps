...
cerr<<" -- ExecHomologyAssembly: matchedProts["<<matchIdx<<"][0/1] = "<<matchedProts[matchIdx][0]<<"/"<<matchedProts[matchIdx][1]<<", matchedAAs["<<matchIdx<<"].size() = "<<matchedAAs[matchIdx].size()<<endl;
  	...
  	if(m_params.exists("OUTPUT_MATCHES_REF_MIDX"))
  		matchSet.matchedPeaks.SaveSpecSet_pklbin(m_params.getValue("OUTPUT_MATCHES_REF_MIDX").c_str());
  ...
  //		cout << "Loading spectral networks pairs complete (" << pairsASP.size() << " pairs)\n";
  		...
  		SpecSet snMatched;
  	    if(snMatched.LoadSpecSet_pklbin(m_params.getValue("INPUT_SPECNETS_MATCHED").c_str())<=0)
  ...
  //		cout << "Loading full set of spectral networks pairs complete (" << pairsASP_all.size() << " pairs)\n";
      ...
      ofstream dbg_output;   dbg_output.open("dbg_glues.txt");
...
*/
  	...
  	//   csps_matches.matchedPeaks.SaveSpecSet_pklbin((string(m_params.getValue("OUTPUT_MATCHES_CSPS"))+string("_midx.pklbin")).c_str());
  	//   Save_binArray((string(m_params.getValue("OUTPUT_MATCHES_CSPS"))+string("_mp.bin")).c_str(), csps_matches.specProtMatches);
  	// }
  	// pathSpectra.consensus.SaveSpecSet_pklbin(m_params.getValue("OUTPUT_SPECS"));
    ...
    if (!m_params.exists("INPUT_SPECS_PKLBIN") or (m_spectra->LoadSpecSet_pklbin(m_params.getValue("INPUT_SPECS_PKLBIN").c_str())<=0 or m_spectra->size()==0)) {
...
*/
	...
	bool ExecHomologyAssembly::saveOutputData(void)
	{
		if(m_contigs and m_params.exists("OUTPUT_SPECS"))
			m_contigs->SaveSpecSet_pklbin(m_params.getValue("OUTPUT_SPECS").c_str());

		if(m_cspsMatchedIndices and m_params.exists("OUTPUT_MATCHES_CSPS_MIDX"))
			m_cspsMatchedIndices->SaveSpecSet_pklbin(m_params.getValue("OUTPUT_MATCHES_CSPS_MIDX").c_str());
