...
namespace specnets
{
  ...
  bool ExecSpectralLibraryDecoyCreation::invoke(void){
    ...
    //Converting annotations to specnets
    /*for(int i = 0; i < m_library.size(); i++){
        cout<<m_library[i].psmList.front()->m_annotation<<endl;
        string orig_annot = m_library[i].psmList.front()->m_annotation;
        orig_annot = "*." + orig_annot + ".*";
        string specnets_annot;
        PeptideSpectrumMatch::inspectToSpecNets(orig_annot, specnets_annot);
        cout<<specnets_annot<<endl;
    }*/
