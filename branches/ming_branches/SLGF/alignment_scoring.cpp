...
namespace specnets
{
	...
	*/
	...
	//#endif
		...
		else { spec1sym.resize(spec1->size()); for(unsigned int i=0; i<spec1sym.size(); i++) spec1sym[i]=(*spec1)[i]; }
	...
	debug<<"--- added peaks["<<idxPeaks-1<<"] = "<<peaks[idxPeaks-1]<<endl;
		...
		Spectrum peaksTmp;   for(int i=0; i<peaks.size(); i++) peaksTmp.insertPeak(peaks[i],common_scores[i], 0);
		vector<int> indices;
		if (forceSymmetry) {
