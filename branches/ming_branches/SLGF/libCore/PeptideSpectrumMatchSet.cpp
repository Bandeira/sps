...
namespace specnets
{
                                                        ...
                                                        bool zeroIndexed)
  {
    ...
    for (int i = 0; i < header.size(); i++)
    {
      ...
      else if ((header[i].compare("#SpectrumFile") == 0) or (header[i].compare("#SpecFile") == 0))
      {
        spectrumFileIndex = i;
      }
      else if ((header[i].compare("Annotation") == 0) or (header[i].compare("Peptide") == 0))
      {
        annotationIndex = i;
      }
      else if (header[i].compare("Charge") == 0)
                   ...
                   )
  {
    ...
    if(strictenvIndex != -1){
      if (!sscanf(line[strictenvIndex].c_str(), "%f", &(currMatch->m_strict_envelope_score)))
      {
        ERROR_MSG("Unable to get strict envelope score");
        return false;
      }
    ...
    if(unstrictenvIndex != -1){
      if (!sscanf(line[unstrictenvIndex].c_str(), "%f", &(currMatch->m_unstrict_envelope_score)))
      {
    ...
    if(spectrumFDRIndex != -1){
        //cout<<"READINGFDR\t"<<line[spectrumFDRIndex]<<"\t";
      if (!sscanf(line[spectrumFDRIndex].c_str(), "%f", &(currMatch->m_fdr)))
      {
        ERROR_MSG("Unable to get FDR");
        return false;
      }
      //cout<<currMatch->m_fdr<<endl;
    }
    else{
        currMatch->m_fdr = 0.f;
    }
