...
namespace specnets
{
  ...
  bool SpecSet::extractSpectra(SpecSet& outputSpecs, Spectrum::FragType& fragType) {

	  bool foundSpecs = false;
          ...
          != 0)
      {
        ...
        if (strncmp("SLGF=", line, sizeof("SLGF=") - 1) == 0){
            if(specs[specIdx].psmList.size() > 0){
                ...
                //DEBUG_MSG(slgf_prob_pair.substr(0, slgf_prob_pair.find(":")).c_str());
                //DEBUG_MSG(slgf_prob_pair.substr(slgf_prob_pair.find(":")+1).c_str());
  ...
  // -------------------------------------------------------------------------
  unsigned int SpecSet::LoadSpecSet_prmsv3(const char *filename, vector<vector<string> >* prmOrigins)
  {
    unsigned int specIdx, lineIdx, peakIdx;
    list<unsigned int> specSizes;
    char *line;
                                                        ...
                                                        const char * annotation_filename)
  {
    ...
    PeptideSpectrumMatchSetSpectralLibraryLoader psm_set;
    //psm_set.loadSpecnetsResultsFile(annotation_filename);
    psm_set.loadMSGFDBResultsFile(annotation_filename, false, false, false);
      ...
      if (specs[i].psmList.size() > 0)
      {
        ...
        for (it = specs[i].psmList.begin(); it != specs[i].psmList.end(); it++)
        {
          ...
          if ((*it)->m_submission_metadata.size() == (*it)->m_organism.size() 
              && (*it)->m_submission_metadata.size() == (*it)->m_compound_name.size()
              && (*it)->m_submission_metadata.size() == (*it)->m_smiles.size()
              && (*it)->m_submission_metadata.size() == (*it)->m_InChI.size()
              && (*it)->m_submission_metadata.size() == (*it)->m_InChI_Aux.size()){
              
              for(int submission_idx = 0; submission_idx < (*it)->m_submission_metadata.size(); submission_idx++){
                  fprintf(output, "SUBMISSION_METADATA=%s\n", (*it)->m_submission_metadata[submission_idx].c_str());
                          ...
                          const char * peaksFilename /* = 0x0 */)
  {
    ...
    unsigned int *numPeaks = (unsigned int *) malloc(sizeof(int)
        * numSpecs);
    ...
    fwrite(numPeaks, sizeof(int), numSpecs, fp); // Number of peaks per spectrum in the file
  ...
  bool SpecSet::loadPklBin_2(FILE* fp, int numSpecs, char subversion)
  {
	...
	if (subversion > 0) {
		...
		unsigned short* numPeaks_short = (unsigned short *) malloc(
				sizeof(unsigned short) * numSpecs);
