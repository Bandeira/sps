...
void RendererGnu::createGnuplotCommandList(void)
{
  ...
  if(m_rendererImage.fileType.compare("eps") != 0) {
    ...
    m_gnuplotQueue << "\" ";
    m_gnuplotQueue << m_rendererImage.fontSize;
  }
  ...
  if(m_rendererImage.fileType.compare("eps") == 0) {
    ...
    m_gnuplotQueue << " size ";
    m_gnuplotQueue << m_rendererImage.imageSizeX * m_rendererImage.zoom;
    m_gnuplotQueue << ",";
    m_gnuplotQueue << m_rendererImage.imageSizeY * m_rendererImage.zoom;
  }
  m_gnuplotQueue << "\n";
...
void RendererGnu::writeGnuplotFile(void)
{
  ...
  gnuplotCmdFile.open(m_gnuplotCommandFileName.c_str(), fstream::in | fstream::out | fstream::trunc);
...
{
  // defines command line string
  string aux = m_rendererLocation;
  aux += DIRECTORY_SEPARATOR;
  aux += GNU_PLOT_COMMAND;
  aux += '"';
  aux += m_gnuplotCommandFileName;
  aux += '"';
  // Call GnuRenderer
  int ret = system( aux.c_str() );
}
////////////////////////////////////////////////////////////////////////////////
