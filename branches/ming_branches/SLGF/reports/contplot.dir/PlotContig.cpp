...
void PlotContig::drawSequences(void)
{
  ...
  if(m_denovo.length()) {
    ...
    ContigSequence seq(m_rendererObject, &m_deNovoIntervals, previous, &m_denovo, offset, "de Novo:", coord, coordPrev, drawLabels, m_mzLowerLimit, m_mzUpperLimit, "#A00000");
  ...
  if(m_homolog.length()) {
    ...
    ContigSequence seq(m_rendererObject, &m_homologIntervals, previous, &m_homolog, offset, "Homolog:", coord, coordPrev, drawLabels, m_mzLowerLimit, m_mzUpperLimit, "#A00000");
    seq.setPeakMassTol(m_peakMassTol);
    seq.setZoom(m_rendererImage.zoom);
    seq.setImageDimensions(m_rendererImage.imageSizeX, m_rendererImage.imageSizeY);
    seq.setPixelSize(m_rendererImage.pixelWidth, m_rendererImage.pixelHeight);
  ...
  if(m_reference.length()) {
    ...
    ContigSequence seq(m_rendererObject, &m_referenceIntervals, previous, &m_reference, offset, "Reference:", coord, coordPrev, drawLabels, m_mzLowerLimit, m_mzUpperLimit, "#A00000");
    seq.setPeakMassTol(m_peakMassTol);
    seq.setZoom(m_rendererImage.zoom);
    seq.setImageDimensions(m_rendererImage.imageSizeX, m_rendererImage.imageSizeY);
    seq.setPixelSize(m_rendererImage.pixelWidth, m_rendererImage.pixelHeight);
  ...
  if(m_user.length()) {
    ...
    previous = &m_deNovoIntervals;
    coordPrev = ntp + disp;
    ContigSequence seq(m_rendererObject, &m_userIntervals, previous, &m_user, offset, "User:", coord, coordPrev, drawLabels, m_mzLowerLimit, m_mzUpperLimit, "#4040F0", bend);
    seq.setPeakMassTol(m_peakMassTol);
    seq.setZoom(m_rendererImage.zoom);
    seq.setImageDimensions(m_rendererImage.imageSizeX, m_rendererImage.imageSizeY);
    seq.setPixelSize(m_rendererImage.pixelWidth, m_rendererImage.pixelHeight);
...
void PlotContig::drawSpectrum(int spectrumIndex, int spectrumCount)
{
  ...
  //cout << "size draw " << spectrumIndex << ":  " << sp->size() << endl;
  ...
  double sx = abs(mzUpperLimit - (*sp)[0][0]) / denom;
  double sy = (1.0 / nps) * nys;


  double factor = m_mzUpperLimit / mzUpperLimit;
