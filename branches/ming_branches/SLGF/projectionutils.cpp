...
//Reentrant Function
float spectrum_similarity(psmPtr library, psmPtr query, int peptideLength, MS2ScoringModel &model,
        vector<string> &ionsToExtract, string allIons){

    psmPtr psm1temp(new PeptideSpectrumMatch);
    psmPtr psm2temp(new PeptideSpectrumMatch);
    
    Spectrum temp_spectrum1 = *(library->m_spectrum);
    Spectrum temp_spectrum2 = *(query->m_spectrum);
    
    Spectrum * old_spectrum1 = library->m_spectrum;
    Spectrum * old_spectrum2 = query->m_spectrum;
    
    psm1temp->m_spectrum = &temp_spectrum1;
    psm2temp->m_spectrum = &temp_spectrum2;
    psm1temp->m_annotation = library->m_annotation;
    ...
    library->m_spectrum = old_spectrum1;
...
float spectrum_similarity_sqrt_librarypeaks_isocombine(psmPtr library,
    ...
    deleteIons(psm1temp,peptideLength,model,ionsToExtract_precursor);
    ...
    for(int i = 0; i < ion_intensities1.size(); i++){
        if(ion_intensities1[i] < 0.0001){
            ion_intensities2[i] = 0.f;
        }
    }
...
float spectrum_similarity_sqrt_librarypeaks(psmPtr library,
    ...
    for(int i = 0; i < ion_intensities2.size(); i++){
        DEBUG_MSG(ion_intensities2[i]<<"\t"<<ion_intensities1[i]);
    }
    
    DEBUG_MSG("COSINE NO SPEED\t"<<cosine_val);*/
...
/*
    Assume Preprocessing already done for library spectrum, including ion extraction
*/
...
float spectrum_similarity_sqrt_librarypeaks_isocombine_speed_optimization(psmPtr library,
    ...
    for(int i = 0; i < library->m_ion_extraction.size(); i++){
        //DEBUG_MSG(i<<"\t"<<library->m_ion_extraction[i]);
    }
    ...
    //DEBUG_MSG("AFTRE ION EXTRACTION");
    
    //DEBUG_MSG(library->m_ion_extraction.size());
    //DEBUG_MSG(psm2temp->m_ion_extraction.size());
...
float spectrum_similarity_nosqrt(psmPtr psm1, psmPtr psm2, int peptideLength, MS2ScoringModel &model,
        vector<string> &ionsToExtract, string allIons){
...
float full_spectrum_similarity_nosqrt(Spectrum spec1, Spectrum spec2){
    ...
    preprocess_spectrum_intensities(&spec1_temp, 0,0);
    preprocess_spectrum_intensities(&spec2_temp, 0,0);
...
void printIon(vector<string> &ionsToExtract, vector<float> ions) {
    int peptide_length = ions.size()/ionsToExtract.size();
    for(int i = 0; i < ions.size(); i++){
        DEBUG_MSG(ions[i]<<"\t"<<ionsToExtract[i/peptide_length]<<"\t"<<i%peptide_length + 1);
    }
        
}

void printDoubleIon(vector<string> &ionsToExtract, vector<float> library, vector<float> query){
    if(library.size() != query.size()) return;
    int peptide_length = library.size()/ionsToExtract.size();
    for(int i = 0; i < library.size(); i++){
        DEBUG_MSG("\t"<<library[i]<<"\t"<< query[i] <<"\t"<<ionsToExtract[i/peptide_length]<<"\t"<<i%peptide_length + 1);
    }
}
...
void montecarlo_spectrum(Spectrum &spectrum, MS2ScoringModel model, vector<string> ions_to_extract, string allIons, vector<float> ion_mean, vector<float> ion_variance, vector< pair < float, float > > insertion_probability , vector< pair < float, float > > deletion_probability, vector< pair< float, float> > &output_distribution, vector<vector<pair<float, float> > > histograms, int training_model){
    ...
    float max_intensity = max_ion_intensity(ion_mass_intensity_pair_vector);
    
    //for(int i = 0 ; i < 100000; i++){
    //    float intensity_variation = random_ion_variance(histograms, 0);
    //    cout<<log(intensity_variation)/log(2)<<endl;
    //}
    
    vector<float> similarity_samples;
    
    for(int iteration = 0; iteration < iterations; iteration++){
        vector<float> random_varied_spectrum;
        for(int ion_idx = 0; ion_idx < ion_size; ion_idx++){
            float ion_intensity = ion_mass_intensity_pair_vector[ion_idx];
            int band_idx = min( ((int)(min(ion_intensity/max_intensity, 0.999999f)/(1.0/ion_variance.size()))), ((int)(ion_variance.size() - 1)));
            float intensity_variation = random_ion_variance(histograms, band_idx);
            
            float varied_ion_intensity = intensity_variation * ion_intensity;
            random_varied_spectrum.push_back(varied_ion_intensity);
        }
        normalize_extracted_ions(random_varied_spectrum);
        float montecarlo_sim = cosine(random_varied_spectrum, ion_mass_intensity_pair_vector);
        similarity_samples.push_back(montecarlo_sim);
        //DEBUG_MSG("MONTECARLO_SIM\t"<<montecarlo_sim);
    }
    
    
    vector<pair<float, float> > sim_histogram = create_histogram2(100, 0.f, 1.f, similarity_samples, true);
    output_distribution.clear();
    for(int i = 0; i < sim_histogram.size(); i++){
        output_distribution.push_back(sim_histogram[i]);
    }
...
void dynamicprogramming_spectrum(Spectrum &spectrum, MS2ScoringModel model, vector<string> ions_to_extract, string allIons, vector<float> ion_mean, vector<float> ion_variance, vector< pair < float, float > > insertion_probability , vector< pair < float, float > > deletion_probability, vector< pair< float, float> > &output_distribution, vector<vector<pair<float, float> > > histograms, int training_model){
    ...
    /*
    for(int i = 0; i < psm->m_peakAnnotations.size(); i++){
        if(psm->m_peakAnnotations[i].first != NULL){
            DEBUG_MSG(psm->m_peakAnnotations[i].first->name<<"\t"<<(*(psm->m_spectrum))[i][0]<<"\t"<<(*(psm->m_spectrum))[i][1]);
        }
    }*/
    
    /*for(int i = 0; i < ion_mass_intensity_pair_vector.size(); i++){
        DEBUG_MSG(ion_mass_intensity_pair_vector[i]);
    }*/
    
    
    //combine_isotopic(ions_to_extract, ion_mass_intensity_pair_vector, create_deliminated_aminoacids(psm->m_annotation).size());
...
vector<pair<float, float> > create_histogram2(int buckets, float start_range, float end_range, vector<float> input, bool normalize){
    vector<pair<float, float> > histogram;
    float bucket_size = (end_range - start_range)/buckets;
    for(int i = 0 ; i < buckets; i++){
        pair<float, float> histogram_cell;
        histogram_cell.first = start_range + i*bucket_size;
        histogram_cell.second = 0;
        histogram.push_back(histogram_cell);
    }
    ...
    int inbounds_count = 0;
    for(int i = 0 ; i < input.size(); i++){
        int histogram_idx = (input[i]-start_range)/bucket_size;
        if(histogram_idx >= buckets || histogram_idx < 0) continue;
...
float percent_explained_intensity(psmPtr psm, 
    ...
    psmPtr psm1temp(new PeptideSpectrumMatch);
    Spectrum temp_spectrum1 = *(psm->m_spectrum);
    Spectrum * old_spectrum1 = psm->m_spectrum;
    psm1temp->m_spectrum = &temp_spectrum1;
    psm1temp->m_annotation = psm->m_annotation;
    ...
    float accumulate_intensity = 0.f;
    for(int i = 0; i < ion_intensities1.size(); i++){
        accumulate_intensity += ion_intensities1[i] * ion_intensities1[i];
    }
    accumulate_intensity = sqrt(accumulate_intensity);

    psm->m_spectrum = old_spectrum1;

    return accumulate_intensity;
}

float percent_sequence_similarity(string peptide1, string peptide2){
    peptide1 = remove_annotation_ends(peptide1);
    peptide2 = remove_annotation_ends(peptide2);
    if(peptide1.length() != peptide2.length()){
        return 0.f;
    }
    
    //converting all I's to L's
    ...
    vector<string> peptide1_deliminated = create_deliminated_aminoacids(peptide1);
    vector<string> peptide2_deliminated = create_deliminated_aminoacids(peptide2);
    
    int difference_count = 0;
    int similarity_count = 0;
    for(int i = 0; i < peptide1_deliminated.size(); i++){
        if(peptide1_deliminated[i] != peptide2_deliminated[i]){
            difference_count++;
        }
	else{
	    similarity_count++;
    ...
    return ((float)similarity_count)/((float)peptide1_deliminated.size());
}
...
double random_ion_variance(vector<vector<pair<float, float> > > &histograms, int bandidx){
    vector<pair <float, float> > band_cumu;
    
    float cumu_val = 0.f;
    for(int i = 0; i < histograms[bandidx].size(); i++){
        pair<float, float> cumu_pair;
        cumu_pair.first = histograms[bandidx][i].first;
        cumu_val += histograms[bandidx][i].second;
        cumu_pair.second = cumu_val;
        band_cumu.push_back(cumu_pair);
    }
    
    float random_value = (float)rand()/(float)RAND_MAX;
    //DEBUG_MSG(random_value);
    
    for(int i = 0; i < band_cumu.size(); i++){
        if(random_value < band_cumu[i].second){
            //DEBUG_MSG(band_cumu[i].first<<"\t"<<band_cumu[i].second);
            return pow(2.0,band_cumu[i].first);
        }
    }
    
    
    return pow(2.0, band_cumu[band_cumu.size() - 1].first);
}

vector<float> reverse_vector(vector<float> input){
    vector<float> return_vector;
    for(int i = input.size()-1; i>=0; i--){
        return_vector.push_back(input[i]);
    }
...
int spectrum_signal_peak_count(MS2ScoringModel &model, vector<string> &ionsToExtract, string allIons, psmPtr psm, float percent_minimum){
    psmPtr psm1temp(new PeptideSpectrumMatch);
    int peptideLength = create_deliminated_aminoacids(psm->m_annotation).size();
    ...
    int signal_count = 0;
    for(int i = 0; i < ion_intensities1.size(); i++){
        if(ion_intensities1[i] > percent_minimum){
            signal_count ++;
        }
    }
