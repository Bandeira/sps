...
namespace specnets
{
                                      ...
                                      MS2ScoringModel * model)
  {
    ...
    if (m_starSpectraStatsHeader == NULL) // if stats vectors not yet defined.
    {
    ...
    *currPsm = psm;
    ...
    currPSMSet[0]->m_spectrum = msSpectra;
    ...
    currPSMSet[0]->m_spectrum = scoredSpectra;
    ...
    currPSMSet[0]->m_spectrum = starSpectra;
                                               ...
                                               MS2ScoringModel * model)
  {
    ...
    msPeptideResults.addSpectra(specsMsSpectra);
    ...
    scoredPeptideResults.addSpectra(specsScoredSpectra);
    ...
    starPeptideResults.addSpectra(starSpectra);
  ...
  bool ExecSvmStatistics::invoke(void)
  {
    ...
    {
      vector<float> statsVector(keyMapping.back()[2]); //Last key index should be largest.
      vector<float> scaledVector(keyMapping.back()[2]);
  ...
  bool ExecSvmStatistics::saveOutputData(void)
  {
    if (m_params.exists("OUTPUT_SPECTRA_SVM_SCORES"))
    {
      ...
      DEBUG_MSG("Opening output svm scores file...");
      outputFile.open(m_params.getValue("OUTPUT_SPECTRA_SVM_SCORES").c_str(),
                      ios::out | ios::trunc);
      if (outputFile.fail())
      {
        ERROR_MSG("Unable to open stats file! " << m_params.getValue("OUTPUT_SPECTRA_SVM_SCORES"));
        return false;
      }

      if (outputFile.is_open() && outputFile.good())
      {
        outputFile << "Scan#\tAnnotation\tCharge\tMQScore\tProtein" << endl;

        for (int i = 0; i < m_peptideOutputResults->size(); i++)
        {
          psmPtr currResult = m_peptideOutputResults->m_psmSet[i];
          outputFile << currResult->m_scanNum; //scan number
          outputFile << "\t";
          outputFile << currResult->m_annotation; //spectrum annotation
          outputFile << "\t";
          outputFile << currResult->m_charge; //spectrum annotation
          outputFile << "\t";
          outputFile << currResult->m_score; //score
          outputFile << "\t";
          outputFile << currResult->m_protein;
          outputFile << endl;
        }
      }
      else
