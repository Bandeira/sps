...
/*	ofstream output(alignsOutFN);  if (output.fail()) { cerr<<"Error opening "<<alignsOutFN<<endl; return(-1); }
//	output<<okCount<<endl;
	for(unsigned int i=0; i<aligns.size(); i++) 
...
*/
	...
	FILE *fp = fopen(alignsOutFN,"w");  if ((int)fp==0) { cerr<<"Error opening "<<alignsOutFN<<endl; return(-1); }
