...
namespace specnets
{
   ...
   * @return
   */
  void DeconvSpectrum::AssignChargesKLDiv(IsoEnvelope* env,
                                          float threshold)
  {
  ...
  void DeconvSpectrum::ConvertPeaksChargeOne(Spectrum& putSpec)
  {
    ...
    for (int i = 0; i < size(); i++) {
      ...
      if (thisPeak->charge == 0) {
      ...
      else { // or add peaks that have assigned charge
        putSpec[peakIdxUse][0]
            = GetMonoisotopicMass(peakList[i][0], thisPeak->charge);
        putSpec[peakIdxUse][1] = thisPeak->binnedIntensity;
        putSpec.setTolerance(peakIdxUse, peakTols[i] * ((float) thisPeak->charge));
        ...
        if (MZRange::EqualWithinRange(peakList[closestIdx][0], putSpec[peakIdxUse][0], pkTolUse)) {
          putSpec[peakIdxUse][0] = (peakList[closestIdx][0] + putSpec[peakIdxUse][0])/2.0;
