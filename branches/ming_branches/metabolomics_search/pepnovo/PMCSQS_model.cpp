								   ...
								   vector< vector<float> >* inputWeights)
{
	...
	vector< vector< MlDataSet > > mlDataSets;
	...
	mlDataSets.resize(maximalChargeWithModels_+1);
	...
	for (charge=0; charge<=maximalChargeWithModels_; charge++)
	{
		...
		mlDataSets[charge].resize(numSizes+1);
		...
		//	samples[charge][sizeIndex].resize(sl.getNumHeaders());
			mlDataSets[charge][sizeIndex].reserve(sl.getNumHeaders());
			int i;
			for (i=0; i<sl.getNumHeaders(); i++)
			{
			...
			//	samples[charge][sizeIndex][i].label = label;
				...
				mlDataSets[charge][sizeIndex].addSample(sample);
				sample.print();
	...
	for (charge=0; charge<=maximalChargeWithModels_; charge++)
	{
		cout << charge;
		size_t i;
		for (i=0; i<mlDataSets[charge].size(); i++)
			cout << "\t" << mlDataSets[charge][i].getNumSamples();
	...
	for (charge=1; charge<=maximalChargeWithModels_; charge++)
	{
		...
		for (sizeIndex=0; sizeIndex<=numSizes; sizeIndex++)
		{
			...
			mld.addSamples(mlDataSets[0][sizeIndex].getSamples(),1);
			mld.addSamples(mlDataSets[charge][sizeIndex].getSamples(),0);
			mld.tallyClassStatistics();

			char outPath[256];
			sprintf(outPath,"DataFiles/%s_SQS_train_0_%lu_%d.txt",config->get_resource_dir().c_str(),
				(unsigned long)charge, sizeIndex);

			mld.writeWholeDataFile(outPath);
