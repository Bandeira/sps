...
void PRMSpecEdge::appendEdges(PRMSpecEdge* edge1, PRMSpecEdge* edge2,
	...
	if (edge2->spec1 == node2) {
		forwardShift = edge1->getShift(node1, node2) + edge2->getShift(node2,
				node3);
		reversedShift = edge1->getReversedShift(node1, node2)
				+ edge2->getReversedShift(node2, node3);

		outEdge1Plus2->spec1 = node1;
		outEdge1Plus2->spec2 = node3;
	} else {
		if (edge2->spec2rev) {
			forwardShift = edge2->getShift(node3, node2)
					+ edge1->getReversedShift(node2, node1);
			reversedShift = edge2->getReversedShift(node3, node2)
					+ edge1->getShift(node2, node1);
		} else {
			forwardShift = edge2->getShift(node3, node2) + edge1->getShift(
					node2, node1);
			reversedShift = edge2->getReversedShift(node3, node2)
					+ edge1->getReversedShift(node2, node1);
		}

		outEdge1Plus2->spec1 = node3;
		outEdge1Plus2->spec2 = node1;
	}

	outEdge1Plus2->shift1 = forwardShift;
	outEdge1Plus2->shift2 = reversedShift;
	outEdge1Plus2->spec2rev = edge2->spec2rev;
}

PRMSpecEdge& PRMSpecEdge::operator=(const PRMSpecEdge &other) {
	index = other.index;
	(SpectrumPair&) (*this) = (SpectrumPair&) other;
}
...
void PRMSpecEdge::initializeEdge(const int idx, const SpectrumPair& contigPair,
