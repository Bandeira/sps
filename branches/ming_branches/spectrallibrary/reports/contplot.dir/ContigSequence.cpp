...
///////////////////////////////////////////////////////////////////////////////
void ContigSequence::drawExec(void)
{
  // Check for spectrum
...
void ContigSequence::drawDashedLines(void)
{
  ...
  for(int i = 0 ; i < m_breaks->size() ; i++)
    ...
    for(int j = 0 ; j < m_previousBreaks->size() ; j++)
      ...
      if( fabs( (*m_breaks)[i] - (*m_previousBreaks)[j]) < m_peakMassTol)

        // draw the vertical dashed line
        if(m_bendLine)
          addDashedSegmentBent( m_offset + (*m_breaks)[i], m_yPosition, m_offset + (*m_previousBreaks)[j], m_yPositionPrevious );
        else
          addDashedSegment( m_offset + (*m_breaks)[i], m_yPosition, m_offset + (*m_previousBreaks)[j], m_yPositionPrevious );
}
///////////////////////////////////////////////////////////////////////////////
//
///////////////////////////////////////////////////////////////////////////////
...
void ContigSequence::breakSequence(vector<string> &oSequenceMapping, string &iSequence, bool omitValues, bool useParantesis)
{
  ...
  while(lastPosition < iSequence.size()) {
    ...
    // in case of mass
      ...
      // get contents
      if(!omitValues)
        cell += iSequence.substr(lastPosition+1, currentPosition - lastPosition - 1);
      ...
      if(iSequence[currentPosition] == ',') {
        ...
        // get contents
        if(!omitValues)
          cellAux += iSequence.substr(lastPosition, currentPosition - lastPosition);
