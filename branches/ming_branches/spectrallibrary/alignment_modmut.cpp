...
namespace specnets
{
                                 ...
                                 SpecSet &in_matchedPeaks)
  {
    ...
    for (unsigned int matchIdx = 0; matchIdx < matchedPeaks.size(); matchIdx++) {
        ...
        if (matchedPeaks[matchIdx].size() > 0) {
          curMatch.set((int) round(matchedPeaks[matchIdx][0][1]),
                       (int) matchIdx);
  ...
  bool AMM_match_set::SetMatches(SpecSet * specset)
  {
    ...
    for (unsigned int i = 0; i < specset->size(); i++) {
      ...
      for (unsigned int j = 0; j < peakListSize; j++) {
        matchedPeaks[i][j][0] = (*specset)[i].psmList.front()->m_matchedPeaks[j][0];
        matchedPeaks[i][j][1] = (*specset)[i].psmList.front()->m_matchedPeaks[j][1];
      }
    }
                                  ...
                                  const char *matchedPeaksIdx)
  {
    ...
    if (matchedPeaks.LoadSpecSet_pklbin(matchedPeaksIdx) <= 0
    ...
    for (unsigned int pIdx = 0; pIdx < matches.size(); pIdx++)
      for (specsIter = matches[pIdx].begin(); specsIter != matches[pIdx].end(); specsIter++) {
        ...
        for (; otherIter != matches[pIdx].end() and (*otherIter)[0]
            <= lastProtMatch; otherIter++) { // Spectra overlap, look for matches
      ...
      for (dbSpecIdx = 0; dbSpecIdx < dbSpec.size(); dbSpecIdx++) {
        ...
        for (specIdx = 0; specIdx < spec.size(); specIdx++) {
          ...
          for (mPeaksIdx = 0; mPeaksIdx < min(minMatchedPeaks, specIdx + 1); mPeaksIdx++) {
            ...
            // 0 Mods - find other peaks at the same distance in both spectra
            predSpecIdx = specIdx - 1;
            while(predSpecIdx>=0 and fabs(spec[specIdx][0] - spec[predSpecIdx][0])<minSpecDist-tolerance)
            	predSpecIdx--;
            predDbSpecIdx = dbSpecIdx - 1;
            usedModSize = 0;
            // Zero mods case also needs to search for best predecessor because the previous peaks may have been
        ...
        for (specIdx = 0; specIdx < spec.size(); specIdx++) {
          ...
          if (!startFlags[specIdx][dbSpecIdx]) {
            for (mPeaksIdx = 0; mPeaksIdx < min(minMatchedPeaks, specIdx + 1); mPeaksIdx++) {
              ...
              // Update bestSoFar; give precedence to options with same score and less mutations or more matched peaks
              if (modIdx > 0)
                bestSoFar[modIdx][dbSpecIdx][specIdx][mPeaksIdx].replaceWithMax(bestSoFar[modIdx - 1][dbSpecIdx][specIdx][mPeaksIdx]);
              if (specIdx > 0)
                bestSoFar[modIdx][dbSpecIdx][specIdx][mPeaksIdx].replaceWithMax(bestSoFar[modIdx][dbSpecIdx][specIdx - 1][mPeaksIdx]);
              if (dbSpecIdx > 0)
                bestSoFar[modIdx][dbSpecIdx][specIdx][mPeaksIdx].replaceWithMax(bestSoFar[modIdx][dbSpecIdx - 1][specIdx][mPeaksIdx]);
              bestSoFar[modIdx][dbSpecIdx][specIdx][mPeaksIdx].replaceWithMax(matchMatrix[modIdx][dbSpecIdx][specIdx][mPeaksIdx].score,
                                                                              modIdx,
                                                                              specIdx,
                                                                              dbSpecIdx,
...
*/      
      float score = bestSoFar[modIdx][dbSpecIdx - 1][specIdx - 1][minMatchedPeaks - 1].score;
      if (score != -(float)INT_MAX) {
        ...
        AMM_peak_match & matchPtr = bestSoFar[modIdx][dbSpecIdx - 1][specIdx - 1][minMatchedPeaks - 1];
        while (matchPtr.predSpecIdx >= 0) {
          ...
          matchPtr = matchMatrix[matchPtr.predMods][matchPtr.predDbSpecIdx][matchPtr.predSpecIdx][matchPtr.predMPeaks];
        }
