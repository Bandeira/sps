...
bool ExecReportProteinCoverage::loadInputData(void)
{
 ...
 // **** SPS Contigs ****  //
  ...
  rtrim(spsContigMatchesIndexFileName);
	ret = m_spsContigMatchesIndex.LoadSpecSet_pklbin(spsContigMatchesIndexFileName.c_str());
  if(ret < 0)
    return false;
  ...
  rtrim(spsContigSpectraFileName);
	ret = m_spsContigSpectra.LoadSpecSet_pklbin(spsContigSpectraFileName.c_str());
  if(ret < 0)
    return false;
 ...
 // **** CSPS Contig ****  //
  ...
  rtrim(cspsContigMatchesIndexFileName);
	ret = m_cspsContigMatchesIndex.LoadSpecSet_pklbin(cspsContigMatchesIndexFileName.c_str());
  if(ret < 0)
    return false;
  ...
  rtrim(cspsContigSpectraFileName);
	ret = m_cspsContigSpectra.LoadSpecSet_pklbin(cspsContigSpectraFileName.c_str());
  if(ret < 0)
    return false;
