...
namespace specnets
{
  ...
  bool ExecAlignment::loadInputData(void)
  {
    ...
    if (m_params.exists("INPUT_SPECS_PAIRS"))
    {
      ...
      if (!m_pairAlignments->LoadSpecSet_pklbin(specPairsFilename.c_str())
  ...
  bool ExecAlignment::saveInputData(std::vector<std::string> & filenames)
  {
    ...
    {
      if (!m_inputSpectra->SaveSpecSet_pklbin(spectraFilename.c_str()))
      {
  ...
  bool ExecAlignment::saveOutputData(void)
  {
    ...
    {
      DEBUG_VAR(m_pairAlignments->size());
      m_pairAlignments->SaveSpecSet_pklbin(m_params.getValue("OUTPUT_SPECS").c_str());
    }

    if (m_params.exists("OUTPUT_STARS"))
    {
      DEBUG_VAR(m_starSpectraOnly->size());
      m_starSpectraOnly->SaveSpecSet_pklbin(m_params.getValue("OUTPUT_STARS").c_str());
    }

    if (m_params.exists("OUTPUT_STARS_ALL"))
    {
      DEBUG_VAR(m_starSpectra->size());
      m_starSpectra->SaveSpecSet_pklbin(m_params.getValue("OUTPUT_STARS_ALL").c_str());
    }
  ...
  bool ExecAlignment::loadOutputData(void)
  {
    if (m_params.exists("OUTPUT_SPECS"))
    {
      if (!m_pairAlignments->LoadSpecSet_pklbin(m_params.getValue("OUTPUT_SPECS").c_str()))
      {
    ...
    if (m_params.exists("OUTPUT_STARS"))
    {
      if (!m_starSpectraOnly->LoadSpecSet_pklbin(m_params.getValue("OUTPUT_STARS").c_str()))
      {
    ...
    if (m_params.exists("OUTPUT_STARS_ALL"))
    {
      if (!m_starSpectra->LoadSpecSet_pklbin(m_params.getValue("OUTPUT_STARS_ALL").c_str()))
      {
