...
if(curPair.spec1==6876 or curPair.spec2==6876)
   ...
   */
  // -------------------------------------------------------------------------
  void ProjectSpectrum(SpecSet &specSet,
                       ...
                       unsigned int minNumMatchedPeaks)
  {
    ...
    for (unsigned int p = 1; p < numPeaks; p++)
    {
      ...
      nextMatchedIdxDelta[numPeaks - p - 1] = min(nextMatchedIdxDelta[numPeaks
          - p], matchedIdxDelta[numPeaks - p - 1]);
    }
    ...
    { // This is the last spectrum being projected into - compute match scores
      ...
      for (unsigned int p = 1; p < numPeaks; p++)
      {
        cumFromLeft[p] = cumFromLeft[p - 1] + specLocal[p][1];
      ...
      unsigned int bestLocalDelta = numPeaks + 1, nextPeak, bestNextPeak; // Index of the first peak after the modification
      float bestLocalScore = -1, curScore;
      for (int deltaIdx = (int)numPeaks; deltaIdx >= 0; deltaIdx--)
      {
        if (deltaIdx == (int)numPeaks)
        ...
        else
        {
          ...
          {
            nextPeak = deltaIdx;
            while (nextPeak < numPeaks && (prevMatchedIdx[deltaIdx - 1]
                >= nextMatchedIdxDelta[nextPeak] or specLocal[deltaIdx - 1][0]
                + 56 >= specLocalDelta[nextPeak][0] + curDelta))
