...
namespace specnets
{
  ...
  void PeptideSpectrumMatch::internalCopy(const PeptideSpectrumMatch &other)
  {
    ...
    m_library_metadata = other.m_library_metadata;
  ...
  bool PeptideSpectrumMatch::saveToFile(std::ofstream & ofs)
  {
    ...
    if (m_library_metadata.empty())
    {
      ofs << "N/A";
    }
    else
    {
        ofs << m_library_metadata;
    }
    ...
    if(m_smiles.size() == 0){
        ...
        if(m_smiles[m_smiles.size()-1].length() == 0){
            ofs << "N/A" ;
        }
        else{
            ofs << m_smiles[m_smiles.size()-1];
        }
    }
    ...
    if(m_InChI.size() == 0){
        ...
        if(m_InChI[m_InChI.size()-1].length() == 0){
            ofs << "N/A" ;
        }
        else{
            ofs << m_InChI[m_InChI.size()-1];
        }
    }
