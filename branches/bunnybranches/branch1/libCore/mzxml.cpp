...
template <typename T>
  unsigned int LoadMzxml(const T & xmlFile, SpecSet &specs, vector<short> *msLevels, short minMsLevel)
  {
    specs.resize(0);
...
template
  unsigned int LoadMzxml(char * const & xmlFile, SpecSet &specs, vector<short> *msLevels, short minMsLevel);

template
  unsigned int LoadMzxml(const char * const & xmlFile, SpecSet &specs, vector<short> *msLevels, short minMsLevel);

template
  unsigned int LoadMzxml(const MemBufInputSource & xmlFile, SpecSet &specs, vector<short> *msLevels, short minMsLevel);
...
unsigned int LoadMzxml(const char *filename, SpecSet &specs, const string & sscan, vector<short> *msLevels, short minMsLevel)
{
  ifstream smzxml(filename);

  //static const sps::regex ctag("</?scan");
  //streampos np[] = {grep(smzxml, ("<scan num=\"" + sscan).c_str()), egrep(smzxml, ctag)};
  streampos np[] = {grep(smzxml, ("<scan num=\"" + sscan).c_str()), egrep2(smzxml)};
  ...
  MemBufInputSource cmzxml((const XMLByte *)(ssxml.c_str()), ssxml.size(), __PRETTY_FUNCTION__);

  return LoadMzxml(cmzxml, specs, msLevels, minMsLevel);
}
...
void mzxmlSAX2Handler::startElement(const   XMLCh* const    uri,
    ...
    if(!strcmp(name,"scan")) {
    	if(specs->size()==0) {
	      stringstream aux;
	      aux<<"ERROR: Can't read scans before knowing how many scans in the file (from msRun.scanCount)!\n";
	      throw mzxmlEx(1,aux.str().c_str());
    	}
    	if(numSpecs>=specs->size()) {
	      stringstream aux;
	      aux<<"ERROR: Can't read more scans than indicated in msRun.scanCount! " << numSpecs << " : " << specs->size() << "\n";
	      throw mzxmlEx(1,aux.str().c_str());
    	}
		...
		if(intValue>=minMsLevel) {
	    	...
	    	(*specs)[curSpec.front()].msLevel = max((unsigned int)intValue,(unsigned int)curSpec.size());;
	    	state = STATE_SCAN;
		...
		if(state == STATE_SCAN) {
			...
			(*specs)[curSpec.front()].resize(intValue);
                ...
                xmlStr = XMLString::transcode("retentionTime");   str = XMLString::transcode(attrs.getValue(xmlStr));
    ...
    if(!strcmp(name,"precursorMz")) {
    	...
    	xmlStr = XMLString::transcode("precursorCharge");    str = XMLString::transcode(attrs.getValue(xmlStr));
		(*specs)[curSpec.front()].parentCharge = str?atoi(str):0;

        xmlStr = XMLString::transcode("precursorIntensity");    str = XMLString::transcode(attrs.getValue(xmlStr));
                (*specs)[curSpec.front()].precursor_intensity = str?atof(str):0.0;

		xmlStr = XMLString::transcode("activationMethod");   str = XMLString::transcode(attrs.getValue(xmlStr));
		(*specs)[curSpec.front()].msFragType = Spectrum::FragType_CID;  // Default value
	if(str and strncmp(str,"ETD",3)==0)  {(*specs)[curSpec.front()].msFragType = Spectrum::FragType_ETD;}
		if(str and strcmp(str,"HCD")==0) (*specs)[curSpec.front()].msFragType = Spectrum::FragType_HCD;
    	XMLString::release(&str);    XMLString::release(&xmlStr);
    }
    if(!strcmp(name,"peaks")) {
    	state = STATE_PEAKS;
    	xmlStr = XMLString::transcode("precision");    str = XMLString::transcode(attrs.getValue(xmlStr));
    	if((str?atoi(str):0)!=32) {
	      stringstream aux;
	      aux<<"ERROR: "<<atoi(str)<<"-bit precision not supported, only 32-bit.\n";
	      throw mzxmlEx(1,aux.str().c_str());
    	}
    	XMLString::release(&str);    XMLString::release(&xmlStr);
    }
...
void mzxmlSAX2Handler::characters(const XMLCh* const chars, const XMLSize_t length) {
	char *str = XMLString::transcode(chars);
	if(curSpec.empty() or str!=(char *)0 and (str[0]==(char)0 or str[0]==(char)13 or str[0]==(char)10)) { XMLString::release(&str); return; }
    ...
    if(state == STATE_PRECURSORMZ) {
    	...
    	if(spec->parentCharge>0) spec->parentMass = spec->parentMass * spec->parentCharge - 1.0072763*(spec->parentCharge-1);  // Convert to standard MH+
    }
    ...
    if(state == STATE_PEAKS and spec->size()>0) {
      ...
      stringstream aux;
      aux<<"ERROR: Could not decode peak list for scan "<<spec->scan<<" - needed "<<spec->size() * 8 + 8<<" bytes, got "<<bytesRead<<" bytes ("<<spec->size()<<" peaks).\n";
      throw mzxmlEx(1,aux.str().c_str());
		}
		...
		for(unsigned int peakIdx=0; peakIdx<spec->size(); peakIdx++) {
			for(unsigned int colIdx=0; colIdx<2; colIdx++, valIdx++) {
			    ...
			    try{
			      ...
			      stringstream aux;
			      aux<<"FNB2: "<<peakIdx<<"/"<<spec->size()<<", col "<<colIdx<<"\n";
			      throw mzxmlEx(1,aux.str().c_str());
			    }
			}
		}
