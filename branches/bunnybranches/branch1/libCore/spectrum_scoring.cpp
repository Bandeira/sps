...
namespace specnets
{
  ...
  void MS2ScoringModel::getBreakIonsNames(vector<string> &bIons, vector<string> &yIons) const
  {
	  bIons.clear(); yIons.clear();
	  for (int i = 0; i < probs.size(); i++)
	...
	bool MS2ScoringModel::LoadModel(const char *filename)
	{
		...
		unsigned int pIdx = 0;
		for (list<ftIonFragment>::iterator iter = tmp_probs.begin(); iter != tmp_probs.end(); iter++)
		{
			probs[pIdx++] = (*iter);
		}
										 ...
										 Spectrum *out)
	{
		...
		for (peakIdx = 0; peakIdx < masses.size(); peakIdx++)
		{
			...
			for (curCI = model.probs_ci.begin(); curCI != model.probs_ci.end(); curCI++)
				if (ifIntensity[curCI->fragIndex1] > 0 and ifIntensity[curCI->fragIndex2]
						> 0)
				{
		...
		for (peakIdx = 0; peakIdx < masses.size(); peakIdx++)
		{
			...
			for (ionIdx = 0; ionIdx < model.probs.size(); ionIdx++)
			{
				...
				if (model.probs[ionIdx].prob > 0)
				{
					if (ifIntensity[ionIdx] > 0)
						...
						curScore += log((1 - model.probs[ionIdx].prob)
								/ (1 - model.noiseProb));
				}
			}
			...
			for (curCO = model.probs_co23.begin(); curCO != model.probs_co23.end(); curCO++)
			{
				if (model.probs[curCO->fragIndex2].isNTerm)
					...
					ionMass = ms2in.parentMass - AAJumps::massMH - (massToScore + ms3shift)
							+ model.probs[curCO->fragIndex2].massOffset;
