...
namespace specnets
{
                                           ...
                                           int numConsec)
  {
    ...
    for (unsigned int i = 1; i < size(); i++)
    {
      ...
      if (prevSpec->parentMass == nextSpec->parentMass && (numConsec <= 0
          || (numConsec > 0 && curConsec < numConsec)))
      {
        curConsec++;
  ...
  // -------------------------------------------------------------------------
  unsigned int SpecSet::Load(const char *filename)
  {
    ...
    if(fm.extension.compare("pklbin") == 0)
      return LoadSpecSet_pklbin(filename);

    if(fm.extension.compare("ms2") == 0)
      return LoadSpecSet_ms2(filename);

    if(fm.extension.compare("prmsv3") == 0)
      return LoadSpecSet_prmsv3(filename);

    if(fm.extension.compare("prms") == 0)
  ...
  unsigned int SpecSet::LoadSpecSet_mgf(const char *filename)
  {
    ...
    for (specIdx = 0; specIdx < specs.size(); specIdx++)
    {
      ...
      while (lineIdx < blr.size() and strncmp("END IONS",
                                              blr.getline(lineIdx),
                                              8) != 0)
      {
        ...
        if (strncmp("SOURCE_INSTRUMENT=", line, sizeof("SOURCE_INSTRUMENT=")
            - 1) == 0)
        {
                    ...
                    sizeof("SUBMISSION_METADATA=") - 1) == 0)
        {
          if (specs[specIdx].psmList.size() > 0)
          {
            ...
            specs[specIdx].psmList.front()->m_submission_metadata.push_back(submission_metadata);
          }
        }
        if (strncmp("ORGANISM=", line, sizeof("ORGANISM=") - 1) == 0)
        {
          if (specs[specIdx].psmList.size() > 0)
          {
            ...
            specs[specIdx].psmList.front()->m_organism.push_back(organism);
          }
        }
        ...
        if (strncmp("COMPOUNDNAME=", line, sizeof("COMPOUNDNAME=") - 1) == 0)
        {
          if (specs[specIdx].psmList.size() > 0)
          {
            ...
            specs[specIdx].psmList.front()->m_compound_name.push_back(compound);
          }
        }
        ...
        if (strncmp("SMILES=", line, sizeof("SMILES=") - 1) == 0)
        {
          if (specs[specIdx].psmList.size() > 0)
          {
            ...
            specs[specIdx].psmList.front()->m_smiles.push_back(smiles);
          }
        }
        if (strncmp("INCHI=", line, sizeof("INCHI=") - 1) == 0)
        {
          if (specs[specIdx].psmList.size() > 0)
          {
            ...
            specs[specIdx].psmList.front()->m_InChI.push_back(InChI);
          }
        }
        if (strncmp("INCHIAUX=", line, sizeof("INCHIAUX=") - 1) == 0)
        {
          if (specs[specIdx].psmList.size() > 0)
          {
            ...
            specs[specIdx].psmList.front()->m_InChI_Aux.push_back(InChI_aux);
          }
        }
        ...
        if (strncmp("LIBMETADATA=", line, sizeof("LIBMETADATA=") - 1) == 0)
        {
          if(specs[specIdx].psmList.size() > 0){
              string metadata = line + sizeof("LIBMETADATA=") - 1;
              specs[specIdx].psmList.front()->m_library_metadata = metadata;
          }
        }
  ...
  // -------------------------------------------------------------------------
  unsigned int SpecSet::LoadSpecSet_prmsv3(const char *filename, vector<vector<
      string> >* prmOrigins)
  {
    unsigned int specIdx, lineIdx, peakIdx;
    list<unsigned int> specSizes;
  ...
  // -------------------------------------------------------------------------
  short SpecSet::SaveSpecSet(const char* filename, const char* activation)
  {
    // split the filename into components
    FilenameManager fm(filename);
    ...
    // select load method according to extension
    if(fm.extension.compare("mgf") == 0)
      return SaveSpecSet_mgf(filename, activation);

    if(fm.extension.compare("pkl") == 0)
      return SaveSpecSet_pkl(filename);

    if(fm.extension.compare("pklbin") == 0)
      return SaveSpecSet_pklbin(filename);
    ...
    for (int i = 0; i < specs.size(); i++)
    {
      ...
      if (specs[i].psmList.size() > 0)
      {
        ...
        for (it = specs[i].psmList.begin(); it != specs[i].psmList.end(); it++)
        {
          ...
          for(int SLGF_Distirbution_idx = 0; SLGF_Distirbution_idx < (*it)->SLGF_distribution.size(); SLGF_Distirbution_idx++){
              fprintf(output, "SLGF=%f:%f\n", (*it)->SLGF_distribution[SLGF_Distirbution_idx].first, (*it)->SLGF_distribution[SLGF_Distirbution_idx].second);
          }

          if ((*it)->m_submission_metadata.size() == (*it)->m_organism.size()
              && (*it)->m_submission_metadata.size()
                  == (*it)->m_compound_name.size()
              && (*it)->m_submission_metadata.size() == (*it)->m_smiles.size()
              && (*it)->m_submission_metadata.size() == (*it)->m_InChI.size()
              && (*it)->m_submission_metadata.size()
                  == (*it)->m_InChI_Aux.size())
          {

            for (int submission_idx = 0; submission_idx
                < (*it)->m_submission_metadata.size(); submission_idx++)
            {
              fprintf(output,
                      "SUBMISSION_METADATA=%s\n",
                      (*it)->m_submission_metadata[submission_idx].c_str());
              fprintf(output,
                      "ORGANISM=%s\n",
                      (*it)->m_organism[submission_idx].c_str());
              fprintf(output,
                      "NAME=%s\n",
                      (*it)->m_compound_name[submission_idx].c_str());
              fprintf(output,
                      "SMILES=%s\n",
                      (*it)->m_smiles[submission_idx].c_str());
              fprintf(output,
                      "INCHI=%s\n",
                      (*it)->m_InChI[submission_idx].c_str());
              fprintf(output,
                      "INCHIAUX=%s\n",
                      (*it)->m_InChI_Aux[submission_idx].c_str());
            }
          }
          else
          {
            ERROR_MSG("PSM has inequal number of submissions per field");
          }
  ...
  int SpecSet::compare(SpecSet &toSet, SpecCompareData &cd)
  {
    ...
    if (size1 != size2)
    {
      ...
      aux.push_back(size1);
      aux.push_back(size2);
      cd.push_back(make_pair<int, vector<int> > (-1, aux));
      return -1;
    }
    ...
    for (unsigned i = 0; i < size1; i++)
    {
      ...
      cd.push_back(make_pair<int, vector<int> > (i, aux));
    }
