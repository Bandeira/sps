...
SpsFiles::~SpsFiles()
{
  ...
  if(m_inputMapping != NULL)          delete m_inputMapping;
...
int SpsFiles::loadData(const ReportGeneratorData &reportGeneratorData)
{
  int ret = OK;

  if(loadInputSpectraList(reportGeneratorData.projectDir, reportGeneratorData.filenameInputSpectraList) == ERROR) {
    WARN_MSG("Error loading " << reportGeneratorData.filenameInputSpectraList);
    ret = ERROR;
  }

  if(loadInputSpectraFiles(reportGeneratorData.projectDir, reportGeneratorData.filenameClusterMS) == ERROR) {
    WARN_MSG("Error loading " << reportGeneratorData.filenameClusterMS);
    ret =  ERROR;
  }

  if(loadScanFiles(reportGeneratorData.projectDir, reportGeneratorData.filenameScanFiles) == ERROR)
    WARN_MSG("Error loading " << reportGeneratorData.filenameScanFiles);

  if(loadConsensusSpectraFile(reportGeneratorData.projectDir, reportGeneratorData.filenameConsensusSpectra) == ERROR) {
    WARN_MSG("Error loading " << reportGeneratorData.filenameConsensusSpectra);
    ret =  ERROR;
  }

  if(loadMsClusterData(reportGeneratorData.projectDir, reportGeneratorData.projectDir) == ERROR)
    WARN_MSG("Error loading clusterData.bin");

  if(loadProteinsFile(reportGeneratorData.projectDir, reportGeneratorData.filenameProteins) == ERROR)
    WARN_MSG("Error loading " << reportGeneratorData.filenameProteins);

  if(loadContigIndices(reportGeneratorData.projectDir, reportGeneratorData.filenameContigIndices) == ERROR)
    WARN_MSG("Error loading " << reportGeneratorData.filenameContigIndices);
  if(loadHomglueRefMidx(reportGeneratorData.projectDir, reportGeneratorData.filenameHomglueRefMidx) == ERROR)
    WARN_MSG("Error loading " << reportGeneratorData.filenameHomglueRefMidx);
  if(loadHomglueRefMp(reportGeneratorData.projectDir, reportGeneratorData.filenameHomglueRefMp) == ERROR)
    WARN_MSG("Error loading " << reportGeneratorData.filenameHomglueRefMp);

  if(loadContigsMidxAll(reportGeneratorData.projectDir, reportGeneratorData.filenameContigsMidx) == ERROR)
    WARN_MSG("Error loading " << reportGeneratorData.filenameContigsMidx);
  if(loadContigsMpAll(reportGeneratorData.projectDir, reportGeneratorData.filenameContigsMp) == ERROR)
    WARN_MSG("Error loading " << reportGeneratorData.filenameContigsMp);
  if(loadSpsSeqs(reportGeneratorData.projectDir, reportGeneratorData.filenameSpsSeqs) == ERROR)
    WARN_MSG("Error loading " << reportGeneratorData.filenameSpsSeqs);

  if(loadStarSpectra(reportGeneratorData.projectDir, reportGeneratorData.filenameStarSpectra) == ERROR)
    WARN_MSG("Error loading " << reportGeneratorData.filenameStarSpectra);
  if(loadAbruijn(reportGeneratorData.projectDir, reportGeneratorData.filenameAbruijn) == ERROR) {
    WARN_MSG("Error loading " << reportGeneratorData.filenameAbruijn);
    ret =  ERROR;
  }

  if(loadContigSpectra(reportGeneratorData.projectDir, reportGeneratorData.filenameContigSpectra) == ERROR)
    WARN_MSG("Error loading " << reportGeneratorData.filenameContigSpectra);

  if(loadCspsMatchesMp(reportGeneratorData.projectDir, reportGeneratorData.filenameHomglueMatchesMp) == ERROR)
    WARN_MSG("Error loading " << reportGeneratorData.filenameHomglueMatchesMp);
  if(loadCspsMatchesMidx(reportGeneratorData.projectDir, reportGeneratorData.filenameHomglueMatchesMidx) == ERROR)
    WARN_MSG("Error loading " << reportGeneratorData.filenameHomglueMatchesMidx);
  if(loadCspsSpectra(reportGeneratorData.projectDir, reportGeneratorData.filenameHomglueMatches) == ERROR)
    WARN_MSG("Error loading " << reportGeneratorData.filenameHomglueMatches);

  if(loadContigNames(reportGeneratorData.projectDir, reportGeneratorData.filenameContigNames) == ERROR)
    WARN_MSG("Error loading " << reportGeneratorData.filenameContigNames);

  // load Input Mapping silentlyS
  loadInputMapping(reportGeneratorData.projectDir, reportGeneratorData.filenameInputMapping);

  return ret;
}
////////////////////////////////////////////////////////////////////////////////
// File name composition
string SpsFiles::composeFileName(const string &projectDir, const string &fileName)
{
  if(fileName[0] == '/')
    return fileName;
...
////////////////////////////////////////////////////////////////////////////////
// Read binary array
int SpsFiles::readBinArray(vector<vector<int> >  *&binArray, const string &fileName)
{
  // Test if file already loaded
  if(binArray)
    return OK;
...
int SpsFiles::loadInputSpectraList(const string &projectDir, const string &fileName)
{
...
int SpsFiles::loadInputSpectraFiles(const string &projectDir, const string &fileName)
{
  // get filename with full path
  string fn = fileName;
...
int SpsFiles::loadConsensusSpectraFile(const string &projectDir, const string &fileName)
{
  // get filename with full path
...
////////////////////////////////////////////////////////////////////////////////
// load MsCluster data.
int SpsFiles::loadMsClusterData(const string &projectDir, const string &fileName)
{
  // Test if file already loaded
  if(m_clusterData)
    return OK;
  ...
  int ret = m_clusterData->load(projectDir, true, false);
  // check for errors
  if(ret == ERROR) {
...
////////////////////////////////////////////////////////////////////////////////
// (4) Load Star Spectra
int SpsFiles::loadStarSpectra(const string &projectDir, const string &fileName)
{
  // get filename with full path
  string fn = composeFileName(projectDir, fileName);
  // log file to load
...
int SpsFiles::loadProteinsFile(const string &projectDir, const string &fileName)
{
  // Test if file already loaded
  if(m_fasta)
    return OK;
...
int SpsFiles::loadContigSpectra(const string &projectDir, const string &fileName)
{
  // get filename with full path
  string fn = composeFileName(projectDir, fileName);
  // log file to load
...
int SpsFiles::loadContigIndices(const string &projectDir, const string &fileName)
{
  // get filename with full path
  string fn = composeFileName(projectDir, fileName);
  // log file to load
...
////////////////////////////////////////////////////////////////////////////////
// where to load contig indices to
int SpsFiles::loadInputMapping(const string &projectDir, const string &fileName)
{
  // get filename with full path
  string fn = composeFileName(projectDir, fileName);
  // log file to load
...
int SpsFiles::loadHomglueRefMidx(const string &projectDir, const string &fileName)
{
  // get filename with full path
  string fn = composeFileName(projectDir, fileName);
  // log file to load
...
int SpsFiles::loadHomglueRefMp(const string &projectDir, const string &fileName)
{
  // get filename with full path
  string fn = composeFileName(projectDir, fileName);
  // log file to load
...
////////////////////////////////////////////////////////////////////////////////
//
int SpsFiles::loadContigsMidxAll(const string &projectDir, const string &fileName)
{
  // get filename with full path
  string fn = composeFileName(projectDir, fileName);
  // log file to load
...
int SpsFiles::loadContigsMpAll(const string &projectDir, const string &fileName)
{
  // get filename with full path
  string fn = composeFileName(projectDir, fileName);
  // log file to load
...
int SpsFiles::loadCspsMatchesMp(const string &projectDir, const string &fileName)
{
  // get filename with full path
  string fn = composeFileName(projectDir, fileName);
  // log file to load
...
int SpsFiles::loadCspsMatchesMidx(const string &projectDir, const string &fileName)
{
  // get filename with full path
  string fn = composeFileName(projectDir, fileName);
  // log file to load
...
int SpsFiles::loadCspsSpectra(const string &projectDir, const string &fileName)
{
  // get filename with full path
  string fn = composeFileName(projectDir, fileName);
  // log file to load
...
int SpsFiles::loadContigNames(const string &projectDir, const string &fileName)
{
...
void SpsFiles::dump_binArray(ostream &sout, vector<vector<int> > *a, char *title, bool web)
{
  if(!a) return;
...
void SpsFiles::dump_specset(ostream &sout, specnets::SpecSet *set, char *title, bool web)
{
  if(!set) return;
