#ifndef __ExecSpecProtAlignTgtDecoy_H__
#define __ExecSpecProtAlignTgtDecoy_H__
...
namespace specnets
{
   ...
   */
  class ExecSpecProtAlignTgtDecoy: public ExecBase
  {
  public:
    //! \name CONSTRUCTORS
    //@{
    /*! \brief The exemplar constructor.
     ...
     @sa clone()
     */
  	ExecSpecProtAlignTgtDecoy(void);
     ...
     */
    ExecSpecProtAlignTgtDecoy(const ParameterList & inputParams);
    ...
    //! \name DESTRUCTOR
    //@{
    virtual ~ExecSpecProtAlignTgtDecoy(void);
    //@}
  ...
  private:
    ...
    DB_fasta * m_dbDecoy; //! Input database of decoy protein sequences
    PeptideSpectrumMatchSet * m_psmSetTag;
    PeptideSpectrumMatchSet * m_psmSetTagDecoy;
    ...
    SpecSet * m_matchedSpectra; //! Matched versions of spectra significantly matched to a protein (Only with matches)
    PeptideSpectrumMatchSet * m_psmSet;
    PeptideSpectrumMatchSet * m_psmSetDecoy;
    PeptideSpectrumMatchSet * m_psmSetFdr; // Used to store the computed PSM set according to FDR
    bool ownOutput; //! Does this object "own" the output data pointers (and hence has to free them)

    SpecSet m_matchedSpectraAllDecoy; //! Decoy matched versions of spectra significantly matched to a protein (All)
    vector<unsigned int> m_matchedSpectraIndices; //! Indices of spectra significantly matched to a protein (if non-NULL and of same size as m_inputSpectra)
  };

} // namespace specnets

#endif // __ExecSpecProtAlignTgtDecoy_H__
