...
namespace specnets
{
      ...
      while (curCandidate != candidates.end())
      {
        ...
        if (abs(pmDiff) > pmTol + 0.00001)
        {
              ...
              += shiftIncrement)
          {
            ...
            numMatchedPeaks = min(numMatchedPeaks, (short)idxUnion.size());

            if ((score1 + score2 > bestScore1 + bestScore2) || (score1 + score2
                == bestScore1 + bestScore2 && abs(shift - pmDiff)
                < abs(bestShift - pmDiff)))
            {
              bestScore1 = score1;
              bestScore2 = score2;
              bestShift = shift;
        ...
        else
        {
              ...
              += shiftIncrement)
          {
            ...
            if ((score1 + score2 > bestScore1 + bestScore2) || (score1 + score2
                == bestScore1 + bestScore2 && abs(shift) < abs(bestShift)))
            {
              bestScore1 = score1;
              bestScore2 = score2;
              bestShift = shift;
        ...
        if (curRatios[0] >= minMatchRatio && 
        	...
        	numMatchedPeaks >= minNumMatchedPeaks)
        {
  ...
  public:
    ...
    bool operator<(GPCAux & other) { return score < other.score; }
  };
  ...
  //
  //---------------------------------------------------------------------------
  void getPairCosines(SpecSet & specSet,
                        vector<int> & baseSpectraIdx,
                        float maxPMdiff,
                        float pmTol,
                        float peakTol,
                        ...
                        float symmetryOffset)
  {
    ...
    // Initialize means vector
    vector<float> ajustedMinMatchIntensity; // total intensity per spectrum for later computation of match ratios
    ajustedMinMatchIntensity.resize(specSet.size());
    means.resize(specSet.size());
    varTerms.resize(specSet.size());
    for (unsigned int i = 0; i < specSet.size(); i++)
    {
      means[i].set(0, 0);
      varTerms[i] = 0;
      ajustedMinMatchIntensity[i] = 0;
      for (unsigned int p = 0; p < specSet[i].size(); p++)
      	ajustedMinMatchIntensity[i] += specSet[i][p][1];
...
//cerr<<"Got "<<numMatchedPeaks<<" matched cosine peaks with cosine "<<cosine<<" and matched intensities "<<score1<<"/"<<score2<<"\n";
        ...
        curResult.score2 = min(score1,score2);
        curResult.shift1 = pmDiff;

        // Update means/variances
        means[spec1][0]
            = (means[spec1][0] * means[spec1][1] + cosine)
                / (means[spec1][1] + 1);
        means[spec2][0]
            = (means[spec2][0] * means[spec2][1] + cosine)
                / (means[spec2][1] + 1);
        means[spec1][1]++;
        means[spec2][1]++;
        float updateRatio = (means[spec1][1] - 1) / means[spec1][1]; // Update variance terms
        varTerms[spec1] = updateRatio * varTerms[spec1] + (cosine
            * cosine) / means[spec1][1];
        updateRatio = (means[spec2][1] - 1) / means[spec2][1];
        varTerms[spec2] = updateRatio * varTerms[spec2] + (cosine
            * cosine) / means[spec2][1];

        // Compute match ratios
        curRatios[0] = score1;
        curRatios[1] = score2;
        if (curRatios[0] >= ajustedMinMatchIntensity[spec1] && curRatios[1] >= ajustedMinMatchIntensity[spec2])
        {
          //! /todo Implement an efficient push_back (or equivalent) for SpectrumPairSet
          results.push_back(curResult);
          ratios.push_back(curRatios);
