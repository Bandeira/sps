...
namespace specnets
{
  ...
  bool ExecParallelAssembly::invoke(void)
  {
    ...
    abruijn::AbruijnGraph::REVERSE_STARS = m_params.getValueInt("REVERSE_STARS", 1) > 0;

    abruijn::AbruijnGraph::ENFORCE_B_ENDPTS = m_params.getValueInt("ENFORCE_B_ENDPTS", 0) > 0;

    abruijn::AbruijnGraph::PATH_EXPAND_LIMIT = m_params.getValueInt("PATH_EXPAND_LIMIT", 5);

    if (!AAJumps::globalJumpsInitialized())
    {
      AAJumps::initializeGlobalJumps(abruijn::AbruijnGraph::MAX_NUM_JUMPS,
                                     0.01,
                                     0);
    }
        ...
        != m_inputAbruijn->end(); abIt++)
    {
      ...
      abruijn::AbruijnGraph::DEBUG_EXPANSION = i == debugIdx;

      abruijn::AbruijnGraph abGraph(m_inputAbruijn->at(i),
                                    *m_stars,
                                    peakTol,
                                    &outAbinfo);
