...
void addDefaultParameterValues(ParameterList &p)
{
  ...
  p.addIfDoesntExist("MIN_METACONTIG_SCORE", "2.8");
  ...
  p.addIfDoesntExist("GRID_NUMNODES", "64");
  ...
  p.addIfDoesntExist("REPORT_MASS_SHIFT", "0");
 ...
 */
bool loadSpecsMS_separate(const string& fileList,
                          vector<SpecSet>& outputSpectra)
{
  vector<string> names;
  ...
  outputSpectra.resize(names.size());
  for (unsigned int i = 0; i < names.size(); i++) {
    if (!ExecMergeConvert::loadSpecset(names[i], &outputSpectra[i])) {
      ERROR_MSG("ExecMergeConvert::loadSpecset() failed for " << names[i]);
      return false;
    }
  }
  return true;
...
/**
 * Saves an ordered collection of SpecSets to a list of filenames
 * @param fileList list of filenames, specifies which file to save for each SpecSet
 * @param inputSpectra input collection of spectra
 * @return true if successful, false otherwise
 */
bool saveSpecsMS_separate(const string& fileList, vector<SpecSet>& inputSpectra)
{
  vector<string> names;
  if (!readFilesFromFile(fileList, names))
  {
  ...
  if (names.size() != inputSpectra.size())
  {
    ERROR_MSG("Did not find same number of spectra as in \'" << fileList << "\'!");
    return false;
  }

  for (unsigned int i = 0; i < inputSpectra.size(); i++)
  {
    if (!ExecMergeConvert::saveSpecset(names[i], &inputSpectra[i]))
    {
      return false;
    }
  }
                     ...
                     vector<SpecSet>& loadedSpectra)
{
  ...
  string pklbinPath, binPath;

  // List of generated data files
  vector<string> pklbinFileList, binFileList;
  //
  ...
  loadedSpectra.resize(filenames.size());
  ...
  for (unsigned int i = 0; i < filenames.size(); i++)
  {
    ...
    cmd2 << DEFAULT_INPUT_FILE_BASE << i + 1 << ".bin";
    ...
    binPath = getProjPath(ip, cmd2.str());
    binFileList.push_back(binPath);

    // save it with the name we want (pklbin)

    DEBUG_MSG("Saving \'" << pklbinPath << "\' and \'" << binPath << "\' ...");
    loadedSpectra[i].SaveSpecSet_pklbin(pklbinPath.c_str(), binPath.c_str());

    //=================================================
    // LARS - TEMPORARY MEASURE SO REPORTS KEEP WORKING
    //=================================================
    loadedSpectra[i].SaveSpecSet_bin(binPath.c_str());
    //=================================================
  ...
  if (!writeFileIndex(aux.c_str(), filenames))
  {
    return false;
  }
  ...
  if (binFileList.size())
  {
    string aux = getProjPath(ip, DEFAULT_BIN_FILES_FILENAME);
                        ...
                        SpecSet& ms2spectra)
{
  ...
  if (!Save_binArray(aux3.c_str(), link))
  {
 ...
 * @return true if successful, false if not
 */
bool performMS2Deconv(ParameterList & ip, vector<SpecSet>& inputSpecs, vector<
    SpecSet>& outputSpecs)
{
  ...
  string isoModelPath = ip.getValue("EXE_DIR");
  isoModelPath += "/resources/";
  isoModelPath += DEFAULT_DECONV_MODEL;
  IsoEnvelope isoModel;

  DEBUG_MSG("Loading input iso envelope from \'" << isoModelPath << "\' ...");
  if (!isoModel.LoadModel(isoModelPath.c_str()))
  {
    ERROR_MSG("Could not load " << isoModelPath);
    return false;
  }

  vector<string> msSpecFiles;
  if (!readFilesFromFile(DEFAULT_INPUT_FILES_LIST, msSpecFiles))
  {
    return false;
  }

  // deconvolute that SpecSets one at a time
  vector<string> deconvFilesList(inputSpecs.size());
  ...
  for (unsigned int i = 0; i < inputSpecs.size(); i++)
  {
    ...
    if (!splitText(msSpecFiles[i].c_str(), inFile, "."))
    {
      return false;
    }
    // save the output to "specs_ms_*_z.pklbin"

    string outFile = inFile[0];
    outFile += "_z.pklbin";
    deconvFilesList[i] = outFile;
    deconvParams.setValue("OUTPUT_SPECTRA", deconvFilesList[i].c_str());
  ...
  if (!writeFileIndex(DEFAULT_DECONV_FILES_LIST, deconvFilesList))
  {
    return false;
  }
                      ...
                      SpecSet &clustSpectra)
{
  ...
  string outputPaths = getProjPath(ip, "spectra/specs_ms.pklbin");
  outputPaths += ";";
  outputPaths += getProjPath(ip, "spectra/specs_ms.mgf");
  ...
  ExecMsCluster moduleMsCluster(msclusterParams, inputFilesList, &clustSpectra);
                    ...
                    SpecSet & outputSpectra)
{
  ...
  {
    scoringParams.setValue("MERGE_SAME_PREC", "1");
  ...
  string ms2SpecPath = getProjPath(ip, "spectra/specs_ms.pklbin");
  scoringParams.setValue("PEPNOVO_OUTDIR",getProjPath(ip,"spectra"));
  scoringParams.setValue("INPUT_SPECTRA", ms2SpecPath);
  scoringParams.setValue("OUTPUT_SPECTRA",
                         getProjPath(ip, "spectra/specs_scored.pklbin"));
  ...
  if (scoringParams.getValueInt("MERGE_SAME_PREC", 0) > 0)
  {
    scoringParams.setValue("OUTPUT_CLUSTERS_DIR", "./");
  }
...
bool performMergeOfCSPSAndGenoMS(ParameterList & ip)
{
  ...
  ///////////////////////////////////////////////
  if((ip.exists("CLUSTER_TOOL") && ip.getValue("CLUSTER_TOOL", "") == CLUSTER_PRMS) ||
     (ip.exists("CLUSTER_MIN_SIZE") && ip.getValueInt("CLUSTER_MIN_SIZE") <= 0)) {
  ...
  if (inputSpectra.LoadSpecSet_pklbin("./homology/csps.contigs_midx.pklbin")
      <= 0)
  {
  ...
  if (inputSpectra.LoadSpecSet_pklbin("./homology/csps.contigs_midx_all.pklbin")
      <= 0)
  {
  ...
  if (other.LoadSpecSet_pklbin("./homology/genoms.contigs_midx_all.pklbin")
      <= 0)
  {
  ...
  inputSpectra.appendSpecSet(other);
  if (inputSpectra.SaveSpecSet_pklbin("./homology/contigs_midx_all.pklbin")
      <= 0)
  {
  ...
  if (inputSpectra.LoadSpecSet_pklbin("./homology/csps.homglue_ref_midx.pklbin")
      <= 0)
  {
  ...
  if (other.LoadSpecSet_pklbin("./homology/genoms.homglue_ref_midx.pklbin")
      <= 0)
  {
  ...
  inputSpectra.appendSpecSet(other);
  if (inputSpectra.SaveSpecSet_pklbin("./homology/homglue_ref_midx.pklbin")
      <= 0)
  {
  ...
  if (inputSpectra.LoadSpecSet_pklbin("./homology/csps.homglue_matches.pklbin")
      <= 0)
  {
  ...
  ///////////////////////////////////////////////
  DEBUG_MSG("Merging homglue_matches.pklbin");
  if (inputSpectra.LoadSpecSet_pklbin("./homology/csps.homglue_matches_midx.pklbin")
      <= 0)
  {
  ...
  if (other.LoadSpecSet_pklbin("./homology/genoms.homglue_matches_midx.pklbin")
      <= 0)
  {
  ...
  inputSpectra.appendSpecSet(other);
  if (inputSpectra.SaveSpecSet_pklbin("./homology/homglue_matches_midx.pklbin")
      <= 0)
  {
...
bool generateRelaunchScript(ParameterList & ip)
{
  ...
  relauncherScript += " -lf " + ip.getValue("LOG_FILE_NAME",
                                            "relauncher_log.txt");
                        ...
                        bool resume)
{
  ...
  filterpairsParams.setValue("MIN_NUM_MATCHED_PEAKS",
  ...
  filterpairsParams.setValue("OUTPUT_MEANS", getProjPath(ip,
                                                         "./aligns/means.bin"));
                         ...
                         std::vector<TwoValues<float> > & pvalues)
{
  ...
  filteralignsParams.setValue("INPUT_ALIGNS",
                              getProjPath(ip, "./aligns/pairs_raw.bin"));
  filteralignsParams.setValue("INPUT_MEANS", getProjPath(ip,
                                                         "./aligns/means.bin"));
                            ...
                            SpectrumPairSet & contigPairs)
{
  ...
  alignParams.setValue("INPUT_SPECTRA", getProjPath(ip,
                                                    "assembly/sps_seqs.pklbin"));
                         ...
                         abinfo_t & metaContigAbruijn)
{
  ...
  assemblyParams.setValue("OUTPUT_ABINFO",
                          getProjPath(ip, "assembly/meta_component_info.bin"));
                      ...
                      vector<unsigned int> *specsToSearch)
{
  ...
  DEBUG_TRACE;
  ExecTagSearch moduleTagsearch(tagsearchParams, &spectra, &db, specsToSearch);
  DEBUG_TRACE;
                          ...
                          bool resume)
{
  ...
  if (ip.exists("FDR_CONTIG_THRESHOLD")) {
    specProtAlignParams.setValue("FDR_THRESHOLD", ip.getValue("FDR_CONTIG_THRESHOLD"));
  }
  ...
  specProtAlignParams.setValue("GRID_DATA_DIR", getProjPath(ip, "./homology/grid"));
  ...
  specProtAlignParams.setValue("OUTPUT_REF_SPS_NAMES",
                               getProjPath(ip, "homology/ref_sps_names.txt"));

  specProtAlignParams.addIfExists(ip, "ENFORCE_ENDPEAKS");
  specProtAlignParams.addIfDoesntExist("ENFORCE_ENDPEAKS", "0");
  ...
  if (ip.exists("CONTIGPROTALIGN_START_IDX")) {
    specProtAlignParams.setValue("IDX_START", ip.getValue("CONTIGPROTALIGN_START_IDX"));
  }
  if (ip.exists("CONTIGPROTALIGN_END_IDX")) {
    specProtAlignParams.setValue("IDX_END", ip.getValue("CONTIGPROTALIGN_END_IDX"));
  }
  ...
  specProtAlignParams.writeToFile(getProjPath(ip, "debug_contigprotalign.params"));
  ...
  DEBUG_VAR(matchedSpectraAll->size());
                          ...
                          bool resume)
{
  ...
  if (ip.exists("FDR_SPECTRUM_THRESHOLD")) {
    specProtAlignParams.setValue("FDR_THRESHOLD", ip.getValue("FDR_SPECTRUM_THRESHOLD"));
  }
  ...
  specProtAlignParams.addIfExists(ip, "GRID_SGE_EXE_DIR");
  specProtAlignParams.addIfExists(ip, "GRID_PARAMS");
  specProtAlignParams.setValue("GRID_DATA_DIR", getProjPath(ip, "./homology/grid2"));
  ...
  specProtAlignParams.setValue("OUTPUT_MATCHED_SPECS",
                               getProjPath(ip, "spectra/spectrum_matches.pklbin"));
  specProtAlignParams.setValue("OUTPUT_PSM",
                               getProjPath(ip, "homology/spectrum_psm.txt"));
  specProtAlignParams.setValue("OUTPUT_PSM_MOD_FREQS",
                               getProjPath(ip, "homology/spectrum_psm_mod_freqs.txt"));
  ...
  if (ip.exists("SPECPROTALIGN_START_IDX")) {
    specProtAlignParams.setValue("IDX_START", ip.getValue("SPECPROTALIGN_START_IDX"));
  }
  if (ip.exists("SPECPROTALIGN_END_IDX")) {
    specProtAlignParams.setValue("IDX_END", ip.getValue("SPECPROTALIGN_END_IDX"));
  }
...
bool performProtProtAlign(ParameterList & ip, unsigned int refProtIdx, set<
    unsigned int> dbIndexes, DB_fasta & db)
{
  DEBUG_TRACE;
...
bool performReport(ParameterList & ip)
{
  ...
  if (ip.getValueInt("CLUSTER_MIN_SIZE") == 0 && ip.getValue("CLUSTER_TOOL", CLUSTER_MSCLUST) != CLUSTER_PRMS)
  ...
  if(ip.getValue("CLUSTER_TOOL", CLUSTER_MSCLUST) == CLUSTER_PRMS)
    aux = "spectra/specs_scored.pklbin";
  else
    aux = "spectra/specs_ms.pklbin";
  ...
  string source = ip.getValue("EXE_DIR"); source += "/resources/css";
                             ...
                             vector<vector<int> > * snets_mp)
{
  ...
  specnetsParams.setValue("TOLERANCE_PEAK", ip.getValue("TOLERANCE_PEAK",
                                                        "0.45"));
  ...
  specnetsParams.addIfExists(ip, "MAX_ALIGN_GAP_SIZE");
  ...
  ofstream spsIndex(getProjPath(ip, "homology/ref_snets_names.txt").c_str(), ios_base::out | ios_base::binary);
  ...
  ofstream spsProj(getProjPath(ip, "sps_projects.txt").c_str(), ios_base::out | ios_base::binary);
...
int main(int argc, char ** argv)
{
  ...
  if (argc < 2 || showHelp)
  {
    if (showHelp)
    {
      ...
      cout
          ...
          << endl;
      cout
          << "                         specprotalign, protprotalign, homologyassembly, genoms"
          << endl;
  ...
  if (!ip.exists("EXE_DIR")) {
    ...
    if (aux.rfind(mainSpecnetsStr) == aux.length() - mainSpecnetsStr.length())
      aux.erase(aux.length() - mainSpecnetsStr.length(),  mainSpecnetsStr.length());
    else {
  ...
  res = mkdir_if_not_exist("homology/grid");
  if (res) {
    DEBUG_MSG("Made directory \'homology/grid\'");
  }
  res = mkdir_if_not_exist("homology/grid2");
  if (res) {
    DEBUG_MSG("Made directory \'homology/grid2\'");
  }
  ...
  vector<SpecSet>* ms2Spectra_separate = new vector<SpecSet> ; // MS/MS spectra in separate files
  ...
  if (initialStage == STAGE_BEGIN)
  {
    ...
    loadInitialData(ip, ip.getValue("INPUT_SPECS_MS"), *ms2Spectra_separate);
  ...
  {
    if (!loadSpecsMS_separate(DEFAULT_INPUT_FILES_LIST, *ms2Spectra_separate))  {
      ERROR_MSG("loadSpecsMS_separate() exited in error. Exiting program");
      exit(0);
    }
  }
  ...
  string msClustInput = DEFAULT_INPUT_FILES_LIST;

  if (initialStage <= STAGE_MS2DECONV && ip.getValueInt("DECONV_MS2", 0) > 0)
  {
    vector<SpecSet> deconvSpecs;

    if (!performMS2Deconv(ip, *ms2Spectra_separate, deconvSpecs))
    {
      exit(0);
    }

    *ms2Spectra_separate = deconvSpecs;
    ...
    msClustInput = DEFAULT_DECONV_FILES_LIST;
  }
  else if (ip.getValueInt("DECONV_MS2", 0) > 0)
  {
    msClustInput = DEFAULT_DECONV_FILES_LIST;
    if (!loadSpecsMS_separate(DEFAULT_DECONV_FILES_LIST, *ms2Spectra_separate)) {
      ERROR_MSG("loadSpecsMS_separate() exited in error. Exiting program");
      exit(0);
    }
  }
  ...
  // merge spectra and save scan # mapping
  if (!mergeSeparateSpecs(ip, *ms2Spectra_separate, ms2Spectra))
  {
    exit(0);
  }

  // Clear separated files to save memory
  delete ms2Spectra_separate;

  TEST_RETURN("moduleMS2Deconv", ms2Spectra);
  ...
  if (initialStage <= STAGE_MSCLUSTER && performClustering && clusterTool
      == CLUSTER_MSCLUST)
  {

    DEBUG_MSG("Starting stage McCluster on: " << getCurrentTimeString());
    if (!performMsCluster(ip, msClustInput, ms2Spectra))
    {
  ...
  else if (initialStage <= STAGE_MSCLUSTER && (!performClustering
      || clusterTool != CLUSTER_MSCLUST))
  {
    // if we are skipping the clustering step then just save the MS/MS spectra
    ...
    // Need to save mgf spectra
    if (!ExecMergeConvert::saveSpecset(getProjPath(ip, "./spectra/specs_ms.mgf").c_str(),
                                       &ms2Spectra))
    {
      exit(0);
    }
  }
  else if (performClustering && clusterTool == CLUSTER_MSCLUST)
  {
  ...
  if (initialStage <= STAGE_SCORING)
  {
    ...
    if (runGenoMSFlag || runMergedFlag)
    {
      ...
      if (!ExecMergeConvert::saveSpecset(getProjPath(ip,
                                                     "./spectra/specs_scored.mgf"),
                                         &prmSpectra))
      {
        ERROR_MSG("Failed to save to " << getProjPath(ip,
                "./spectra/specs_scored.mgf"));
        exit(-3);
      }
    }
  ...
  if (starSpectra.LoadSpecSet_pklbin(getProjPath(ip, "./spectra/stars.pklbin").c_str())
      <= 0)
  {
  ...
  //---------------------------------------------------------------------------
  float resolution = ip.getValueDouble("RESOLUTION", 1.0);
  ...
  PenaltyMatrix penaltyMatrixBlosum(jumps, resolution);
  PenaltyMatrix penaltyMatrixMods(jumps, resolution);

  bool penaltyAlign = (bool)ip.getValueInt("PENALTY_ALIGNMENT");
  if (penaltyAlign)
  {
    if (!ip.exists("BLOSUM_PENALTY_FILE")) {
      WARN_MSG("Penalty alignment specified but BLOSUM_PENALTY_FILE not set");
    }
    string blosumFilename = ip.getValue("BLOSUM_PENALTY_FILE");
    DEBUG_VAR(blosumFilename);
    penaltyMatrixBlosum.loadFromBlosum(blosumFilename);
  ...
  if (execMode == string("specnets"))
  {
    ...
    float resolution = ip.getValueFloat("RESOLUTION", .1);
  ...
  if (initialStage <= STAGE_ASSEMBLY and execMode == string("sps"))
  {
    ...
    bool res = FileCopy("assembly/sps_seqs.pklbin", "assembly/old_sps_seqs.pklbin");
        //spsSystem("cp assembly/sps_seqs.pklbin assembly/old_sps_seqs.pklbin");
    res = FileCopy("assembly/path_spectra_as_cluster.txt", "assembly/old_path_spectra_as_cluster.txt");
        //= spsSystem("cp assembly/path_spectra_as_cluster.txt assembly/old_path_spectra_as_cluster.txt");
    res = FileCopy("assembly/component_info.bin", "assembly/old_component_info.bin");
  ...
  else
  {
    ...
    if (contigShifts.consensus.LoadSpecSet_pklbin("assembly/old_sps_seqs.pklbin")
        <= 0
        and contigShifts.consensus.LoadSpecSet_pklbin("assembly/sps_seqs.pklbin")
            <= 0)
    {
      ERROR_MSG("Problem encountered while skipping Assembly stage" );
      writeStatusFile(statusFileName, "Error");
      ...
      and ip.getValueInt("MIN_METACONTIG_SIZE", 0) > 0)
  {
    ...
    bool res = FileCopy("assembly/meta_sps_seqs.pklbin", "assembly/sps_seqs.pklbin");
        //spsSystem("cp assembly/meta_sps_seqs.pklbin assembly/sps_seqs.pklbin");
    res = FileCopy("assembly/meta_sps_seqs.pklbin", "assembly/test_sps_seqs.pklbin");
        //= spsSystem("cp assembly/meta_sps_seqs.pklbin assembly/test_sps_seqs.pklbin");
    res = FileCopy("assembly/meta_path_spectra_as_cluster.txt", "assembly/path_spectra_as_cluster.txt");
        //= spsSystem("cp assembly/meta_path_spectra_as_cluster.txt assembly/path_spectra_as_cluster.txt");
    res = FileCopy("assembly/meta_component_info.bin", "assembly/component_info.bin");
        //= spsSystem("cp assembly/meta_component_info.bin assembly/component_info.bin");
  }
  else if (ip.getValueInt("MIN_METACONTIG_SIZE", 0) > 0)
  {
    DEBUG_MSG("Bypassing MetaAssembly stage" );
    if (contigShifts.Load("assembly/meta_path_spectra_as_cluster.txt") <= 0)
    {
      ERROR_MSG("Problem encountered while skipping MetaAssembly stage" );
      writeStatusFile(statusFileName, "Error");
      exit(-STAGE_METAASSEMBLY);
    }
    if (!Load_abinfo("assembly/meta_component_info.bin", contigAbinfo))
    {
      ERROR_MSG("Problem encountered while skipping MetaAssembly stage" );
      writeStatusFile(statusFileName, "Error");
      exit(-STAGE_METAASSEMBLY);
  ...
  if (ip.getValueFloat("FDR_CONTIG_THRESHOLD", -1.0) >= 0.0 || ip.getValueFloat("FDR_SPECTRUM_THRESHOLD", -1.0) >= 0.0) {
    dbAll.addDecoyReversed();
  }
  ...
  if (ip.exists("FASTA_DATABASE"))
  {
    ...
    if (initialStage <= STAGE_TAGSEARCH)
    {
      ...
      if (execMode == string("sps"))
      {
        ok = performTagsearch(ip, contigShifts.consensus, dbAll, (vector<
            unsigned int> *)0); //specsToSearch,
      }
      else
      {
    ...
    //---------------------------------------------------------------------------
    // Add decoys to database if we are using FDR Cutoff
    //---------------------------------------------------------------------------
    double fdrCutoff = ip.getValueDouble("PEPTIDE_FDR_CUTOFF", 1.0);
    if (fdrCutoff != 1.0)
    {
      dbAll.addDecoyReversed();
      dbAll.Save("decoys.fasta");
    }

    //---------------------------------------------------------------------------
    // Contig/Protein Alignment STAGE
    //---------------------------------------------------------------------------
    SpecSet matchedContigsAll; // Matched versions of spectra significantly matched to a protein
    ...
    else
    {
      ...
      //if (matchedContigsAll.LoadSpecSet_pklbin(getProjPath(ip,"homology/contigs_midx_all.pklbin").c_str()) <= 0)
      //{
      //  ERROR_MSG("Problem encountered while skipping ContigProtAlign stage" );
      //  writeStatusFile(statusFileName, "Error");
      //  exit(-STAGE_CONTIGPROTALIGN);
    ...
    //TEST_RETURN("ExecContigProtAlign", matchedContigIndices);
    ...
    //---------------------------------------------------------------------------
    if (initialStage <= STAGE_SPECPROTALIGN && ip.getValueInt("ALIGN_STARS", 0) > 0)
    {
      ...
      getAssembledShifts(contigShifts.consensus, contigAbinfo, outputAssembledShifts);
      ...
      for (int i = 0; i < matchedContigs.size(); i++) {
       ...
       if (matchedContigs[i].psmList.size() == 0) {
        ...
        int scanIndex = origPsm->m_scanNum - 1;
        //DEBUG_VAR(scanIndex);
        //DEBUG_VAR(outputAssembledShifts[scanIndex].size());
        for (int j = 0; j < outputAssembledShifts[scanIndex].size(); j++) {

          int spectrumIndex = outputAssembledShifts[scanIndex][j].m0;
          int spectrumMassShift = outputAssembledShifts[scanIndex][j].m1;

          //DEBUG_MSG("  " << spectrumIndex << "  " << spectrumMassShift);
          ...
          *ptrSpectrumPsm = *origPsm;
          ...
          ptrSpectrumPsm->m_matchOrientation = ptrSpectrumPsm->m_matchOrientation;
      ...
      SpecSet starPrmAll; // Matched versions of spectra significantly matched to a protein
      SpecSet starPrm; // Matched versions of spectra significantly matched to a protein
      vector<unsigned int> matchedPrmIndices; // Indices of spectra significantly matched to a protein
	...
	//Since we didn't write out the PMS matches for these contigs (they are GenoMS contigs)
	else if(matchedContigs[i].psmList.size() == 0 && (runGenoMSFlag || runMergedFlag)) {
	  ...
	  if (index >= 0 and index < dbAll.size())
	  {
	    ...
	    //DEBUG_VAR(matchedContigs[i].psmList.front()->m_matchedPeaks.size());
	    numMatches[index]
	      += matchedContigs[i].size(); // Select by highest number of matched masses
	    if (numMatches[index] > refProtMatchCount)
	      {
    ...
    else
    {
      ...
      if (cspsContigs.LoadSpecSet_pklbin(getProjPath(ip,
                                                     "homology/homglue_matches.pklbin").c_str())
          <= 0
          or cspsMatchedIndices.LoadSpecSet_pklbin(getProjPath(ip,
  ...
  {
    ofstream
        spsProj(getProjPath(ip, "sps_projects.txt").c_str(), ios_base::out | ios_base::binary);
    spsProj << "sps;.;" << ip.getValue("TOLERANCE_PEAK") << ";"
        << ip.getValue("TOLERANCE_PM") << "\n";
    spsProj.close();
