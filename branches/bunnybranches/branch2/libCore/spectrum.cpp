...
namespace specnets
{
  ...
  const unsigned short Spectrum::BIN_SUBVERSION = 2;
  ...
  void Spectrum::consolidatePeaks(const bool addIntensities)
  {
    ...
    for (int i = 0; i < size(); i++)
    {
      ...
      for (list<int>::iterator pIt = peakGroup.begin(); pIt != peakGroup.end(); pIt++)
      {
        if (*pIt != maxIntenPeak)
        {
                                   ...
                                   bool outputGFTables)
  {
    ...
    {
      cerr << "Warning: bad spectrum with parent mass " << parentMass
          << ", skipping GF calculation.\n";
      return 1.0;
    }
    unsigned int maxMass = (unsigned int)roundMasses ? round(0.9995
        * (parentMass - AAJumps::massMH)) : round(parentMass - AAJumps::massMH);
    if (maxMass != atMass)
      maxMass = atMass;
    ...
    if (aaMasses)
    {
      ...
      {
        int peakMass = (int)roundMasses ? round(0.9995 * peakList[peakIdx][0])
            : peakList[peakIdx][0];
    ...
    else
    {
      ...
      cerr << "Spectrum::getGFProbability(): score " << score
          << " out of range [0," << m_gfProbs.size() - 1
          << "], returning prob 1.0\n";
    }
