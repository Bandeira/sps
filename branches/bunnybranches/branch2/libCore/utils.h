...
class FilenameManager {
 ...
 public:
  ...
  // path
  string path;
  // just the file name (excluding extension
  string filename;
  // file extension
  string extension;
  ...
  FilenameManager(const char *str) {filenameFull=str;splitFilename();lowerCaseExtension();};

  // split the full filename into its components
  void splitFilename(void);

  // build filename from its components
...
template<class T> int Compare_binArray(vector<vector<T> > &a1, vector<vector<T> > &a2, vector<pair<int,int> > &res)
{
  ...
  if(numRows1 != numRows2) {
    res.push_back(make_pair<int,int>(numRows1, numRows2));
    return -1;
  }
  ...
  if(numCols1 != numCols2) {
    res.push_back(make_pair<int,int>(numCols1, numCols2));
    return -3;
  }
