...
namespace abruijn
{
  ...
  AbruijnGraph::ParallelPath::ParallelPath() :
    m_path(0), m_nodes(0), m_outgoingEdges(), m_weight(0)
  {
  }

  AbruijnGraph::ParallelPath::ParallelPath(const AbruijnGraph& abG,
                                           AbruijnNode& start) :
    m_path(0), m_nodes(0), m_outgoingEdges(), m_weight(0)
  {
  ...
  void AbruijnGraph::ParallelPath::initialize(const AbruijnGraph& abG,
                                              AbruijnNode& start)
  {
    m_path.resize(0);
    m_weight = 0;
    m_nodes.resize(1);
    m_nodes[0].resize(1);
    m_nodes[0][0] = start.getIndex();

    m_outgoingEdges.resize(0);
    const IndexVector& outEdges = abG.getOutEdges(&start);
    for (unsigned long i = 0; i < outEdges.size(); i++)
    {
      ...
      AbruijnEdge* abEdge =
          AbruijnEdge::castEdgePtr(abG.getEdge(outEdges[i]));

      MZRange jumpMass(abEdge->getMass(), 0, 0.04);
      JumpEdge tempEdge;
      tempEdge.copy((Edge &)*abEdge);
      tempEdge.loadLabel(abEdge->getJumpLabel(), &jumpMass);

      pair<JumpEdge, unsigned int> outEdge(tempEdge, tempEdge.getLength() - 1);
      m_outgoingEdges.push_back(outEdge);
    }
  }

  void AbruijnGraph::ParallelPath::advance(const AbruijnGraph& abG,
                                           const JumpEdge& advanceEdge)
  {
    MZRange fJumpMass = advanceEdge.getMassRange();

    vector<pair<JumpEdge, unsigned int> > newOutgoingEdges;

    for (list<int>::const_iterator cIt = advanceEdge.begin(); cIt
        != advanceEdge.end(); cIt++)
    {
      JumpEdge pathEdge;
      pathEdge.addJump(*cIt);
      m_path.push_back(pathEdge);
    }
    m_nodes.resize(m_nodes.size() + advanceEdge.getLength());

    for (vector<pair<JumpEdge, unsigned int> >::const_iterator eIt =
        m_outgoingEdges.begin(); eIt != m_outgoingEdges.end(); eIt++)
    {
      const unsigned short comparVal = eIt->first.compareTo(advanceEdge);
      ...
      {
        const unsigned int edgeWeightIdx = m_path.size() - 1 - (advanceEdge.getLength()
            - eIt->first.getLength()) - eIt->second;

        m_path[edgeWeightIdx].addWeight(eIt->first.getWeight());
        m_weight += eIt->first.getWeight();

        const unsigned int nodeIdx = m_nodes.size() - 1 - (advanceEdge.getLength()
            - eIt->first.getLength());
        addNode(nodeIdx, eIt->first.getToNodeIndex());
      }
    }

  }

  bool AbruijnGraph::ParallelPath::addNode(const unsigned int& pathIdx, const long& node)
  {
    vector<long>& idxSet = m_nodes.at(pathIdx);

    if (std::binary_search(idxSet.begin(), idxSet.end(), node))
    {
      return false;
    }

    idxSet.push_back(node);
    sort(idxSet.begin(), idxSet.end());
    return true;
  }
