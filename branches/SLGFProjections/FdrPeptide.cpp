...
namespace specnets
{
                                           ...
                                           double scalingFactor)
  {
    ...
    if (!calculatePValues(temp, outputPeptides, scalingFactor))
    {
      return false;
    }
    return true;
                                       ...
                                       double scalingFactor)
  {
    ...
    if (!calculatePValues(concatenatedFiltered, outputPeptides, scalingFactor))
    {
      return false;
    }
    return true;
                                    ...
                                    double scalingFactor)
  {
    ...
    for (unsigned int i = 0; i < outputPeptides.size(); i++)
    {
      ...
      if (correctHits > 0)
      {
        double pValue = ((double) incorrectHits * (double) scalingFactor) / (double) correctHits;
        if (pValue > 1)
        {
        ...
        {
          outputPeptides[i]->m_pValue = pValue;
        }
      }
      else
      {
