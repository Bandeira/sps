...
vector<float> get_all_substitution_masses(char amino_acid){

    vector<char> amino_acids;
    vector<float> return_masses;
    
    amino_acids.push_back('g');
    amino_acids.push_back('a');
    amino_acids.push_back('s');
    amino_acids.push_back('p');
    amino_acids.push_back('v');
    amino_acids.push_back('t');
    amino_acids.push_back('c');
    amino_acids.push_back('l');
    amino_acids.push_back('i');
    amino_acids.push_back('n');
    amino_acids.push_back('d');
    amino_acids.push_back('q');
    amino_acids.push_back('k');
    amino_acids.push_back('e');
    amino_acids.push_back('m');
    amino_acids.push_back('h');
    amino_acids.push_back('f');
    amino_acids.push_back('r');
    amino_acids.push_back('y');
    amino_acids.push_back('w');
    
    
    map<char, float> all_masses = getAminoAcidLookup();
    
    for(int i = 0; i < amino_acids.size(); i++){
        float mass_difference = all_masses[amino_acids[i]] - all_masses[tolower(amino_acid)];
        return_masses.push_back(mass_difference);
    }
    
    return return_masses;
}

map<char, vector<float> > get_all_substitution_masses(){
    vector<char> amino_acids;
    map<char, vector<float> > return_map;
    ...
    for(int i = 0; i < amino_acids.size(); i++){
        return_map[amino_acids[i]] = get_all_substitution_masses(amino_acids[i]);
    }
    
    return return_map;   
}

char closest_amino_acid_mass(float mass){
    vector<char> amino_acids;
    ...
    float min_diff = 1000.f;
    char min_aa;
    for(int i = 0; i < amino_acids.size(); i++){
        if(min_diff > abs(all_masses[amino_acids[i]] - mass)){
            min_aa = amino_acids[i];
            min_diff = abs(all_masses[amino_acids[i]] - mass);
        }
    }
...
char closest_amino_acid(char amino_acid, float diff){
    map<char, float> all_masses = getAminoAcidLookup();
    return closest_amino_acid_mass(all_masses[tolower(amino_acid)] + diff);
}

int is_valid_mass_diff(map<char, vector<float> > mass_diffs, float difference, string amino_acid, float tolerance, string & valid_amino_acid){
    
    //For now
    if(amino_acid.length() > 1 || amino_acid.length() == 0) return -1;
    
    for(int i = 0; i < mass_diffs[tolower(amino_acid[0])].size(); i++){
        
        if(abs(difference - (mass_diffs[tolower(amino_acid[0])])[i]) < tolerance){
            valid_amino_acid = closest_amino_acid(amino_acid[0], difference);
            return 1;
        }
    }
    return -1;
