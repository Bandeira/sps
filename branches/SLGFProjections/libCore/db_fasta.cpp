...
namespace specnets
{
  ...
  void DB_fasta::addDecoyReversed()
  {
    ...
    for (unsigned int i = 0; i < sizeDb; i++) {
      if (desc[i]) {
        desc[i + sizeDb] = (char *)malloc(strlen(desc[i]) + 1);
      ...
      if (sequences[i]) {
        ...
        sequences[i + sizeDb] = (char *) malloc(lenStr + 1);
      ...
      if (IDs[i]) {
        ...
        IDs[i + sizeDb] = (char *) malloc(strlen(IDs[i]) + 4); // 1 for null, 3 for XXX
  ...
  void DB_fasta::addDecoyShuffled()
  {
    ...
    for (unsigned int i = 0; i < sizeDb; i++) {
      ...
      if (sequences[i]) {
        int lenStr = strlen(sequences[i]);
        sequences[i + sizeDb] = (char *) malloc(lenStr + 1);
        strcpy(sequences[i + sizeDb], sequences[i]);
        srand ( 100 );
        // Decoy is shuffled version of original protein
        for (int j = 0; j < lenStr; j++) {
          int rndChar = rand() % (lenStr - j) + 1;
          char temp = sequences[i + sizeDb][lenStr - j - 1];
          sequences[i + sizeDb][lenStr - j - 1] = sequences[i + sizeDb][rndChar];
          sequences[i + sizeDb][rndChar] = temp;
        }
        sequences[i + sizeDb][lenStr] = '\0'; // Null terminate it
      }
      else
