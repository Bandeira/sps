...
namespace specnets
{
  ...
  const unsigned short Spectrum::BIN_SUBVERSION = 1;
  ...
  Spectrum::FragType Spectrum::parseActivation(const string& activString)
  {
    ...
    {
      ERROR_MSG("Found unknown fragmentation ID \'" << activString << "\'");
    }
  }
  ...
  string Spectrum::activationToString(Spectrum::FragType& activ)
  {
    ...
    {
      ERROR_MSG("Found unknown fragmentation type \'" << activ << "\'");
    }
  }
                          ...
                          int* startIdx) const
  {
    ...
    if (MZRange::EqualWithinRange(peakList[idx][0], mass, peakTols[idx]
        + tolerance))
    {
      if (matches != 0)
      {
  ...
  bool Spectrum::removePeaks(list<int>& peakIndices)
  {
    ...
    if (size() < peakIndices.size() || peakIndices.size() == 0)
    {
      return false;
    }
                                  ...
                                  bool includeY0k)
  {
    ...
    float massB0 = ionOffset * idDist, massBk = parentMass - (AAJumps::massMH
        - ionOffset) * idDist,
        massY0 = (AAJumps::massH2O + ionOffset) * idDist, massYk = parentMass
  ...
  // -------------------------------------------------------------------------
  float Spectrum::cosine(Spectrum &withSpec, vector<pair<unsigned int,
      unsigned int> > *matchedPeaksIdx)
  {
                              ...
                              int startIdx)
  {
    ...
    int idxUse = 0;
    for (list<int>::iterator mIt = matchesList.begin(); mIt
        != matchesList.end(); mIt++)
    {
      if (*mIt >= startIdx)
      {
  ...
  void Spectrum::mergePeakList(Spectrum &spec, Spectrum *putHere)
  {
	  ...
	  vector<float> &updatedPeakTols = (putHere == 0) ? peakTols : putHere->peakTols;
	  ...
	  while (idxOld < oldPeakList->size() or idxNew < spec.size())
	  {
		  ...
		  {
			  updatedPeakTols[idxUpdated] = min((*oldPeakTols)[idxOld],spec.peakTols[idxNew]);
			  updatedPeakList[idxUpdated][0] = (*oldPeakList)[idxOld][0];
			  updatedPeakList[idxUpdated++][1] = (*oldPeakList)[idxOld++][1] + spec[idxNew++][1];
		  }
  ...
  unsigned int Spectrum::filterLowMassPeaks(float minMass) {
	  ...
	  for(minIdxToKeep=0; minIdxToKeep<peakList.size(); minIdxToKeep++)
		  if(peakList[minIdxToKeep][0]>=minMass) break;
	  if(minIdxToKeep==0) return 0;

	  for(unsigned int peakIdx=0; peakIdx<peakList.size()-minIdxToKeep; peakIdx++) {
  ...
  unsigned int Spectrum::filterHighMassPeaks(float maxMass) {
	  ...
	  for(maxIdxToKeep=peakList.size()-1; maxIdxToKeep>0; maxIdxToKeep--)
		  if(peakList[maxIdxToKeep][0]<=maxMass) break;
	  if(maxIdxToKeep==peakList.size()-1) return 0;
  ...
  void Spectrum::selectTopK(unsigned int topK, Spectrum *putHere)
  {
    ...
    cerr
        << "selectTopK has not been updated for per-peak tolerances and should be replaced by rankFilterPeaks\n";
      ...
      vector<float> > &pairs, vector<vector<int> > &pairsIdx)
  {
    ...
    vector<float> peakScores; // Peak scores including scores of neighboring peaks within tolerance
    ...
    while (i <= j)
    {
      ...
      {
        for (k = j; k > i && peakList[k][0] >= aaMass - peakList[i][0]
            - getTolerance(k) + getTolerance(i); k--)
        {
      ...
      {
        for (k = i; k < j && peakList[k][0] <= aaMass - peakList[j][0]
            + getTolerance(k) + getTolerance(j); k++)
        {
          pairs[curPair].resize(3);
          pairsIdx[curPair].resize(2);
  ...
  // -------------------------------------------------------------------------
  float Spectrum::getGFProbability(int score,
                                  unsigned int atMass,
                                  bool roundMasses,
                                  AAJumps *aaMasses,
                                  ...
                                  bool outputGFTables)
  {
    ...
    if(parentMass<AAJumps::massMH) {
      cerr<<"Warning: bad spectrum with parent mass "<<parentMass<<", skipping GF calculation.\n";
      return 1.0;
    }
    unsigned int maxMass = (unsigned int) roundMasses?round(0.9995*(parentMass - AAJumps::massMH)):round(parentMass - AAJumps::massMH);
    if(maxMass != atMass) maxMass = atMass;
    ...
    if(aaMasses) {
      ...
      for(unsigned int peakIdx = 0; peakIdx<peakList.size(); peakIdx++) {
        int peakMass = (int) roundMasses?round(0.9995*peakList[peakIdx][0]):peakList[peakIdx][0];
      ...
      vector<vector<float> > *currentProbs = new vector<vector<float> >,
                             *prevProbs  = new vector<vector<float> >,
                             *tmp;  // Used when reassigning currentProbs to prevProbs
      DEBUG_VAR(maxMass);
      ...
      if(ntermModMasses) {
        ...
        for(unsigned int deltaIdx = 0; deltaIdx < ntermModMasses->size(); deltaIdx++)
          if(not (*ntermModMasses)[deltaIdx].second) numGenericMods++;
        ...
        for(unsigned int deltaIdx = 0; deltaIdx < ntermModMasses->size(); deltaIdx++) {
          ...
          if((*ntermModMasses)[deltaIdx].first <= maxMass) {
            if((*ntermModMasses)[deltaIdx].second)  // Mod mass includes one amino acid mass (e.g., PyroGlu)
              (*currentProbs)[(*ntermModMasses)[deltaIdx].first][scoreAtZero] = aaProb;
            else
              (*currentProbs)[(*ntermModMasses)[deltaIdx].first][scoreAtZero] = ntermProb;
          }
        }
      ...
      for(unsigned int numMods = 0; numMods <= maxNumAAMods; numMods++) {
        ...
        for(unsigned int massIdx = 0; massIdx <= maxMass; massIdx++) {
          ...
          for(unsigned int deltaIdx = 0; deltaIdx < intAAMasses.size(); deltaIdx++) {
            if(intAAMasses[deltaIdx]<=massIdx) {
              unsigned int prevMass = massIdx - intAAMasses[deltaIdx];
              for(unsigned int scoreIdx = curPRMScore; scoreIdx<=maxScore; scoreIdx++){
                (*currentProbs)[massIdx][scoreIdx] += aaProb * (*currentProbs)[prevMass][scoreIdx - curPRMScore];
                /*
          ...
          if(numMods>0) {
            for(unsigned int deltaIdx = 0; deltaIdx < modMasses->size(); deltaIdx++) {
              if((*modMasses)[deltaIdx]<=massIdx) {
                unsigned int prevMass = massIdx - (*modMasses)[deltaIdx];
                for(unsigned int scoreIdx = curPRMScore; scoreIdx<=maxScore; scoreIdx++){
                  (*currentProbs)[massIdx][scoreIdx] += aaProb * (*prevProbs)[prevMass][scoreIdx - curPRMScore];
                  /*
                  if((massIdx==97 and scoreIdx==1) or (massIdx==424 and scoreIdx==4)) {
                  cerr<<"prevMass = "<<prevMass<<", curPRMScore = "<<curPRMScore
                  <<", (*currentProbs)["<<"massIdx"<<"]["<<scoreIdx<<"] ("<<(*currentProbs)[massIdx][scoreIdx]
        ...
        if(outputGFTables) {
          stringstream s; s<<"currentProbs_scan_"<<scan<<"_numMods_"<<numMods<<".bin";
    ...
    if(score>=0 and score < m_gfProbs.size())
      return m_gfProbs[score];
    else {
      WARN_MSG("Spectrum::getGFProbability(): score "<<score<<" out of range [0,"<<m_gfProbs.size()-1<<"], returning prob 1.0\n");
      cerr << "Spectrum::getGFProbability(): score "<<score<<" out of range [0,"<<m_gfProbs.size()-1<<"], returning prob 1.0\n";
    }
                                              ...
                                              float peakTol)
  {
    ...
    for(unsigned int i=0; i < tmpSpec.size(); i++) {  // GF uses rounded masses/scores
      ...
      float scaledPeak = ceil(tmpSpec[i][1] / maxScore * 100.0); // Scale the peaks at the same time
