...
namespace specnets
{
  ...
  public:
    ...
    /**
     * Maintains a mapping of every single AA jump to its mass
     */
    map<string, double> massLookup;
     ...
     */
    string getLabel(const unsigned int i)
    {
      if (m_generateLabels)
      {
     ...
     *@param resolution
     */
    void forceTolerance(float tolerance, float resolution = 0.01, bool resetRefMasses = false);
     ...
     */
    void getPRMMasses(const string sequence,
                      vector<float> &masses,
     ...
     * @param addZeroMass: if true add offset mass to beginning of masses
     */
    void getSRMMasses(const string sequence,
                      vector<float> &masses,
                      const float offset = 0.0,
     ...
     *
     */
    void getPRMandSRMMasses(const string &sequence,
