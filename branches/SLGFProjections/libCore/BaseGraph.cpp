...
namespace specnets
{
  ...
  BaseGraph::BaseGraph(BaseGraph& other) :
    m_label("Graph")
  {
    copy(other);
  }
  ...
  void BaseGraph::initialize(unsigned long numNodes, unsigned long numEdges)
  {
    ...
    m_nodes.resize(numNodes, (Node*)0);
    m_edges.resize(numEdges, (Edge*)0);

    pair<IndexVector, FreeList> emptyPair;
    m_outEdges.resize(numNodes, emptyPair);
    m_inEdges.resize(numNodes, emptyPair);
  ...
  void BaseGraph::m_beginTraverse(Node* source)
  {
    m_queueStack.clear();
    m_visited.clear();
