...
namespace specnets
{
  ...
  // Blocked cysteines
  const double AAmasses[] = { 71.0371137870, 156.1011110260, 115.0269430310,
                                      ...
                                      short useMods)
  {
    ...
    globalJumps
        = new AAJumps(maxJumpSize, resolution, -1, useMods, false, true);
  }
  ...
  AAJumps& AAJumps::getGlobalJumps()
  {
    ...
    {
      cerr << "Global jumps have not been initialized!\n";
      abort();
    }
  }
                               ...
                               vector<string>& outputSuffixes)
  {
    ...
    while (i < peptideStr.length())
    {
      ...
      if (c != '(')
      {
        outputSuffixes[idxUse++]
            = peptideStr.substr(i, peptideStr.length() - i);
        i++;
        continue;
      }
      else if (i == peptideStr.length() - 1)
                         ...
                         bool generateLabels)
  {
    ...
    {
      cerr << "Getting labels with unique masses is not yet supported";
      masses.resize(0);
      abort();
    }
    ...
    if (refMasses.empty())
    {
      ...
      for (aaIndex = 0; aaIndex < glbMasses.size(); aaIndex++)
      {
        ...
        massLookup[letter] = refMasses[aaIndex];
      }
      ...
      for (aaIndex = 0; aaIndex < glbMods.size(); aaIndex++)
      {
        ...
        for (modIndex = 0; modIndex < refMods[aaIndex].size(); modIndex++)
        {
          ...
          massLookup[modStr] = refMasses[aaIndex] + refMods[aaIndex][modIndex];
        }
      }
    }
    ...
    for (step = 2; step <= maxJumpSize; step++)
    {
      ...
      for (prevIter = prevStepStart; prevIter <= prevStepEnd; prevIter++)
      {
        ...
        {
          double addMass = (aaIndex < refMasses.size()) ? refMasses[aaIndex]
                               ...
                               bool clearOldAA)
  {
    ...
    for (map<char, double>::iterator massIt = aaMasses.begin(); massIt
        != aaMasses.end(); massIt++)
    {
    ...
    for (map<char, list<double> >::iterator massIt = aaMods.begin(); massIt
        != aaMods.end(); massIt++)
    {
      unsigned int refIdx = charToIdx[massIt->first];
      refMods[refIdx].resize(massIt->second.size());
      idxUse = 0;
      for (list<double>::iterator modIt = massIt->second.begin(); modIt
          != massIt->second.end(); modIt++)
      {
  ...
  bool AAJumps::saveJumps(const char *filename)
  {
    ...
    if (refMasses.size() == 0)
    {
      cerr << "No amino acid masses to save in saveJumps()" << endl;
      return false;
    }
  ...
  // ************************************************************************************************
  //
  void AAJumps::forceTolerance(float tolerance, float resolution, bool resetRefMasses)
  {
    double iter;
  ...
  void AAJumps::computeIndex(double peakTol, double resolution, double maxMass)
  {
    ...
    else
    {
      ...
      // Initialize jStart/jEnd
      for (jStart = 0; jStart < jumpLabels.size() and jumpLabels[jStart].second
          <= 0; jStart++)
        ;
                                 ...
                                 int maxNumMods)
  {
    ...
    {
      cerr << "both mass and tolerance must be positive\n";
      abort();
    }
    if (!m_generateLabels)
    {
      cerr << "AAJumps was not initialized to generate labels\n";
      abort();
    }
    ...
    if (startMass >= index.size() || jumpLabels[index[startMass][0]].second
        > mass + tolerance)
    {
      return;
    }
    ...
    for (unsigned int i = index[startMass][0]; i <= index[endMass][1]; i++)
    {
      ...
      if (strictNumJumps >= 0 && getNumJumps(jumpLabels[i].first)
          != strictNumJumps)
      {
        continue;
      }
   ...
   */
  void AAJumps::getPRMMasses(const string sequence,
                             vector<float> &masses,
                             const float offset,
                             ...
                             const bool addZeroMass) const
  {
    ...
    for (unsigned int i = start; i < size - start; i++)
    {
      ...
      else
      {
        ...
        if (aa_index >= max_aa)
        {
          cerr << "Warning2: unknown aa! " << sequence[i] << " in sequence "
              << sequence << endl;
        }
        else
        {
  ...
  {
    vector<float> masses;
    getPRMMasses(sequence, masses, offset, (vector<string> *)0, addZeroMass);

    prmMassesAsSpectrum.resize(masses.size());
    if (masses.empty())
      return;
    for (unsigned int idxPeak = 0; idxPeak < prmMassesAsSpectrum.size(); idxPeak++)
    {
   ...
   */
  void AAJumps::getSRMMasses(const string sequence,
                             vector<float> &masses,
                             float offset,
                             bool addZeroMass) const
  {
   ...
   *
   */
  void AAJumps::getPRMandSRMMasses(const string &sequence,
  ...
  double AAJumps::getModPeptideMass(const string &sequence) const
  {
    ...
    {
      totalMass += massLookup.at(singleJumps[i]);
    }
    return totalMass;
  }
