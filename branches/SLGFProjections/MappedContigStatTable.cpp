...
namespace specnets
{
  ...
  void MappedContigStatTable::prepareTable(int index)
  {
    ...
    int numRows = ((contig->length - 1) * 12) + 10 + 31;
    ...
    setValue(19, 1, parseFloat(contig->getPercGaps(2), 1));

    setValue(20, 0, "% Annotated Gaps Incorrect", true);
    setValue(20, 1, parseFloat(contig->getPercGaps(1), 1));

    setValue(21, 0, "% Un-annotated Gaps", true);
    setValue(21, 1, parseFloat(contig->getPercGaps(0), 1));
    values[22].resize(0);

    setValue(23, 0, "Inspect Protein Index", true);
    setValue(23, 1, parseInt(contig->starProtIdx));

    setValue(24, 0, "Matchma Protein Index", true);
    setValue(24, 1, parseInt(contig->vertProtIdx));

    setValue(25, 0, "Matchma Reversed", true);
    val = (contig->reversed) ? "1" : "0";
    setValue(25, 1, val);

    setValue(26, 0, "Matchma AA Coverage", true);
    setValue(26, 1, parseInt(contig->lastResidue - contig->firstResidue + 1));

    setValue(27, 0, "Matchma Beg Residue Index", true);
    setValue(27, 1, parseInt(contig->firstResidue));

    setValue(28, 0, "Matchma End Residue Index", true);
    setValue(28, 1, parseInt(contig->lastResidue));
    values[29].resize(0);

    values[30].resize(7, dflt);
    setValue(30, 0, "Abruijn Vertex/Gap", true);
    setValue(30, 1, "Mass", true);
    setValue(30, 2, "Residue Idx", true);
    setValue(30, 3, "Intensity", true);
    setValue(30, 4, "Label", true);
    setValue(30, 5, "Annotation", true);
    setValue(30, 6, "Assembled Ions", true);
    ...
    for (int i = 0; i < contig->length; i++) {
      ...
      val
          = (contig->abruijnVerts[i].residueIdx >= 0) ? parseInt(contig->abruijnVerts[i].residueIdx) : "";
      setValue(tr, 2, val);
