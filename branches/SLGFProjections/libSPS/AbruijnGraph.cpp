...
namespace abruijn
{
  ...
  const double AbruijnGraph::MULTI_EDGE_PENALTY = 0.90;

  const unsigned int AbruijnGraph::PATH_EXPAND_LIMIT = 5;
  ...
  AbruijnGraph::AbruijnGraph(AbruijnGraph& other) :
    m_globalJumps(specnets::AAJumps::getGlobalJumps())
  {
    copy(other);
  }
  ...
  AbruijnGraph::AbruijnGraph(const pair<pair<vector<int> , vector<int> > ,
    ...
    m_globalJumps(specnets::AAJumps::getGlobalJumps())
  {
    loadAssembly(inAbinfo, assembledSpecs, peakTol, reverseStars, outAbinfo);
  }
  ...
  void AbruijnGraph::loadAssembly(const pair<pair<vector<int> , vector<int> > ,
                                      vector<
                                          pair<vector<int> , vector<double> > > >& inAbinfo,
                                  const SpecSet& assembledSpecs,
                                  const float& peakTol,
                                  bool reverseStars,
                                          ...
                                          pair<vector<int> , vector<double> > > >* outAbinfo)
  {
    ...
    // Find unique assmebled specs w/ their reversed flags
        ...
        != uAssembledSpecs.end(); sIt++)
    {
      ...
      for (unsigned int i = 0; i < spec.size();)
      {
        unsigned int j;
        float totalScore = spec[i][1];
        float maxScore = spec[i][1];
        unsigned int maxIdx = i;
        set<unsigned int> removeIdxs;
        removeIdxs.insert(i);
        for (j = i + 1; j < spec.size() && spec[j][0] < spec[i][0] + (2
            * peakTol); j++)
        {
          totalScore += spec[j][1];
          removeIdxs.insert(j);
          if (spec[j][1] > maxScore)
          {
            maxScore = spec[j][1];
            maxIdx = j;
          }
        }
        removeIdxs.erase(maxIdx);
        peaksToRemove.insert(peaksToRemove.end(),
                             removeIdxs.begin(),
                             removeIdxs.end());
        spec[maxIdx][1] = totalScore;
        i = j;
      }
      if (peaksToRemove.size() > 0)
      {
        spec.removePeaks(peaksToRemove);
      }

      for (unsigned int i = 1; i < spec.size() - 1; i++)
      {
        totalIntensity += spec[i][1];
        numIntensity += 1.0;
      }
        ...
        != foundSpecs.end(); specIdxIt++)
    {
      ...
      for (unsigned int i = 0; i < newAssembledSpecs[specIdx].size(); i++)
      {
        for (unsigned int j = i + 1; j < newAssembledSpecs[specIdx].size(); j++)
        {
          ...
          addAbruijnEdges(newAssembledSpecs[specIdx], i, j, &addedEdges);
  ...
  {
    AbruijnGraph* otherPtr = castGraphPtr(&otherGraph);
    BaseGraph::copy(otherGraph);

    m_consensusPath = otherPtr->m_consensusPath;
    m_consensusPathVerts = otherPtr->m_consensusPathVerts;
    m_reversed = otherPtr->m_reversed;
    m_nodeLookup = otherPtr->m_nodeLookup;
    m_globalJumps = otherPtr->m_globalJumps;
  }
  ...
  AbruijnNode* AbruijnGraph::getMultiJumpDestination(AbruijnEdge* abEdge,
                                                     bool goReverse,
  ...
  {
    AbruijnNode* nextDest = (goReverse) ? getNode(abEdge->getFromNodeIndex())
        : getNode(abEdge->getToNodeIndex());
    foundJumpMass = abEdge->getMass();
    foundWeight = abEdge->getWeight();
    foundPath = abEdge->getJumpLabel();
    foundPathFor = abEdge->getFLabel();
    foundPathRev = abEdge->getRLabel();
    if (specnets::AAJumps::getNumJumps(abEdge->getJumpLabel()) > 1
        || nextDest->haveRealPeak())
    {
      return nextDest;
    }
    ...
    while (true)
    {
      ...
      const IndexVector& outEdges = (goReverse) ? getInEdges(nextDest)
          : getOutEdges(nextDest);
      for (unsigned long j = 0; j < outEdges.size(); j++)
      {
        if (outEdges[j] < 0)
          continue;
        nextEdge = getEdge(outEdges[j]);
        numEdges++;
      }
      if (numEdges == 0)
      {
      ...
      if (numEdges > 1)
      {
        ERROR_MSG("Empty node " << nextDest->toString() << " has in/out degree " << numEdges);
        abort();
      }
      foundJumpMass += nextEdge->getMass();
      foundWeight += nextEdge->getWeight();
      if (goReverse)
        foundPath = nextEdge->getJumpLabel() + foundPath;
      else
        foundPath += nextEdge->getJumpLabel();

      nextDest = (goReverse) ? getNode(nextEdge->getFromNodeIndex())
          : getNode(nextEdge->getToNodeIndex());

      if (nextDest->haveRealPeak())
      {
        foundPathFor = foundPath;
        foundPathRev = specnets::AAJumps::reversePeptide(foundPath);

        string moreFor = (goReverse) ? abEdge->getFLabel()
            : nextEdge->getFLabel();
        moreFor.erase(0, 1);
        foundPathFor += moreFor;

        string moreRev = (goReverse) ? nextEdge->getRLabel()
            : abEdge->getRLabel();
        moreRev.erase(0, 1);
        foundPathRev += moreRev;
  ...
  /*
   void AbruijnGraph::removeEmptyNodes(void)
   {
   list<AbruijnNode*> emptyNodes;
   for (NodeSet::iterator nIt = this->beginNodes(); nIt != this->endNodes(); nIt++)
   {
   AbruijnNode* abNode = AbruijnNode::castNodePtr(*nIt);
   ...
   if (!foundReal)
   {
   emptyNodes.push_back(abNode);
   }
   }

   list<AbruijnEdge*> incoming;
   list<AbruijnEdge*> outgoing;

   DEBUG_VAR(emptyNodes.size());

   for (list<AbruijnNode*>::iterator nIt = emptyNodes.begin(); nIt
   != emptyNodes.end(); nIt++)
   {
   //string incomingLabels = "";
   for (AdjacMap::const_iterator neijIt = (*nIt)->beginIn(); neijIt
   != (*nIt)->endIn(); neijIt++)
   {
   for (EdgeList::const_iterator eIt = neijIt->second.begin(); eIt
   != neijIt->second.end(); eIt++)
   {
   AbruijnEdge* abEdge = AbruijnEdge::castEdgePtr(*eIt);
   incoming.push_back(abEdge);
   //incomingLabels += abEdge->getFLabel();
   //incomingLabels += "(" + parseInt(abEdge->getFrom()->getID()) + "), ";
   }
   }

   //string outgoingLabels = "";
   for (AdjacMap::const_iterator neijIt = (*nIt)->beginOut(); neijIt
   != (*nIt)->endOut(); neijIt++)
   {
   for (EdgeList::const_iterator eIt = neijIt->second.begin(); eIt
   != neijIt->second.end(); eIt++)
   {
   AbruijnEdge* abEdge = AbruijnEdge::castEdgePtr(*eIt);
   outgoing.push_back(abEdge);
   //outgoingLabels += abEdge->getFLabel();
   //outgoingLabels += "(" + parseInt(abEdge->getTo()->getID()) + "), ";
   }
   }

   for (list<AbruijnEdge*>::iterator inIt = incoming.begin(); inIt
   != incoming.end(); inIt++)
   {
   for (list<AbruijnEdge*>::iterator outIt = outgoing.begin(); outIt
   != outgoing.end(); outIt++)
   {
   AbruijnEdge newEdge;
   string jumpLabel = (*inIt)->getJumpLabel();
   jumpLabel += (*outIt)->getJumpLabel();

   newEdge.setJumpLabel(jumpLabel);
   newEdge.setFLabel(jumpLabel);
   newEdge.setRLabel(jumpLabel);
   newEdge.setModified((*inIt)->isModified() || (*outIt)->isModified());
   newEdge.setMass((*inIt)->getMass() + (*outIt)->getMass());
   newEdge.setWeight((*inIt)->getWeight() + (*outIt)->getWeight());

   AbruijnEdge* addedEdge =
   AbruijnEdge::castEdgePtr(this->addEdge((*inIt)->getFrom(),
   (*outIt)->getTo(),
   newEdge));
   }
   }
   ...
   this->removeNode((Node**)(&(*nIt)));
   }
   }
   */
  /*
   void AbruijnGraph::loadAssembly(const SpecSet& prmSpectra,
   const SpectrumAlignmentSet& prmAligns,
   bool addEndpointGlues)
   {
   // clear out graph
   initialize();

   // add star peak nodes and edges between them
   addSpectra(prmSpectra);

   // this->saveGraphviz("preAligns.dot");

   // glue nodes containing aligned peaks
   addGlues(prmAligns);

   // this->saveGraphviz("postAligns.dot");

   // find heaviest path through abruijn graph
   assembleConsensus(addEndpointGlues);

   ///
   list<AbruijnEdge*> delEdges;
   for (EdgeSet::const_iterator eIt = this->beginEdges();
   eIt != this->endEdges(); eIt++)
   {
   AbruijnEdge* abEdge = AbruijnEdge::castEdgePtr(*eIt);
   if (abEdge->getJumpLabel().length() > 1)
   {
   delEdges.push_back(abEdge);
   }
   }
   for (list<AbruijnEdge*>::iterator eIt = delEdges.begin();
   eIt != delEdges.end(); eIt++)
   {
                                     ...
                                     list<AbruijnEdge*>* addedEdges)
  {
        ...
        != foundJumps.end(); jIt++)
    {
      ...
      if (numJumps > 1)
      {
        // skip this jump if it has a redundant sub-jump
      ...
      if (sizeSet.size() > 1 && jIt->first.length() == maxLength)
      {
        edgeWeight *= MULTI_EDGE_PENALTY;
        rEdgeWeight *= MULTI_EDGE_PENALTY;
      }
  ...
  void AbruijnGraph::expandAllPaths(const bool& goReverse)
  {
    if (this->numNodes() == 0)
    {
      WARN_MSG("Calling expandAllPaths on an empty graph!!");
      return;
    }
    /**
        ...
        rankedDistNodes.begin(); nIt != rankedDistNodes.end(); nIt++)
    {
      ...
      if (DEBUG_EXPANSION)
      {
        DEBUG_MSG("Calling expandPaths on " << nIt->second->getIndex() << " with distance " << nIt->first);
      }
  ...
  /*
   string AbruijnGraph::getConsensusSeq(const bool reportMods) const
   {
   string consensus("");

   bool inBreak = false;
   for (Path::const_iterator pIt = m_consensusPath.begin(); pIt
   != m_consensusPath.end(); pIt++)
   {
   AbruijnEdge* abEdge = AbruijnEdge::castEdgePtr(*pIt);
   AbruijnNode* abNode = AbruijnNode::castNodePtr(abEdge->getTo());
   bool havePeaks = false;
   for (unsigned int i = 0; i < abNode->size(); i++)
   {
   if ((*abNode)[i].getIntensity() > -0.01)
   {
   havePeaks = true;
   break;
   }
   ...
   const string& label = abEdge->getJumpLabel();
   const string noModLabel =
   specnets::AAJumps::stripMods(abEdge->getJumpLabel());

   if ((!havePeaks && !inBreak) || (!inBreak && noModLabel.length() > 1))
   {
   consensus += "[";
   inBreak = true;
   }
   consensus += (reportMods) ? label : noModLabel;
   if ((inBreak && havePeaks) || (inBreak && noModLabel.length() > 1))
   {
   consensus += "]";
   inBreak = false;
   }
   ...
   */
  /*
   void AbruijnGraph::assembleConsensus(const bool& addEndpointGlues)
   {
   unsigned int numSpecs = m_assembledSpecs.size();
   unsigned int numAligns = m_alignments.size();

   if (addEndpointGlues)
   {
   // project start and end points to synthetic 'start' and 'end' point spectra
   Spectrum startPtSpec;
   Spectrum endPtSpec;
   SpectrumAlignmentSet startPtAligns;
   SpectrumAlignmentSet endPtAligns;

   map<string, MZRange> leftAlignedShifts;
   getEndPointSpectra(leftAlignedShifts,
   startPtSpec,
   endPtSpec,
   startPtAligns,
   endPtAligns);
   // add nodes and edges for start and end-point spectra
   //startPtSpec.output(cerr);
   //endPtSpec.output(cerr);

   addSpectrum(startPtSpec, true);
   addSpectrum(endPtSpec, true);

   // glue each real start and end point to synthetic spectra
   addGlues(startPtAligns);
   addGlues(endPtAligns);

   // Nodes for these spectra have already been added so remove their actual spectra and alignments
   // m_assembledSpecs.resize(numSpecs);
   // m_alignments.resize(numAligns);
   }

   // Remove nodes containing only y ions
   removeSymmetricYNodes();

   expandAllPaths(false);
   expandAllPaths(true);

   // compress any edges of the same label between the same pair of nodes
   mergeAllParallelEdges();

   //this->saveGraphviz("postExpand.dot");

   if (!computeHeaviestPathDAG())
   {
   ERROR_MSG("Could not find heaviest path!!!");
   abort();
   }
   }
   */
  /*
   void AbruijnGraph::appendGraph(const AbruijnGraph& otherGraph)
   {
   m_assembledSpecs.appendSpecSet(otherGraph.m_assembledSpecs, false);
   m_alignments.appendAlignmentSet(otherGraph.m_alignments);

   BaseGraph::appendGraph(otherGraph);
   }*/
  ...
  void AbruijnGraph::glueNodes(AbruijnNode* node1, AbruijnNode** node2)
  {
    ...
    if (DEBUG_EXPANSION)
    {
      DEBUG_MSG("Gluing \'" << node1->toString() << "\' and \'" << (*node2)->getGraphvizLabel() << "\'");
    }
