...
namespace specnets
{
  ...
  float MappedSpecnets::getPercSpecCov(int prot_idx)
  {
    ...
    for (int p = 0; p < proteins->size(); p++)
    {
      ...
      for (int i = 0; i < spectra->size(); i++)
      {
        ...
        if (spec->size() == 0 || (!spec->mapped) || (spec->residueIdxs.count(p)
            == 0))
        {
          continue;
        }

        for (list<int>::iterator startIt = spec->residueIdxs[p].begin(); startIt
            != spec->residueIdxs[p].end(); startIt++)
        {
  ...
  pair<float, float> MappedSpecnets::getPercBYVerts(int prot_idx)
  {
    ...
    return pair<float, float> ((numeratorB / denominator) * 100.0, (numeratorY
        / denominator) * 100.0);
  }
   ...
   * @return if label != 0, percent of abruijn gaps that have label out of
   *  those that are annotated. if label == 0, percent of abruijn gaps
   *  that have label out of those that are not annotated
   */
  float MappedSpecnets::getPercGaps(int prot_idx, short label)
  {
    float numerator = 0;
    float denominator = 0;
    ...
    for (int i = 0; i < contigs->size(); i++)
    {
      ...
      bool debug = false;//i == 424 && label == 2;

      if (debug)
      {
        DEBUG_VAR(i);
        DEBUG_VAR(numerator);
        DEBUG_VAR(denominator);
        DEBUG_VAR(prot_idx);
        DEBUG_VAR(label);
      }
      ...
      {
        if (label > 0 && contig->abruijnGaps[j].annotated)
        {
          denominator += 1.0;
          if (contig->abruijnGaps[j].label == label)
          {
            checkC++;
            numerator += 1.0;
          }
        ...
        {
          denominator += 1.0;
          numerator += (contig->abruijnGaps[j].annotated) ? 0.0 : 1.0;
        }
      }
      if (label == 1 && checkC > 2 && prot_idx == 1)
       {
       DEBUG_MSG("See contig " << i << " with " << checkC << " incorrect gaps");
       }

      debug = (prot_idx == 1 && label == 1 && checkC > 5);
  ...
  float MappedSpecnets::getPercAssemSpecIdent(int prot_idx)
  {
    ...
    for (int i = 0; i < contigs->size(); i++)
    {
      ...
      if (prot_idx >= 0 && (!contig->vertMapped))
      {
        continue;
      }
      if (prot_idx >= 0 && (contig->vertProtIdx != prot_idx))
      ...
      for (int s = 0; s < contig->size(); s++)
      {
        ...
        for (int v = 0; v < contig->abruijnVerts[s].starPeaks.size(); v++)
        {
          ...
          for (list<int>::iterator resIt =
              (*spectra)[specIdx].residueIdxs[prot_idx_use].begin(); resIt
              != (*spectra)[specIdx].residueIdxs[prot_idx_use].end(); resIt++)
          {
