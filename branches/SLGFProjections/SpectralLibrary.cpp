...
namespace specnets
{
    ...
    int SpectralLibrary::search(Spectrum &query_spec, PeptideSpectrumMatchSet &output_psms, float parentmass_tolerance, int top_hits){
        //AAJumps aajumps(1);
        vector<float> masses;
        int charge;
        ...
        for(int library_idx = 0; library_idx < specs.size(); library_idx++){
            ...
            //DEBUG_MSG(library_mass<<"\t"<<query_mass<<"\t"<<parentmass_tolerance);
            if(abs(query_mass - library_mass) > parentmass_tolerance) continue;
            
            //DEBUG_MSG("CLEARLY A VALID THING");
...
int SpectralLibrary::search_target_decoy_specset_SLGFProjection(SpectralLibrary &decoy, 
                                                                         SpecSet searchable_spectra,
                                                                         float parentmz_tolerance, 
                                                                         vector<Spectrum *> target_library_ptr, 
                                                                         vector<Spectrum *> decoy_library_ptr,
    ...
    vector<int> accepted_fragmentation;
        ...
        DEBUG_MSG("BEGIN EXTRACTED EVERYTHING +++++++++++++++++++++++++++++++++++++++++++++++++++++++++");
        ...
        DEBUG_MSG("EXTRACTED EVERYTHING +++++++++++++++++++++++++++++++++++++++++++++++++++++++++");
        
        int searched_count = 0;
        
        #pragma omp parallel for num_threads(18) schedule(guided)
        //for(int query_idx = 0; query_idx < searchable_spectra.size(); query_idx++){
        for(int query_idx = start_idx; query_idx < end_idx; query_idx++){
            ...
            int target_decoy_search = this->search_target_decoy_SLGFProjection(decoy, 
                                                                searchable_spectra[query_idx], 
                                                                targetdecoy_psm, 
                                                                parentmz_tolerance, 
                                                                target_library_ptr, 
...
int SpectralLibrary::search_target_decoy_SLGFProjection(SpectralLibrary &decoy, 
                                             Spectrum query_spec, 
                                             psmPtr output_psm, 
                                             float parentmz_tolerance,
                                             vector<Spectrum *> target_library_ptr, 
    ...
    //if(query_spec.scan != 9249) return -1;
    
    map<char, vector<float> > valid_mass_diffs = get_all_substitution_masses();
    ...
    float max_mass_diff = 120.0;
    float min_projection_diff = 3.0;
    ...
    for(int library_idx = 0; library_idx < this->size(); library_idx++){
        ...
        //if(abs(mass_difference) > 120) continue;
        if(abs(mass_difference) > max_mass_diff) continue;
        
        //if(mz_difference < -parentmz_tolerance || (mz_difference > (1.0/charge+parentmz_tolerance) && mz_difference < (16.0/charge-parentmz_tolerance) ) || mz_difference > (16.0/charge + parentmz_tolerance)) continue;
        //if(mz_difference < -parentmz_tolerance || (mz_difference > (1.0/charge+parentmz_tolerance) ) )continue;
        
        
        vector<string> deliminated_peptide = create_deliminated_aminoacids(query_psm->m_annotation);
        //DEBUG_VAR(deliminated_peptide.size());
        for(int i = 0; i < deliminated_peptide.size()+1; i++){
            //DEBUG_MSG(i);
            if(i == deliminated_peptide.size()){
                //DEBUG_MSG(mz_difference<<"\t"<<parentmz_tolerance);
                if(abs(mz_difference) > parentmz_tolerance) continue;
                query_psm->m_annotation = specs[library_idx].psmList.front()->m_annotation;
            }
            else{
                //Projection Section
                //Filtering if too low mz difference
                if(abs(mass_difference) < min_projection_diff) continue;
                
                string new_amino_acid;
                int valid = is_valid_mass_diff(valid_mass_diffs, mass_difference, deliminated_peptide[i], parentmz_tolerance, new_amino_acid);
                if(valid != 1) continue;
                
                vector<string> new_delimiated_peptide = deliminated_peptide;
                new_delimiated_peptide[i] = toupper(new_amino_acid[0]);
                
                if(deliminated_peptide[i] == "P" || deliminated_peptide[i] == "R" || deliminated_peptide[i] == "K" || new_delimiated_peptide[i] == "P" || new_delimiated_peptide[i] == "R" || new_delimiated_peptide[i] == "K") continue;
                
                string reconstructed_peptide = "";
                for(int del_idx = 0; del_idx < new_delimiated_peptide.size(); del_idx++){
                    reconstructed_peptide += new_delimiated_peptide[del_idx];
                }
                
                reconstructed_peptide = create_annotation_ends(reconstructed_peptide);
                
                query_psm->m_annotation = reconstructed_peptide;
            }
            ...
            float rescored_sim = SLGF_rescore(specs[library_idx].psmList.front()->SLGF_distribution, sim);
            ...
            search_result->m_score = rescored_sim*explained_intensity;
            search_result->m_strict_envelope_score = sim;
            search_result->m_unstrict_envelope_score = mass_difference;
            search_result->m_origAnnotation = specs[library_idx].psmList.front()->m_annotation;
            
            
            search_results.push_back(search_result);
            
            //cout<<"Mass DIFF\t"<<mass_difference<<"\t"<< specs[library_idx].psmList.front()->m_annotation <<"\t"<<query_psm->m_annotation<<"\t"<<sim<<endl;
        }
    }
    ...
    for(int decoy_idx = 0; decoy_idx < decoy.size(); decoy_idx++){
        ...
        for(int i = 0; i < deliminated_peptide.size()+1; i++){
            if(i == deliminated_peptide.size()){
                ...
                string new_amino_acid;
                int valid = is_valid_mass_diff(get_all_substitution_masses(), mass_difference, deliminated_peptide[i], parentmz_tolerance, new_amino_acid);
                if(valid != 1) continue;
            ...
            float rescored_sim = SLGF_rescore(decoy[decoy_idx].psmList.front()->SLGF_distribution, sim);
            ...
            search_result->m_strict_envelope_score = sim;
            search_result->m_unstrict_envelope_score = mass_difference;
            search_result->m_origAnnotation = decoy[decoy_idx].psmList.front()->m_annotation;
            
            search_results.push_back(search_result);
            
            //cout<<"Mass DIFF\t"<<mass_difference<<"\t"<<deliminated_peptide[i]<<"\t"<<valid<<"\t"<<new_amino_acid<<"\t"<< query_psm->m_annotation <<"\t"<<reconstructed_peptide<<"\t"<<sim<<endl;
        }
    }
    
    sort(search_results.begin(), search_results.end(), search_results_comparator_psmPtr);
    
    for(int i = 0 ; i < min(5, (int)search_results.size()); i++){
        //DEBUG_MSG(search_results[i]->m_annotation<<"\t"<<search_results[i]->m_score<<"\t"<<search_results[i]->m_strict_envelope_score);
    }
    
    if(search_results.size() > 0){
        output_psm->m_annotation = search_results[0]->m_annotation;
        output_psm->m_isDecoy = search_results[0]->m_isDecoy;
        output_psm->m_score = search_results[0]->m_score;
        output_psm->m_strict_envelope_score = search_results[0]->m_strict_envelope_score;
        output_psm->m_unstrict_envelope_score = search_results[0]->m_unstrict_envelope_score;
        output_psm->m_origAnnotation = search_results[0]->m_origAnnotation;
        return 0;
    }
