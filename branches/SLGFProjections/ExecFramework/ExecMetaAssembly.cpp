...
namespace specnets
{
  ...
  bool ExecMetaAssembly::invoke(void)
  {
    ...
    *m_outputAbruijn = params.out_abinfo;
    ...
    //DEBUG_MSG("Assembled " << numMerged << " of " << m_spectrumPairs->size() << " contig/contig alignments");

    /*
     // Get abruijn components of contigs in meta-contigs
     abinfo_t levelOneComps;
     overlapGraph.outputAbinfo(levelOneComps);

     // Get which contigs were reversed during meta-assembly
     vector<bool> reversedContigs;
     overlapGraph.outputReversed(reversedContigs);

     // Get meta-contig PRM spectra
     SpecSet metaContigPRMSpecs;
     overlapGraph.outputConsensusSpectra(metaContigPRMSpecs);

     vector<set<unsigned int> > metacontigs;
     overlapGraph.outputComponents(metacontigs);

     // Mark merged and un-merged contigs
     vector<bool> contigsMerged(metacontigs.size(), false);
     for (unsigned int i = 0; i < metacontigs.size(); i++)
     {
     if (metacontigs[i].size() == 0)
     {
     metaContigPRMSpecs[i].resize(0);
     }
     else if (metacontigs[i].size() == 1)
     {
     contigsMerged[i] = false;
     metaContigPRMSpecs[i] = (*m_spectra)[i];
     reversedContigs[i] = false;
     }
     else
     {
     for (set<unsigned int>::iterator cIt = metacontigs[i].begin(); cIt
     != metacontigs[i].end(); cIt++)
     {
     contigsMerged[*cIt] = true;
     }
     }
     }

     // Get abruijn components of star spectra in meta-contigs
     Combine_abinfo_v1_0(*m_inputAbruijn,
     reversedContigs,
     *m_stars,
     contigsMerged,
     *m_spectra,
     ...
     *m_outputAbruijn);
     ...
     // Format the output data as clusters (so everything is compatible w/ main_specnets)
     m_contigShifts->initializeFromAbinfo(metaContigPRMSpecs,
     *m_outputAbruijn,
     *m_stars,
     pmTol,
     pmTol);
     ...
     Clusters contigsAll;
     contigsAll = *m_contigShifts;
     abinfo_t abinfoAll;
     abinfoAll = *m_outputAbruijn;
     ...
     for (unsigned int i = 0; i < metacontigs.size(); i++)
     {
     if (metacontigs[i].size() >= minCompSize)
     {
     if (metacontigs[i].size() == 1)
     {
     (*m_outputAbruijn)[idxUse] = (*m_inputAbruijn)[i];
     m_contigShifts->consensus[idxUse] = (*m_spectra)[i];
     m_contigShifts->specIdx[idxUse] = contigsAll.specIdx[i];
     m_contigShifts->shifts[idxUse] = contigsAll.shifts[i];
     m_contigShifts->endpoints[idxUse] = contigsAll.endpoints[i];
     }
     else
     {
     (*m_outputAbruijn)[idxUse] = abinfoAll[i];
     m_contigShifts->consensus[idxUse] = contigsAll.consensus[i];
     m_contigShifts->specIdx[idxUse] = contigsAll.specIdx[i];
     m_contigShifts->shifts[idxUse] = contigsAll.shifts[i];
     m_contigShifts->endpoints[idxUse] = contigsAll.endpoints[i];
     }
  ...
  bool ExecMetaAssembly::loadInputData(void)
  {
    ...
    if (!ExecMergeConvert::loadSpecsetMultiple(m_params.getValue("INPUT_STARS"),
        ...
        && m_params.exists("INPUT_FASTA"))
    {
      ...
      if (!ExecMergeConvert::loadSpecset(m_params.getValue("INPUT_MIDX"),
                                         m_overlaps))
      {
        return false;
      }
