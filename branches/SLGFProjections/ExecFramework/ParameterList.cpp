...
namespace specnets
{
  ...
  const string
      PROTEOSAFE_PARAMS[NUM_PROTEOSAFE_PARAMS][2] = {
                                                       {
    ...
    for (size_t npos[3]; input.good();)
    {
      ...
      if (npos[0] != string::npos)
      {
        ...
        if (key == PROTEOSAFE_INFILE_PARAM)
        {
          ...
          sprintf(upload_mapping_buf, "%s%d", key.c_str(), upload_mapping_count);
  ...
  {
    std::map<std::string, std::string>::iterator itrFind = parameters.find(paramName);
