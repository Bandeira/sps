...
namespace specnets
{
  ...
  ExecFilterPairs::ExecFilterPairs(const ParameterList & inputParams,
    ...
    ExecBase(inputParams), m_inputSpectra(inputSpectra), m_inputSpectraMS2(0x0), ownInput(false),
        m_filteredPairs(0x0), m_ratios(0x0), m_means(0x0), m_varTerms(0x0),
        m_alignStats(0x0), m_specStats(0x0),
        ownOutput(true)
  {
    m_name = "ExecFilterPairs";
    m_type = "ExecFilterPairs";
  ...
  // -------------------------------------------------------------------------
  ExecFilterPairs::ExecFilterPairs(const ParameterList & inputParams) :
    ExecBase(inputParams), m_inputSpectra(0x0), m_inputSpectraMS2(0x0), ownInput(true),
        m_filteredPairs(0x0), m_ratios(0x0), m_means(0x0), m_varTerms(0x0),
        m_alignStats(0x0), m_specStats(0x0),
        ownOutput(true)
  {
  ...
  ExecFilterPairs::ExecFilterPairs(const ParameterList & inputParams,
    ...
    ExecBase(inputParams), m_inputSpectra(inputSpectra), m_inputSpectraMS2(inputSpectraMS2), ownInput(false),
        m_filteredPairs(filteredPairs), m_ratios(ratios), m_means(means),
        m_varTerms(varTerms), m_alignStats(alignStats), m_specStats(specStats),
        ownOutput(false)
  {
    m_name = "ExecFilterPairs";
    m_type = "ExecFilterPairs";
  ...
  bool ExecFilterPairs::invoke(void)
  {
    ...
    if (m_filteredPairs == 0x0)
    {
      ...
      m_specStats = new vector<vector<float> > ();
      DEBUG_TRACE;
    }
    ...
    DEBUG_TRACE;
    short minNumMatchedPeaks = (short)m_params.getValueInt("MIN_NUM_MATCHED_PEAKS", 0);
...
cerr<<"Using cosine branch\n";

      	getPairCosines(*m_inputSpectraMS2,
                         baseSpecIdx,
  ...
  bool ExecFilterPairs::loadInputData(void)
  {
    ...
    {
      if (!m_inputSpectra->LoadSpecSet_pklbin(m_params.getValue("INPUT_SPECS_PKLBIN").c_str()))
      {
  ...
  bool ExecFilterPairs::saveOutputData(void)
  {
    ...
    if (m_params.exists("OUTPUT_STATS"))
    {
      ...
      Save_binListArray<float,vector<float>,vector<float>::iterator>(specstatsFilename.c_str(), *m_specStats);

      string alignstatsFilename(statsFilename);
      alignstatsFilename += "_alignStats.bin";
      Save_binListArray<float,vector<float>,vector<float>::iterator>(alignstatsFilename.c_str(), *m_alignStats);
    }
  ...
  bool ExecFilterPairs::loadOutputData(void)
  {
    ...
    if (m_params.exists("OUTPUT_ALIGNS"))
    {
      ...
      if (m_filteredPairs->loadFromBinaryFile(m_params.getValue("OUTPUT_ALIGNS")) < 0)
      {
    ...
    if (m_params.exists("OUTPUT_VARIANCE"))
    {
      ...
      if (Load_binArray(m_params.getValue("OUTPUT_VARIANCE").c_str(), varTerms) < 0)
      {
    ...
    if (m_params.exists("OUTPUT_STATS"))
    {
      string specstatsFilename(statsFilename);
      specstatsFilename += "_specStats.bin";
      Load_binListArray<float,vector<float>,vector<float>::iterator>(specstatsFilename.c_str(), *m_specStats);

      string alignstatsFilename(statsFilename);
      alignstatsFilename += "_alignStats.bin";
      Load_binListArray<float,vector<float>,list<vector<float> >::iterator>(alignstatsFilename.c_str(), *m_alignStats);
    }
  ...
  /*! \brief Helper function to calculate number of spectrum pairs per CPU

   Helper function to calculate number of spectrum pairs per CPU without
   overflowing for large number of spectra / pairs. Used by ExecFilterPairs::split().
   @return Number of spectrum pairs to assign to each CPU
   @sa ExecFilterPairs::split()
   @param startIdx Index of the first spectrum to be aligned
   @param endIdx Index of the last spectrum to be aligned
   @param numCPUs Number of CPUs to be used for matching spectral pairs
   @param numSpecs Total number of spectra in the set
  */
  inline double getNumPairsPerCPU(unsigned int startIdx, unsigned int endIdx,
    ...
    // Base number of comparisons per spectrum: (numSpecs-endIdx-1) per spectrum (only match to spectra with higher indices)
    double baseComparisons =
            (double(numSpecs) - double(endIdx) - 1.0) * ((double(endIdx) - double(startIdx) + 1.0) / double(numCPUs) );
    // Add variable number of comparisons per spectrum: aligning to less spectra for larger indices
    double totalComparisons = baseComparisons +
            (double(endIdx) - double(startIdx) + 1.0) * ( (double(endIdx) - double(startIdx)) / (2.0 * double(numCPUs)) );

//    if(totalComparisons >= double(2^32)) {
//	  ERROR_MSG("(getNumPairsPerCPU) Number of pairs per CPU exceeds MAXINT ("<<totalComparisons<<")!");
//	  exit(-1);
//    }
    return round(totalComparisons);
  }
...
// -------------------------------------------------------------------------
  ...
  vector<ExecBase*> const & ExecFilterPairs::split(int numSplit)
  {
    ...
    double comparisonsPerCpu = getNumPairsPerCPU(startBaseIdx, endBaseIdx, numSplit, spectraSize);
    ...
    for (int i = startBaseIdx; i < endBaseIdx; i++)
    {
      ...
      double comparisonsThisSpectra = spectraSize - i - 1;
...
//      DEBUG_VAR(comparisonsPerCpu);
      ...
      if (runningCount >= comparisonsPerCpu || i == endBaseIdx - 1)
      {
        ...
        theClone->m_params.setValue("OUTPUT_ALIGNS", baseName + "_aligns"
            + ".bin");
        theClone->m_params.setValue("OUTPUT_MEANS", baseName + "_means_"
            + ".bin");
        theClone->m_params.setValue("OUTPUT_VARIANCE", baseName + "_vars_"
            + ".bin");
        theClone->m_params.setValue("OUTPUT_RATIOS", baseName + "_ratios_"
            + ".bin");
...
//        DEBUG_VAR(remainingSplits);
        if(remainingSplits==0)
        	...
        	comparisonsPerCpu = getNumPairsPerCPU(i+1, endBaseIdx, remainingSplits, spectraSize);
//        DEBUG_VAR(comparisonsPerCpu);
  ...
  bool ExecFilterPairs::merge(void)
  {
    ...
    for (int i = 1; i < m_subModules.size(); i++)
    {
      ...
      for (int pivot = 0; pivot < m_inputSpectra->size(); pivot++)
      {
        ...
        {
          ratio1 = (*m_means)[pivot][1] / ((*m_means)[pivot][1]
              + (*fpe->m_means)[pivot].values[1]);
