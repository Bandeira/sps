...
namespace specnets
{
  ...
  bool ExecPairSpectra::invoke(void)
  {
    ...
    outputLpHandle
        << "unmodifiedMgf\tmodifiedMgf\tunmodifiedAnnotation\tlpOutputFilename\tallMassMgf\tvariantOutputFile\tfileName\tscanNum"
        << endl;
    ...
    for (int i = 0; i < filteredPsms.size(); i++)
    {
                                                   ...
                                                   unmodifiedPsmMatches))
      {
        ...
        outputLpHandle << unmodifiedMgfFile << "\t" << modMgfFile << "\t"
            << unmodifiedPsm->m_annotation << "\t" << lpResultsFilename << "\t"
  ...
  // -------------------------------------------------------------------------
  bool ExecPairSpectra::loadSpecSet(string & path,
                                    string & fileExt,
                                    SpecSet & currSet)
  {
    //load in file based on file ext.
    if (fileExt.compare("mzXML") == 0)
    {
      vector<short> msLevels;
      if (!LoadMzxml(path.c_str(), currSet, &msLevels, 2))
      {
        return false;
      }
    }
    else if (fileExt.compare("pklbin") == 0)
    {
      std::stringstream ss;
      ...
      if (!currSet.LoadSpecSet_pklbin(path.c_str()))
      {
        return false;
      }
                                            ...
                                            SpecSet &outputResults)
  {
    ...
    for (int i = 0; i < spectraFileList.size(); i++)
    {
      ...
      for (int j = 0; j < psms.m_psmSet.size(); j++)
      {
        ...
        key.str("");
        key.clear();
        key << psms.m_psmSet[j]->m_scanNum << filename;

        if (psmFilename.compare(filename) == 0 && passedSpectra.find(key.str())
            == passedSpectra.end())
        {
  ...
  bool ExecPairSpectra::loadInputData(void)
  {
    ...
    if (m_params.exists("INPUT_INSPECT_FILE_LIST"))
    {
      if (!m_peptideResults->loadInspectResultsFiles(m_params.getValue("INPUT_INSPECT_FILE_LIST").c_str()))
      {
        ERROR_MSG("Unable to load peptide results! " << m_params.getValue("INPUT_INSPECT_FILE_LIST"));
        return false;
      }
    }
