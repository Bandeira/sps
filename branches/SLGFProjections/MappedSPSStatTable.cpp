...
void MappedSPSStatTable::prepareTable() {
	...
	values[row].resize(2, deflt);
	values[row][0].first = "Modified Spectra";
	values[row][0].second = true;
	...
	values[row][0].first = "Da Length Per Contig";
	values[row++][0].second = true;

	values[row][0].first = "AA Covered Per Contig";
	values[row++][0].second = true;

	values[row][0].first = "Longest AA Region Covered by Contig";
	values[row++][0].second = true;

	values[row][0].first = "Contig Covering Longest AA Region";
	values[row++][0].second = true;
	...
	values[row][0].first = "Annotated Gaps Correct (%)";
	values[row++][0].second = true;

	values[row][0].first = "Annotated Gaps Incorrect (%)";
	values[row++][0].second = true;

	values[row][0].first = "Un-annotated Gaps (%)";
	values[row++][0].second = true;
	...
	for (int prot_idx = -1; prot_idx < numProts; prot_idx++) {
		...
		int tp = prot_idx + 2;
		//cout << "on " << prot_idx << "\n";
		if (prot_idx >= 0 && (*mapped_sps_proj->proteins)[prot_idx].length()
				== 0) {
		...
		values[row++][tp].first = parseInt(
				mapped_sps_proj->getNumContigsMapped(prot_idx));
		values[row++][tp].first = parseInt(
				mapped_sps_proj->getNumContigsSpecMapped(prot_idx));
		values[row++][tp].first = parseInt(
				mapped_sps_proj->getNumContigsVertMapped(prot_idx));
		values[row++][tp].first = parseInt(
				mapped_sps_proj->getNumContigsVertSpecMapped(prot_idx));
		values[row++][tp].first = parseInt(mapped_sps_proj->getNumSpecMapped(
				prot_idx));
		values[row++][tp].first = parseInt(
				mapped_sps_proj->getNumAssembledSpec(prot_idx));
		values[row++][tp].first = parseFloat(
				mapped_sps_proj->getPercAssemSpecIdent(prot_idx), 1);
		values[row++][tp].first = parseFloat(
				mapped_sps_proj->getPercAssemIdentSpecMod(prot_idx), 1);
		values[row++][tp].first = parseFloat(
				mapped_sps_proj->getMatchmaAccuracy(prot_idx), 1);
		values[row++][tp].first = parseFloat(mapped_sps_proj->getPercSpecCov(
				prot_idx), 1);
		values[row++][tp].first = parseFloat(mapped_sps_proj->getPercSeqCov(
				prot_idx), 1);
		values[row++][tp].first = parseFloat(mapped_sps_proj->getCovRedundancy(
				prot_idx), 1);
		values[row++][tp].first = parseFloat(mapped_sps_proj->getSpecPerContig(
				prot_idx), 1);
		values[row++][tp].first = parseFloat(mapped_sps_proj->getPepPerContig(
				prot_idx), 1);
		values[row++][tp].first = parseFloat(
				mapped_sps_proj->getDaLengthPerContig(prot_idx), 1);
		values[row++][tp].first = parseFloat(
				mapped_sps_proj->getAALengthPerContig(prot_idx), 1);
		...
		values[row++][tp].first = parseFloat(mapped_sps_proj->getPercVerts(
				prot_idx, 3), 1);
		values[row++][tp].first = parseFloat(mapped_sps_proj->getPercVerts(
				prot_idx, 2), 1);
		values[row++][tp].first = parseFloat(mapped_sps_proj->getPercVerts(
		...
		values[row++][tp].first = parseFloat(mapped_sps_proj->getPercGaps(
				prot_idx, 2), 1);
		values[row++][tp].first = parseFloat(mapped_sps_proj->getPercGaps(
				prot_idx, 1), 1);
		values[row++][tp].first = parseFloat(mapped_sps_proj->getPercGaps(
