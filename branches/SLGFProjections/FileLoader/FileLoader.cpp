//
// $Id: msconvert.cpp 3545 2012-04-18 00:26:14Z pcbrefugee $
//
//
...
Config parseCommandLine(int argc, const char* argv[])
{
    ...
    {
        ifstream is(configFilename.c_str(), ios::binary);
    ...
    {
        ifstream is(filelistFilename.c_str(), ios::binary);
        while (is)
        {
            string filename;
            getline(is, filename);
...
{
    ifstream is(contactFilename.c_str(), ios::binary);
    if (!is)
    {
