...
namespace specnets
{
  ...
  int compareAbInfo(abinfo_t & abinfo1, abinfo_t & abinfo2, abDiff_t &diffData)
  {
    ...
    // cycle thru
    for (it1 = abinfo1.begin(), it2 = abinfo2.begin() ; it1 != abinfo1.end() && it2 != abinfo2.end() ; it1++, it2++) {
      ...
      if(rev1IdxSize == rev1FlgSize && rev2IdxSize == rev2FlgSize && rev1IdxSize == rev2IdxSize)
      ...
      // get the nodes
      vector<std::pair<vector<int> , vector<double> > > &nodes1 = it1->second.second;
      vector<std::pair<vector<int> , vector<double> > > &nodes2 = it2->second.second;
      ...
      for(int i = 0 ; i < diffContig.nodesList.size1 ; i++) {
        ...
        if(node.size1i != node.size2i || node.size1i != node.size1d || node.size2i != node.size2d) {
