...
//cerr<<" -> spec sizes "<<(*specSet)[specIdxL].size()<<" / "<<(*specSet)[specIdxR].size()<<"\n"; cerr.flush();
			...
			// modification pair?
			if( (fabs(aligns[pIdx].shift1)<=pmTol and fabs(aligns[pIdx].shift2)<=maxModMass and fabs(aligns[pIdx].shift2) > pmTol )
					or (fabs(aligns[pIdx].shift1)<=maxModMass and fabs(aligns[pIdx].shift2)<=pmTol and fabs(aligns[pIdx].shift1) > pmTol) ) {
				jumpMassLeft = max( fabs(aligns[pIdx].shift1) , fabs(aligns[pIdx].shift2) );
...
//cerr<<endl;
				...
				jumpMassLeft = (*specSet)[specIdxR][peakIdxR][0] - (*specSet)[specIdxL][peakIdxL][0];
				// Set jumpMassLeft to closest shift
				if( fabs(jumpMassLeft-aligns[pIdx].shift1) < fabs(jumpMassLeft-aligns[pIdx].shift2))
				{
...
//cerr<<", left(2)="<<jumpMassLeft; cerr.flush();
				...
				jumpMassRight = (*specSet)[specIdxR].parentMass + jumpMassLeft - (*specSet)[specIdxL].parentMass;
...
//cerr<<" +Start-edge"; cerr.flush();
					for(unsigned int p=0; p<2; p++)
						if(jumpMassLeft>-0.00001) {
							fromV = addVertex(specIdxL,p);	  toV = addVertex(specIdxR,p);   if(fromV==toV) continue; // Don't add self cycles
							vertices[fromV].addEdge(jumpMassLeft, (*specSet)[specIdxL][p][1]+(*specSet)[specIdxR][p][1], specIdxR, p, p);
						} else {
							fromV = addVertex(specIdxR,p);	  toV = addVertex(specIdxL,p);   if(fromV==toV) continue; // Don't add self cycles
							vertices[fromV].addEdge(-jumpMassLeft, (*specSet)[specIdxL][p][1]+(*specSet)[specIdxR][p][1], specIdxL, p, p);
						}
				}
			}
//else cerr<<"(jumpMassLeft invalid)"; cerr.flush();
			...
			if( jumps.isValid(fabs(jumpMassRight), pmTol) ) {
				...
				if(!isModPair or (isModPair and modPos[pIdx]>=(*specSet)[specIdxL].parentMass-pmTol) ) {
					unsigned int szL=(*specSet)[specIdxL].size(), szR=(*specSet)[specIdxR].size();  // In this case the edge is from smaller PM to larger PM
...
//cerr<<" +End-edge"; cerr.flush();
					for(unsigned int p=1; p<3; p++)
						if(jumpMassRight>-0.00001) {
							fromV = addVertex(specIdxL,szL-p);	  toV = addVertex(specIdxR,szR-p);  if(fromV==toV) continue; // Don't add self cycles
							vertices[fromV].addEdge(jumpMassRight, (*specSet)[specIdxL][szL-p][1]+(*specSet)[specIdxR][szR-p][1], specIdxR, szL-p, szR-p);
						} else {
							fromV = addVertex(specIdxR,szR-p);	  toV = addVertex(specIdxL,szL-p);  if(fromV==toV) continue; // Don't add self cycles
							vertices[fromV].addEdge(-jumpMassRight, (*specSet)[specIdxL][szL-p][1]+(*specSet)[specIdxR][szR-p][1], specIdxL, szR-p, szL-p);
						}
				}
			}
...
//cerr<<endl;
	...
	void VertexSet::buildGraph(MSGraph &g, AAJumps &jumps, float peakTol, vector<int> &vSet_index,
			vector<SpectrumPeakLabels> &labels, short edgeScoreType) {
	...
	//	list<MatchEdge>::iterator edge;
		...
		while(curVert<(int)vertices.size()) {
			...
			for(; iter!=vertices[curVert].specPeaks.end(); iter++) {
				...
				if(!labels.empty()) g.vPeakLabels[gVert].merge(labels[(*iter)[0]][(*iter)[1]]);
			}
	...
	void VertexSet::enumerateHeaviestPath(MSGraph &g, vector<int> &vSet_index, SpecSet &enumSpecs, vector<int> &enumSpecsIdx) {
		...
		for(;specIter != matchedSpecsIdx.end(); specIter++)  {
			...
			enumSpecs[esIdx].resize(numMatchedPeaks);   numMatchedPeaks=0;
	...
	*/
	 ...
	 */
	bool merge_abinfo(abinfo_t & info1, abinfo_t & info2, const char * filename, unsigned int totalSpecCount)
	{
	  ...
	  for (compit = info2.begin(); compit != info2.end(); compit++) {
	    ...
	    vertsPerComp[compit->first + numComponents1] = (compit->second).second.size();
	    for (int i = 0; i < (compit->second).second.size(); i++) {
	      peaksPerVert[gVidx++] = (compit->second).second[i].second.size();

	      //Spectrum index needs to be shifted by N
	      for (int j = 0; j < (compit->second).second[i].second.size(); j++) {
		vertsSpecIdx[gPidx] = (unsigned int)(compit->second).second[i].first[j] + totalSpecCount;
		vertsPeakMass[gPidx] = (compit->second).second[i].second[j];
		gPidx ++;
	      }
	    }
