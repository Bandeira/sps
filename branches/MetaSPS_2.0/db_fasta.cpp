...
namespace specnets
{
  ...
  /**
    ...
    {
    	int org_size = desc.size();
    	desc.resize(org_size*2);
    	sequences.resize(org_size*2);
    	masses.resize(org_size*2);

    	for(int i = 0; i < org_size; i++)
    	{
    		string rev = sequences[i];
    		std::reverse(rev.begin(), rev.end());
    		const char* cc_rev = rev.c_str();
    		char* c_rev = const_cast<char*>(cc_rev);
    		sequences[org_size +i] = (char *) malloc(strlen(c_rev) + 1);
    		strcpy(sequences[org_size +i], c_rev);

    		string rev_desc = "XXX" + (string)desc[i];
    		const char* cc_desc = rev_desc.c_str();
    		char* c_desc = const_cast<char*>(cc_desc);
            desc[org_size +i] = (char *) malloc(strlen(c_desc) + 1);
            strcpy(desc[org_size +i], c_desc);
    	}
    }
  /**
    * Added by Christina Boucher, cboucher@ucsd.edu, Jul 2011
...
//		  DEBUG_MSG(peptide);
  ...
  DB_fasta &DB_fasta::operator=(DB_fasta &other)
  {
    ...
    for (unsigned int i = 0; i < sizeDB; i++) {
      if (other.desc[i]) {
        desc[i] = (char *) malloc(strlen(other.desc[i]) + 1);
        strcpy(desc[i], other.desc[i]);
      }
      else
        desc[i] = (char *) 0x0;
      if (other.sequences[i]) {
        sequences[i] = (char *) malloc(strlen(other.sequences[i]) + 1);
        strcpy(sequences[i], other.sequences[i]);
      }
      else
        sequences[i] = (char *) 0x0;
      if (other.IDs[i]) {
        IDs[i] = (char *) malloc(strlen(other.IDs[i]) + 1);
  ...
  void DB_fasta::reset()
  {
    for (unsigned int seqIdx = 0; seqIdx < IDs.size(); seqIdx++) {
      if (IDs[seqIdx] != emptyStr)
        free( IDs[seqIdx]);
      if (desc[seqIdx] != emptyStr)
        free( desc[seqIdx]);
      if (sequences[seqIdx] != emptyStr)
        free( sequences[seqIdx]);
    }
  }
  ...
  void DB_fasta::Add(char *ID, char *des, char *seq)
  {
    ...
    char * aux1 = (char *)malloc( strlen(aux.c_str()) + 1);
    char * aux2 = (char *)malloc( strlen(des) + 1);
    char * aux3 = (char *)malloc( strlen(seq) + 1);
  ...
  unsigned int DB_fasta::Load(const char *filename)
  {
    ...
    for (; lineIdx < blr.size(); lineIdx++)
      if (blr.getline(lineIdx)[0] == '>') {
        sequences[seqIdx] = (char *) malloc(seqSizes.front() + 1);
        if (!sequences[seqIdx]) {
        ...
        if (token) {
          IDs[seqIdx] = (char*) malloc(strlen(token) + 1);
          if (!IDs[seqIdx]) {
        ...
        token = strtok(NULL, "");
        if (token) {
          desc[seqIdx] = (char*) malloc(strlen(token) + 1);
          if (!desc[seqIdx]) {
            cerr << "ERROR: Could not allocate " << strlen(token) + 1
                << " bytes!\n";
            return 0;
  ...
  {
    FILE * pepsFID = (FILE *) fopen(filename, "w");
    if (pepsFID == 0x0) {
      return -1;
    } 
                     ...
                     vector<TwoValues<int> > &matchedIndices)
  {
    ...
    matchedP1 = (char*) malloc(1 + szP1 + szP2);
    matchedP1[0] = 0;
    matchedP2 = (char*) malloc(1 + szP1 + szP2);
    matchedP2[0] = 0;
                            ...
                            short newTagLength)
  {
    ...
    curTag.text = (char*) malloc(sizeof(char) * (tagLength + 1));
  ...
  bool DB_index::find(char *tag, list<pair<int, short> > **location)
  {
    ...
    (*location) = (list<pair<int, short> > *) 0;
    return false;
  }

  bool DB_index::find(char *tag,
                      DB_fasta &db,
                      ...
                      float tolSuff)
  {
    ...
    if (flankPref < -tolPref or flankSuff < -tolSuff)
      return false;
