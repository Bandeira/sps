...
namespace specnets
{
  ...
  bool ExecFdrPeptide::invoke(void)
  {
    ...
    {
      DEBUG_TRACE;
      if (!FdrPeptide::calculatePValues(*m_peptideResults, *m_fdrPeptides))
      {
        return false;
      }
    }
    else
    {
      ...
      if (!FdrPeptide::mergeTargetDecoy(*m_targetResults,
                                        *m_decoyResults,
                                        mergedPeptides))
      {
        return false;
      }
      if (!FdrPeptide::calculatePValues(mergedPeptides, *m_fdrPeptides))
      {
        return false;
      }
    }
