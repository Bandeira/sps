...
namespace specnets
{
	...
	ExecAssembly::ExecAssembly(void) :
		m_spectra(0x0), m_spectrumPairs(0x0), ownInput(true),
	...
	ExecAssembly::ExecAssembly(const ParameterList & inputParams,
	  ...
	  ExecBase(inputParams),
		  m_spectra(spectra),
		  m_spectrumPairs(spectrumPairs),
		  ownInput(false),
		  m_contigShifts(outputContigShifts),
		  ownOutput(false),
	...
	ExecAssembly::~ExecAssembly(void)
	{
		if (ownInput)
		{
			...
			if(m_spectrumPairs) delete m_spectrumPairs;
		}
		if (ownOutput)
		{
			if(m_contigShifts) delete m_contigShifts;
			if (m_outputAbruijn) delete m_outputAbruijn;
		}
	}
	...
	bool ExecAssembly::invoke(void)
	{
		...
		float penalty_sameVert  = (float) m_params.getValueDouble("PENALTY_SAME_VERTEX");

		int graphType           = m_params.exists("GRAPH_TYPE")?m_params.getValueInt("GRAPH_TYPE"):2;
		int maxAAjump           = m_params.exists("MAX_AA_JUMP")?m_params.getValueInt("MAX_AA_JUMP"):0;
		float maxModMass        = m_params.exists("MAX_MOD_MASS")?(float) m_params.getValueDouble("MAX_MOD_MASS"):100.0;
		float peakTol           = m_params.exists("TOLERANCE_PEAK")?(float) m_params.getValueDouble("TOLERANCE_PEAK"):0.5;
		float pmTol             = m_params.exists("TOLERANCE_PM")?(float) m_params.getValueDouble("TOLERANCE_PM"):1.0;
	  short edgeScoreType     = (short)(m_params.exists("EDGE_SCORE_TYPE")?m_params.getValueInt("EDGE_SCORE_TYPE"):1);
	  unsigned int minMatchedPeaks = m_params.exists("MIN_MATCHED_PEAKS")?(int) m_params.getValueInt("MIN_MATCHED_PEAKS"):1;
	  unsigned int minEdgesToComponent = m_params.exists("MIN_EDGES_TO_COMPONENT")?(int) m_params.getValueInt("MIN_EDGES_TO_COMPONENT"):1;
	  unsigned int pathMinSpecs = (unsigned int)(m_params.exists("PATH_MIN_SPECS")?m_params.getValueInt("PATH_MIN_SPECS"):1);
	  short pathMinPeaks        = (short)(m_params.exists("PATH_MIN_PEAKS")?m_params.getValueInt("PATH_MIN_PEAKS"):1);
	  int specType              = m_params.exists("SPEC_TYPE_MSMS")?((int) m_params.getValueInt("SPEC_TYPE_MSMS")?1:0):0;
		float ionOffset           = specType?AAJumps::massHion:0;

		bool noSequencing         = m_params.exists("NO_SEQUENCING")?(bool)m_params.getValueInt("NO_SEQUENCING"):false;
		bool addEndpoints         = m_params.exists("ADD_ENDPOINTS")?(bool)m_params.getValueInt("ADD_ENDPOINTS"):true;
		...
		if (addEndpoints) {
			...
			if(!m_labels.empty()) {
				for(unsigned int i=0; i<m_spectra->size(); i++) (*m_spectra)[i].addZPMpeaks(peakTol,true,&m_labels[i]);
			} else {
				for(unsigned int i=0; i<m_spectra->size(); i++) (*m_spectra)[i].addZPMpeaks(peakTol,ionOffset,true);
			}
		}
		...
		vector<bool>       specFlipped(m_spectra->size()); for(unsigned int i=0;i<specFlipped.size();i++) specFlipped[i]=false;
	...
	//	for(unsigned int cIdx=20; cIdx<21; cIdx++) {
	    ...
	    for(unsigned int cIdx=0; cIdx<components.size(); cIdx++) {
  		...
  		DEBUG_MSG("Processing component "<<cIdx);
			...
			while(prevNumSpecs!=components.sets[cIdx].size()) {  // Iterates ABruijn/sequencing/split until all
				...
				stringstream logMsg; logMsg << "  - Spectrum indices: ";
				for(list<int>::iterator iter=components.sets[cIdx].begin();iter!=components.sets[cIdx].end();iter++)
				  logMsg<<*iter<<" "; logMsg<<endl;
				logMsg << "  - Component defined by "<< components.cAlignsPA[cIdx].size() << " pairs...\n";
		...
		//		SplitPairs3(*m_spectra, cAlignsASP[cIdx], components.cAlignsPA[cIdx], peakTol, maxAAjump, penalty_sameVert, penalty_ptm, matches, matchesPA, specFlipped, modPos, false, &m_labels);
				SplitPairs(*m_spectra, components.cAlignsPA[cIdx], peakTol, pmTol, maxAAjump, maxModMass, penalty_sameVert, penalty_ptm, matches, specFlipped, modPos, minMatchedPeaks, minEdgesToComponent, false, NULL, &m_labels);
...
if(cIdx==70)
	...
	*/
				...
				if(addEndpoints)
					for(sicIter=cSpectra[cIdx].begin();sicIter!=cSpectra[cIdx].end();sicIter++)
						(*m_spectra)[*sicIter].maximizeZPMpeaks(peakTol,true);
				...
				if(graphType>0)
					for(unsigned int i=0; i<components.cAlignsPA[cIdx].size(); i++) {
						if(spectrumGraphs[components.cAlignsPA[cIdx][i].spec1].numVerts()==0) {
							if(graphType==1) spectrumGraphs[components.cAlignsPA[cIdx][i].spec1].ConnectConsecutive((*m_spectra)[components.cAlignsPA[cIdx][i].spec1]);
							else spectrumGraphs[components.cAlignsPA[cIdx][i].spec1].ConnectJumps((*m_spectra)[components.cAlignsPA[cIdx][i].spec1],jumps2,peakTol);
	...
	//spectrumGraphs[components.cAlignsPA[cIdx][i].spec1].output_graphviz(sBuf);
						...
						if(spectrumGraphs[components.cAlignsPA[cIdx][i].spec2].numVerts()==0) {
							if(graphType==1) spectrumGraphs[components.cAlignsPA[cIdx][i].spec2].ConnectConsecutive((*m_spectra)[components.cAlignsPA[cIdx][i].spec2]);
							else spectrumGraphs[components.cAlignsPA[cIdx][i].spec2].ConnectJumps((*m_spectra)[components.cAlignsPA[cIdx][i].spec2],jumps2,peakTol);
	...
	//spectrumGraphs[components.cAlignsPA[cIdx][i].spec2].output_graphviz(sBuf);
				...
				if(specCount==0) { DEBUG_MSG("  - component is empty, ABruijn graph not built"); continue; }
  ...
  DEBUG_MSG(aux.str());
				...
				for(; iter!=compositeSet.end(); iter++) aux<<(*iter)<<" ";
...
//				if(addEndpoints) vSet.addEndpointEdges(tmpAlignsASP,matches,modPos,jumps2,peakTol);
				if(addEndpoints)
					vSet.addEndpointEdges(components.cAlignsPA[cIdx],matches,modPos,jumps2,maxModMass,peakTol,pmTol);
				...
				//
				vSet.buildGraph(abgB,jumps,peakTol,vSet_indexB,m_labels,edgeScoreType);
	//char filename[2048];   sprintf(filename,"component_%d.txt",cIdx+1);
	//abgB.output_graphviz(filename);
				if(noSequencing) {
					pathVertsIdxB.resize(0);
					if(!wholeABFN.empty()) vSet.getMatchedPeaks(pathVertsIdxB,abVerticesAll[cIdx]);
					continue;
				}
				...
				//  ABruijn endpoint-finishing procedures should go here.
				//
				copy.buildGraph(abgY,jumps,peakTol,vSet_indexY,m_labels,edgeScoreType);
	...
	//			scoreWithBep = max(scoreWithBep,scoreWithYep)+1;  // Force B endpoints
				...
				if(scoreWithBep>=scoreWithYep) { abg=&abgB; m_contigShifts->consensus[cIdx]=specWithBep; vSet_indexP = &vSet_indexB; vSetP = &vSet; pathVertsIdx=&pathVertsIdxB; DEBUG_MSG("  - Selected B endpoints.");}
				  else { abg=&abgY; m_contigShifts->consensus[cIdx]=specWithYep; vSet_indexP = &vSet_indexY; vSetP = &copy; pathVertsIdx=&pathVertsIdxY; DEBUG_MSG("  - Selected Y endpoints.");}
		...
		//		vSetP->output_graphviz_ma(sBuf, *pathVertsIdx);
				...
				m_contigShifts->set_endpoints(cIdx,*m_spectra,abVertices[cIdx],peakTol,pmTol);
		...
		//		cout<<"  - Heaviest path stats: ["; for(unsigned int i=0;i<cStats[cIdx].size();i++) {cout<<cStats[cIdx][i]; if(i<cStats[cIdx].size()-1) cout<<", "; } cout<<"]\n";
				...
				vector<short> numPathPeaks(m_spectra->size());   for(unsigned int i=0; i<m_spectra->size(); i++) numPathPeaks[i]=0;
	...
	//cerr<<endl;
            ...
            stringstream logMsg; logMsg<<"  - Keeping "<<usedSpecs.size()<<" spectra; number of components: "<<components.size()<<" -> ";
					...
					if (includedSpecs.size()>0) {  // Whenever there is at least 1 included spectrum
						stringstream logMsg; logMsg<<"  - Keeping "<<components.sets[cIdx].size()-includedSpecs.size()<<" spectra; number of components: "<<components.size()<<" -> ";
...
//		Save_binArray("abCounts.bin", abCounts);
		if(m_params.exists("OUTPUT_COMPLETE_ABRUIJN")) {
		  ...
		  if(aux.size())
  			Save_abinfo(aux.c_str(),*m_spectra, components.sets, specFlipped, abVerticesAll);
		}
    ...
    // Need to assign scan numbers so that PSMs will associate properly
  	...
  	Merge_abinfo_v1_0(*m_spectra, components.sets, specFlipped, abVertices, *m_outputAbruijn);
    ...
    if(ownInput) {
    	...
    	if(!m_spectrumPairs) m_spectrumPairs = new SpectrumPairSet;
    }
  	m_spectra->resize(0);
  	...
  	if(ownOutput) {
    	if(!m_contigShifts) m_contigShifts = new Clusters;
    	if(!m_outputAbruijn) m_outputAbruijn = new abinfo_t;
    }
    ...
    if(m_params.exists("AMINO_ACID_MASSES"))
		...
		if (m_spectra->LoadSpecSet_pklbin(m_params.getValue("INPUT_SPECS_PKLBIN").c_str())<=0 or m_spectra->size()==0) {
		...
		if(m_params.exists("INPUT_LABELS")) {
			if(LoadLabels(*m_spectra, m_params.getValue("INPUT_LABELS").c_str(), m_labels)<=0) {
	...
	bool ExecAssembly::saveOutputData(void)
	{
		...
		if(m_params.exists("OUTPUT_SPECS"))
			m_contigShifts->consensus.SaveSpecSet_pklbin(m_params.getValue("OUTPUT_SPECS").c_str());

		if(m_params.exists("OUTPUT_ABINFO")) {
		  Save_abinfo_v1_0(m_params.getValue("OUTPUT_ABINFO").c_str(), *m_outputAbruijn);
		}
