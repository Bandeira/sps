...
int BasicSpecReader::get_peak_list_from_MZXML(FILE *mzxml_stream)
{
	...
	char *bf=Buffer;
    if (fread(Buffer, sizeof(char), XML_BUFFER_SIZE, mzxml_stream) != XML_BUFFER_SIZE && !feof(mzxml_stream)) {
		cout << "Error parsing mzxml! " << endl;
        return 0;
	}
	...
	int _peak_bufsize = PeakBufferSize * 22 + 100;
    if (fread(PeakBuffer, sizeof(char), _peak_bufsize, mzxml_stream) != _peak_bufsize && !feof(mzxml_stream)) {
		cout << "Error parsing mzxml stream! " << endl;
        return 0;
	}
