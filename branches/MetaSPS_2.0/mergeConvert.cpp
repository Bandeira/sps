...
int main(int argc, char *argv[], char **envp)
{
  ...
  if (params.paramPresent(in_pklbin)) {
    ...
    for (int i = 0; i < filenames.size(); i++) {
      SpecSet in_specs;
      if (!in_specs.LoadSpecSet_pklbin(filenames[i].c_str())) {
  ...
  if (params.paramPresent(merge_prec) && params.getValueInt(merge_prec) == 1) {

    if (!params.paramPresent(peak_tol_ppm)) {
      cerr << "ERROR: Must specify " << peak_tol_ppm << " to merge spectra\n";
      return -1;
    }

    short mergeType =
        (params.paramPresent(merge_type)) ? params.getValueInt(merge_type) : 2;

    if (!params.paramPresent(merge_type)) {
      cout << merge_type << " not specified, automatically set to "
          << mergeType << "\n";
    }

    float ppmTol = params.getValueDouble(peak_tol_ppm);

    if (allSpecs.size() == 0) {
      cerr << "ERROR: No input spectra to merge.";
      return -1;
    }

    SpecSet mergedSpecs(allSpecs.size());
    if (MZRange::EqualWithinRange(allSpecs[0].parentMass,
                                  allSpecs[1].parentMass,
                                  0.001)) {
      cout << "Merging consecutive spectra from the same precursor\n";
      int rootSpecIdx = 0;
      mergedSpecs[rootSpecIdx] = allSpecs[0];
      float lastPM = allSpecs[0].parentMass;
      for (int i = 1; i < allSpecs.size(); i++) {
        if (MZRange::EqualWithinRange(allSpecs[i].parentMass, lastPM, 0.001)) {
          mergedSpecs[rootSpecIdx].mergeClosestPeaks(allSpecs[i].peakList,
                                                     ppmTol,
                                                     true,
                                                     mergeType);
        }
          ...
          ++rootSpecIdx;
          mergedSpecs[rootSpecIdx] = allSpecs[i];
          lastPM = allSpecs[i].parentMass;
        }
      }
      ...
      cout
          << "Merging spectra from the same precursor at parallel indices in each file\n";

      if (in_spec_sizes.size() <= 1) {
        cerr << "ERROR: No parallel spectra to merge.";
        return -1;
      }
      int mergedSize = in_spec_sizes[1];

      if (allSpecs.size() % mergedSize != 0) {
        cerr << "ERROR: Cannot split " << allSpecs.size() << " spectra into "
            << allSpecs.size() / mergedSize << " parallel sets of size "
            << mergedSize << "\n";
        return -1;
      }

      mergedSpecs.resize(mergedSize);
      for (int i = 0; i < mergedSize; i++) {
        mergedSpecs[i] = allSpecs[i];
        int parallelIdx = i + mergedSize;
        while (parallelIdx < allSpecs.size()) {
          /*
           ...
           */
          mergedSpecs[i].mergeClosestPeaks(allSpecs[parallelIdx].peakList,
                                           ppmTol,
                                           true,
                                           mergeType);
          parallelIdx += mergedSize;
        }
      }
    }
  ...
  if (params.paramPresent(conv_charge)) {

    vector<string> chargeRanges;
    splitText(params.getValue(conv_charge), chargeRanges, ":");
    set<short> chargeRangeVals;
    cout << "Converting charges ";
    if (!getRanges(chargeRanges[0].c_str(), chargeRangeVals)) {
      return -1;
    }
    for (set<short>::iterator zIt = chargeRangeVals.begin(); zIt
        != chargeRangeVals.end(); zIt++) {
      cout << *zIt << ", ";
    }
    short new_charge = (short) getInt(chargeRanges[1].c_str());
    float new_charge_float = (float) new_charge;
    cout << " to " << new_charge << "\n";
    for (int i = 0; i < allSpecs.size(); i++) {
      if (chargeRangeVals.count(allSpecs[i].parentCharge) > 0) {
        float prev_charge = (float) allSpecs[i].parentCharge;
        allSpecs[i].parentMZ = (allSpecs[i].parentMass + (new_charge_float - 1)
            * AAJumps::massHion) / new_charge_float;
        allSpecs[i].parentCharge = new_charge;
      }
    }
  }

  if (params.paramPresent(intensity_filt)) {
    float minIntensity = params.getValueDouble(intensity_filt);

    cout << "Removing peaks with intensity less than " << minIntensity << "\n";
    for (int i = 0; i < allSpecs.size(); i++) {
      Spectrum newSpec = allSpecs[i];
      int idxUse = 0;
      for (int j = 0; j < allSpecs[i].size(); j++) {
        if (allSpecs[i][j][1] >= minIntensity) {
          newSpec[idxUse] = allSpecs[i][j];
          ++idxUse;
        }
      }
      newSpec.resize(idxUse);
      allSpecs[i] = newSpec;
    }
  }

  if (params.paramPresent(merge_triples) && params.getValueInt(merge_triples)
      == 1) {
    cout << "Merging triples 1 Da apart\n";

    if (!params.paramPresent(peak_tol_ppm)) {
      cerr << "ERROR: Must specify " << peak_tol_ppm << " to merge triples\n";
      return -1;
    }

    float ppmTol = params.getValueDouble(peak_tol_ppm) * PPM_FACTOR;
    float mass, massB, massA, tolB, tolA;
    int idxB, idxA;
    for (int i = 0; i < allSpecs.size(); i++) {
      list<TwoValues<float> > mergedPeaks;
      set<int> removedPeaks;
      for (int j = 0; j < allSpecs[i].size(); j++) {
        if (removedPeaks.count(j) > 0) {
          continue;
        }
        mass = allSpecs[i][j][0];
        massB = mass - 1.0;
        massA = mass + 1.0;
        idxB = allSpecs[i].findClosest(massB);
        idxA = allSpecs[i].findClosest(massA);
        tolB = ppmTol * (massB + mass);
        tolA = ppmTol * (massA + mass);
        if (removedPeaks.count(idxB) == 0 && removedPeaks.count(idxA) == 0
            && MZRange::EqualWithinRange(allSpecs[i][idxB][0], massB, tolB)
            && MZRange::EqualWithinRange(allSpecs[i][idxA][0], massA, tolA)) {
          removedPeaks.insert(idxB);
          removedPeaks.insert(j);
          removedPeaks.insert(idxA);
          mergedPeaks.push_back(TwoValues<float> (mass, allSpecs[i][j][1]
              + allSpecs[i][idxB][1] + allSpecs[i][idxA][1]));
        }
      }
      Spectrum newSpec = allSpecs[i];
      newSpec.resize(allSpecs[i].size() - ((2 * removedPeaks.size()) / 3));
      int idxUse = 0;
      for (int j = 0; j < allSpecs[i].size(); j++) {
        if (removedPeaks.count(j) > 0) {
          continue;
      ...
      for (list<TwoValues<float> >::iterator peakIt = mergedPeaks.begin(); peakIt
          != mergedPeaks.end(); peakIt++) {
        newSpec[idxUse] = *peakIt;
        ++idxUse;
      }
      sort(newSpec.peakList.begin(), newSpec.peakList.end());
      allSpecs[i] = newSpec;
    }
  }

  if (params.paramPresent(rank_filt)) {
    int maxRank = params.getValueInt(rank_filt);
    float
        windowRadius =
            (params.paramPresent(rank_filt_radius)) ? params.getValueDouble(rank_filt_radius) : AAJumps::minAAmass
                - 1.0;

    cout << "Keeping peaks in the top " << maxRank
        << " intensities over neighboring peaks in a " << windowRadius
        << " Da radius\n";
    for (int i = 0; i < allSpecs.size(); i++) {
      allSpecs[i].rankFilterPeaks(maxRank, windowRadius);
    }
  }
