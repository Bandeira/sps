...
int ClusterData::getClusterFileNames(const string & directory, const string &prefix, bool doSort)
{
  ...
  if (pdir == NULL)  {
    ...
    cerr << "ERROR! Invalid directory for MsCluster output data.\n";
    return -1;
  }
...
int ClusterData::loadMsClusterData(const string &projectDir)
{
  ...
  for(unsigned i = 0; i < fileNames.size(); ++ i) {
    ...
    aux2 += fileNames[i];
    // open the file
    currentFile.open(aux2.c_str(), ifstream::in);
    // test for errors
    if(!currentFile) {
...
//
////////////////////////////////////////////////////////////////////////////////
int ClusterData::writeCsv(vector<string> *inputFiles, vector<vector<vector<int> > > *scanNumbers, string &outFileName)
{
  // open file to write to
  ofstream file(outFileName.c_str(), ios::out);
    // if error, say so
  if(!file.is_open()) {
  ...
  for( ClusterData_t::iterator ic = data.begin() ; ic != data.end() ; ic++) {
    ...
    for(ic2 = ic->second.begin() ; ic2 != ic->second.end() ; ic2++) {
      ...
      // Write to file
      file << cluster << CSV_SEP << filename << CSV_SEP << spectrumIndex2 << CSV_SEP << scan << endl;
    }
  }
  // close the file
  file.close();
