...
///////////////////////////////////////////////////////////////////////////////
void PlotSpectrum::drawExec(void)
{
  // Check for spectrum
  if(m_spectrum == NULL)
    return;
...
///////////////////////////////////////////////////////////////////////////////
void PlotSpectrum::plotSpectrum(void)
{
...
//    cout << " loading jumps at " << aux << endl;
  ...
  for(int i = 0 ; i < m_peptide.size() ; i++) {
    ...
    if(m_annotationStyle == ANNOTATION_STYLE_INSPECT) {
      ...
      aux += m_peptide[i];
      // check trailing .*
      if(m_peptide[i][m_peptide[i].size() - 2] != '.')
        aux += ".*";
    ...
    jumps.getPRMandSRMMasses(m_peptide[i], m_prmMasses[i], m_srmMasses[i], m_peptideMass[i]);
...
void PlotSpectrum::calcDrawPosition(void)
{
  ...
  m_topMarginPosition = 1.0 - m_titlePos * 2.0 * factor;
...
void PlotSpectrum::initializeGraph(void)
{
  ...
  marginBottom  = m_rendererImage.zoom >= 1.0 ? 6.0 : 4.0;
  ...
  double msdY = m_rendererObject->calculateInterval(m_intensityLimit, 8.0, m_rendererImage.zoom);
  // Set intervals
...
void PlotSpectrum::buildAnnotationOutput(void)
{
  ...
  // Peak list
    ...
    oAnnoAux << left << m_spectrumData[i].peakMass;
    oAnnoAux.fill(' ');
    oAnnoAux.width(ident[1]);
    oAnnoAux << right << m_spectrumData[i].peakIntensity;
    // Add the annotations
    if(m_spectrumData[i].annotations.size()) {
    	for(unsigned int j = 0 ; j < m_spectrumData[i].annotations.size() ; j++) {
    	  // Add the peak data
    	  oAnnotFileContent << oAnnoAux.str();
    	  // Add the specific annotation data
        oAnnotFileContent.fill(' ');
        oAnnotFileContent.width(ident[2]);
        oAnnotFileContent << right << m_spectrumData[i].annotations[j].annotation;
        oAnnotFileContent.fill(' ');
        oAnnotFileContent.width(ident[3]);
        oAnnotFileContent << right << parseInt(m_spectrumData[i].annotations[j].seriesIndex);
        oAnnotFileContent.fill(' ');
        oAnnotFileContent.width(ident[4]);
        oAnnotFileContent << right << parseInt(m_spectrumData[i].annotations[j].charge);
        oAnnotFileContent.fill(' ');
        oAnnotFileContent.width(ident[5]);
        oAnnotFileContent << right << parseInt(m_spectrumData[i].annotations[j].peptideIndex);
        oAnnotFileContent.fill(' ');
        oAnnotFileContent.width(ident[6]);
        oAnnotFileContent << right << parseFloat(m_spectrumData[i].annotations[j].prob, 4);
        oAnnotFileContent << endl;
      }
    } else {
  	  // Add the peak data
...
{
  fstream annotFile(m_annotationsOutputFile.c_str(), fstream::out | fstream::trunc);
