...
void ContigSpectrum::buildVector(void)
{
  ...
  for(int i = 0 ; i < data.size() ; i++)
    for(int j = 0 ; j < data[i].first.size() ; j++)
      if(data[i].first[j] == m_spectrumIndex) {
        abMassItem item;
        item.mass = data[i].second[j];
        item.idxInContig = i;
        m_abMasses.push_back(item);
        //m_abMasses.push_back(data[i].second[j]);
      }
}
///////////////////////////////////////////////////////////////////////////////
  ...
  for(register int j = 0 ; j < size ; j++) {
    ...
    if((index != -1) && (index < m_deNovoIntervals->size())) {
      if(firstFound) {
        ...
        if( fabs( diff ) < m_peakMassTol )
          addGreenArrow(first * factor, 0.0, second * factor, 0.0);
        else {
          addRedArrow(first * factor, 0.0, second * factor, 0.0);
