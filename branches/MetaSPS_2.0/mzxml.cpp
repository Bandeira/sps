...
void mzxmlSAX2Handler::startElement(const   XMLCh* const    uri,
    ...
    if(!strcmp(name,"scan")) {
    	...
    	if(numSpecs>=specs->size()) { cerr<<"ERROR: Can't read more scans than indicated in msRun.scanCount! " << numSpecs << " : " << specs->size() << "\n"; exit(-1); }
    ...
    if(!strcmp(name,"precursorMz")) {
		...
		if(str and strcmp(str,"ETD")==0) (*specs)[curSpec.front()].msFragType = Spectrum::FragType_ETD;
    ...
    if(!strcmp(name,"peaks")) {
    	...
    	if((str?atoi(str):0)!=32) { cerr<<"ERROR: "<<atoi(str)<<"-bit precision not supported, only 32-bit.\n"; exit(-1); }
    	XMLString::release(&str);    XMLString::release(&xmlStr);
    }
...
void mzxmlSAX2Handler::characters(const XMLCh* const chars, const XMLSize_t length) {
    ...
    if(state == STATE_PEAKS and spec->size()>0) {
		...
		if(((unsigned int)bytesRead)!=spec->size()*8) {
		  cerr<<"ERROR: Could not decode peak list for scan "<<spec->scan<<" - needed "<<spec->size() * 8 + 8<<" bytes, got "<<bytesRead<<" bytes ("<<spec->size()<<" peaks).\n";
		  exit(-1);
		}
		decoded = b64.getOutputBuffer();
		if(decoded==(unsigned char *)0) { cerr<<"ERROR: Could not decode peak list for scan "<<spec->scan<<".\n"; exit(-1); }
		...
		for(unsigned int peakIdx=0; peakIdx<spec->size(); peakIdx++) {
			for(unsigned int colIdx=0; colIdx<2; colIdx++, valIdx++) {
			    ...
			    } catch(...) { cerr<<"FNB2: "<<peakIdx<<"/"<<spec->size()<<", col "<<colIdx<<"\n"; exit(-1); }
			}
		}
