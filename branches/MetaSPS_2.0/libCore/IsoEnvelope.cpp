...
float IsoEnvelope::ScoreEnvelope(float monoisotopicMass,
		vector<float> &massEnvelope, bool strictMode) {
...
void IsoEnvelope::ExtractEnvelope(float monoisotopicMass,
		unsigned short charge, Spectrum &spec, float peakWidth,
		vector<float> &intensities, bool strictMode) {
	if (strictMode)
...
void IsoEnvelope::ExtractEnvelope(float monoisotopicMass,
		unsigned short charge, Spectrum &spec, float peakWidth,
		set<int>& ignorePeaks, vector<float> &intensities, vector<list<
				unsigned int> >& indicesUsed, bool usePPM, bool strictMode) {
	float peakTol = (usePPM) ? (monoisotopicMass * peakWidth * PPM_FACTOR)
			: peakWidth;
	indicesUsed.clear();
...
void IsoEnvelope::normalize(vector<float> &values, bool avoidZeros,
