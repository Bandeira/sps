  ...
  // -------------------------------------------------------------------------
   void PeptideSpectrumMatchSet::removePsmSetItem(psmPtr item)
   {
 	  for(int i = 0; i < m_psmSet.size(); i++) {
 		  ...
 		  std::tr1::shared_ptr<PeptideSpectrumMatch> p = std::tr1::dynamic_pointer_cast<PeptideSpectrumMatch>(m_psmSet[i]); //I AM A PSMNETWORK POINTER
...
// 	  		PeptideSpectrumMatch * p = (PeptideSpectrumMatch *)m_psmSet[i].get();
  ...
  bool PeptideSpectrumMatchSet::loadFromFile(const char * filename)
  {
    ...
    for (int i = 0; i < lines.size(); i++)
    {
      ...
      if (!sscanf(lines[i][13].c_str(), "%f", &(currMatch->m_strict_envelope_score)))
      {
        ERROR_MSG("Unable to get strict envelope score!");
        return false;
      }
      if (!sscanf(lines[i][14].c_str(), "%f", &(currMatch->m_unstrict_envelope_score)))
      {
    ...
    for (int i = 0; i < lines.size(); i++) {
      ...
      if (!parseLine(currMatch,
    	...
    	if (updateScanIdxs && haveNumConsec && numConsecutive > 1) {
    		...
    		idx = (zeroIndexed) ? idx / numConsecutive : ((idx - 1) / numConsecutive) + 1;
