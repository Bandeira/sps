...
int main(int argc, char **argv){
	...
	if(params.paramPresent("INPUT_HOMOLOGIES")) {
		...
		if(not Load_clustalw_multiple(params.getValue("INPUT_HOMOLOGIES"), db, matchedProts, matchedAAs)) return -1;
	...
	//
    vector<Results_ASP> pairsASP(0);
    vector<Results_PA> pairsPA(0);
	...
	if(specNets) {
		...
		SpecSet snMatched;
	    if(snMatched.LoadSpecSet_pklbin(params.getValue("INPUT_SPECNETS_MATCHED"))<=0)
	...
	if(params.paramPresent("INPUT_SPECNETS_ALIGNS_ALL")) {
		vector<Results_ASP> pairsASP_all(0);
	...
	for(cIdx=0; cIdx<components.size(); cIdx++) {
		...
		vector<Results_PA> &curAligns     = components.cAlignsPA[cIdx];
		vector<int> &curAlignsIdx         = components.cAlignsPA_idx[cIdx];
		vector<Results_ASP> &curAlignsASP = components.cAlignsASP[cIdx];
...
//		vSet.addEndpointEdges(components.cAlignsASP[cIdx],matches,modPos,jumps,peakTol);
		...
		vSet.buildGraph(abg,jumps,peakTol,vSet_index,edgeScoreType);
...
//    cout << "Done searching. Beginning output... "; cout.flush();
    ...
    char sep=';';
    ofstream output;   output.open(params.getValue("OUTPUT_CSV"));
