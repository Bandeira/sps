...
struct SimpleEdge {
	 ...
	 */
	SimpleEdge(float mass, float score, int in_specIdx, int peakIdx1,
			int peakIdx2) {
	 ...
	 *@return Reference to self
	 */
	SimpleEdge &set(float mass, float score, int in_specIdx, int peakIdx1,
			int peakIdx2) {
		edgeMass = mass;
		edgeScore = score;
		specIdx = in_specIdx;
...
public:
	 ...
	 */
	void addEdge(float edgeMass, float edgeScore, int specIdx, int peakIdx1,
			int peakIdx2);
	 ...
	 */
	bool replaceEdge(int toSpecIdx, int toPeakIdx, int newPeakIdx,
			float addEdgeMass, float addEdgeScore);
};
...
class VertexSet {
	 ...
	 */
	int getFreeVertex() {
		if (firstFreeVertex == -1) { // Current structure is full, need to allocate more space
	 ...
	 *@param vertexAdjChanged
	 */
	void scv_useSplitEdge(int cvIdx, int direction,
			vector<TwoValues<bool> > &vertexAdjChanged);
...
public:
	 ...
	 */
	void addGlues(int specIdx1, int specIdx2, vector<TwoValues<int> > &matches,
			vector<MSGraph> *spectrumGraphs = 0, bool addMissingEdges = false);
	 ...
	 *@param peakTol Allowed tolerance for mass errors (in Da)
	 */
	void addEndpointEdges(vector<Results_ASP> &aligns, vector<vector<TwoValues<int> > > &matches,
			vector<float> &modPos, AAJumps &jumps, float peakTol);
	 ...
	 *                      least one peak in an ABruijn vertex.
	 */
	void splitComposite(vector<MSGraph> &spectrumGraphs, float peakTol,
			vector<bool> *usedSpectra = 0);
	 ...
	 *@param labels
	 */
	void buildGraph(MSGraph &g, AAJumps &jumps, float peakTol,
			vector<int> &vSet_index, short edgeScoreType = EST_EDGE_SCORES,
			vector<SpectrumPeakLabels> *labels = 0);
	 ...
	 *@return
	 */
	void enumerateHeaviestPath(MSGraph &g, vector<int> &vSet_index,
			SpecSet &enumSpecs, vector<int> &enumSpecsIdx);
 ...
 *@param abVertices
 */
void Save_abinfo(const char *filename, SpecSet &specSet,
		vector<list<int> > &cSpectra, vector<bool> &specFlipped, vector<vector<
				list<TwoValues<int> > > > &abVertices);
