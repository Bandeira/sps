...
//	float minRatio = params.paramPresent("MIN_RATIO")?(float) params.getValueDouble("MIN_RATIO"):0;
	...
	vector<Results_ASP> aligns;   unsigned int idxPair;
	if(alignPA) {
		vector<Results_PA> alignsPA;
		if (!Load_results_bin(alignsFN, alignsPA)) { cerr << "Error reading "<<alignsFN<<"!\n"; return -1; }
	...
	} else if (!Load_results_bin(alignsFN, aligns)) { cerr << "Error reading "<<alignsFN<<"!\n"; return -1; }
    ...
    if(params.paramPresent("INPUT_SPECS_PAIRS")) {
    	if(not resultSpecsSet.LoadSpecSet_pklbin(params.getValue("INPUT_SPECS_PAIRS"))
...
//    		if(abs(shift)<=2*pmTol and abs(specSet[spec2].parentMass+shift-specSet[spec1].parentMass)<=2*pmTol) continue; // Same-peptide pairs can't be used to compute spectral stars
    		if(not alignPA or abs(shift)<=2*pmTol or abs(specSet[spec2].parentMass+shift-specSet[spec1].parentMass)<=2*pmTol) {  // Common start or common end
    			...
    			modPositions[idxPair]=dekel_align(&specSet[spec1],&specSet[spec2],peakTol,&resultSpecsSet[idxResultSpecs],&resultSpecsSet[idxResultSpecs+1],maxAAjump,penalty_sameVert,penalty_ptm,debug);
#else
...
//cerr<<"peakTol = "<<peakTol<<", maxAAjump = "<<maxAAjump<<", penalty_sameVert = "<<penalty_sameVert<<", curPenalty = "<<curPenalty<<"\n";
    			modPositions[idxPair]=dekel_align(&specSet[spec1],&specSet[spec2],peakTol,&resultSpecsSet[idxResultSpecs],&resultSpecsSet[idxResultSpecs+1],maxAAjump,penalty_sameVert,curPenalty);
...
return 0;
	...
	resultSpecsSet.LoadSpecSet_pklbin("pairs.pklbin");
	...
	if(params.paramPresent("MATCH_MSMS_INTENSITY") and (params.paramPresent("MATCH_MSMS_RATIOS") or params.paramPresent("MIN_MATCH_MSMS"))) {
		...
		if(not specSet.LoadSpecSet_pklbin(params.getValue("MATCH_MSMS_INTENSITY")))
