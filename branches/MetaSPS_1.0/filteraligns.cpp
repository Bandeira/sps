...
int main(int argc, char **argv){
	...
	if(params.paramPresent("INPUT_MEANS_MULTIPLE") and params.paramPresent("INPUT_VARS_MULTIPLE")) {
		...
		if(aligns.size()>0) {
			...
			if(params.paramPresent("MIN_PVALUE")) { // and params.paramPresent("OUTPUT_ALIGNS")) {
				idxLast=0;   idxKept.resize(aligns.size());   float minPValue = params.getValueDouble("MIN_PVALUE");
				for(idxPair=0; idxPair<aligns.size(); idxPair++)
