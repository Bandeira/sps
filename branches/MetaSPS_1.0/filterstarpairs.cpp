...
template<class T> int FilterPairs(SpecSet &specSet, vector<T> &aligns,
...
int main(int argc, char **argv){
	...
	if (alignPA) {
		if (Load_results_bin(alignsFN, alignsPA)==0) { cerr << "Error reading "<<alignsFN<<"!\n"; return -1; }
	    else  cout << "Loading aligns complete. Number of input pairs: " << alignsPA.size() << "\n";
	} else {
		if (Load_results_bin(alignsFN, alignsASP)==0) { cerr << "Error reading "<<alignsFN<<"!\n"; return -1; }
...
template<class T> int FilterPairs(SpecSet &specSet, vector<T> &aligns,
	...
	vector<vector<T> > cAligns;    vector<vector<int> > cAligns_idx;  // To allow going back from components to original order
	...
	for(unsigned int cIdx=0; cIdx<cAligns.size(); cIdx++) {
		SplitPairs(specSet, cAligns[cIdx], peakTol, pmTol, maxAAjump, maxModMass, penalty_sameVert, penalty_ptm, matches, specFlipped, modPos, 0, 0, false, &cRatios);
