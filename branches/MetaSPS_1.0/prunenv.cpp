...
int main(int argc, char *argv[], char **envp ) {
	...
	if (params.paramPresent(pep_file)) {
		...
		SpecSet bestOut;
		int charge_c = 2;
		cout << "Threshold - Min ParentCharge - #B Ions Matched - #Y Ions Matched - Total Ions Matched - Number of Peaks in Output\n";
		//for (float thresh = minThresh; thresh <= maxThresh; thresh += stepThresh) {
		for (float thresh = threshold; thresh <= threshold + 0.1; thresh += 1.0) {
			penv.transferAllSpecs(InputParams::PeakTol, thresh, false, usePPM);
			TwoValues<int> count = countBY(penv.prunedSpecs, params.getValue(pep_file), InputParams::PeakTol);
			int tot = count[0] + count[1];
			int peakcount = 0;
			for (int i = 0; i < penv.prunedSpecs.size(); i++) peakcount += penv.prunedSpecs[i].size();
			cout << thresh << " - 0 - "<< count[0] << " - " << count[1] << " - " << tot << " - " << peakcount << "\n";
			SpecSet highChargeSpecs = penv.prunedSpecs;
			peakcount = 0;
			for (int i = 0; i < highChargeSpecs.size(); i++) {
				if (highChargeSpecs[i].parentCharge != charge_c) highChargeSpecs[i].resize(0);
				peakcount += highChargeSpecs[i].size();
			}
			count = countBY(highChargeSpecs, params.getValue(pep_file), InputParams::PeakTol);
			tot = count[0] + count[1];
			cout << thresh << " - " << charge_c << " - "<< count[0] << " - " << count[1] << " - " << tot << " - " << peakcount << "\n";
			if (tot > maxPeaks) {
				maxPeaks = tot; bestThresh = thresh; bestOut = penv.prunedSpecs;
			}
		}
