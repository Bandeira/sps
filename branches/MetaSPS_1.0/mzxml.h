...
template <typename T>
  unsigned int LoadMzxml(const T & xmlFile, SpecSet &specs, vector<short> *msLevels = 0, short minMsLevel = 2);

unsigned int LoadMzxml(const char *filename, SpecSet &specs, const string & sscan, vector<short> *msLevels = 0, short minMsLevel = 2);
...
public:
	 ...
	 *@param in_minMsLevel
	 */
	mzxmlSAX2Handler(SpecSet &in_specs, vector<short> *in_msLevels = 0,
