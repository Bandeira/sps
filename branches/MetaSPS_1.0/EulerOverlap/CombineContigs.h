...
private:
  ...
  list<Results_OCC> contig_alignments;
  list<Results_OCC> connector_alignments;
  SpecSet overlaps;
  DB_fasta fasta;
  ...
  void
      getAlignmentGraph(map<unsigned int,
                            map<unsigned int, list<unsigned int> > >* graphCont,
  ...
  TwoValues<float>
      mergeContigs(int idx, map<int, map<int, int> >& _graph, vector<vector<
          float> >& _edges, vector<set<int> >& _components, vector<list<
          TwoValues<int> > >& _root_alignments, Spectrum& putSpec, pair<pair<
          vector<int> , vector<int> > , vector<pair<vector<int> ,
          vector<double> > > >& putAb, bool debug);
