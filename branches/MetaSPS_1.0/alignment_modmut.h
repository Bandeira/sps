...
public:
	 ...
	 */
	void set(float newScore, int newPredMods, int newPredSpecIdx,
			int newPredDbSpecIdx, int newPredMPeaks=-1, AMM_match *newPredExonMatch=0) {
		score = newScore;
	 ...
	 *@param newPredExonMatch
	 */
	void replaceWithMax(float newScore, int newPredMods, int newPredSpecIdx,
			int newPredDbSpecIdx, int newPredMPeaks=-1, AMM_match *newPredExonMatch=0) {
		if (newScore >= score)
			set(newScore, newPredMods, newPredSpecIdx, newPredDbSpecIdx, newPredMPeaks, newPredExonMatch);
	}
	...
	void replaceWithMax(AMM_peak_match &other) {
		if (other.score >= score)
			set(other.score, other.predMods, other.predSpecIdx,
					other.predDbSpecIdx, other.predMPeaks, other.predExonMatch);
	}
};
...
public:
	 ...
	 *@param numMods
	 */
	AMM_match *output_csv(ostream &output, DB_fasta &db, float tolerance,
			char separator, short outputPRMs = 0, int specIdx = -1,
			int numMods = -1);
};
...
public:
	 ...
	 */
	void output_csv(ostream &output, DB_fasta &db, float tolerance,
			char separator = ';', short outputPRMs = 0, int specIdx = -1);
};

class AMM_match_simple { // Simplified version of AMM_match above
public:
...
public:
	 ...
	 */
	void GenerateGlues(vector<Results_PA> &pairsPA,
	 ...
	 *
	 */
	void MergeIntoReference(int refIdx, int otherIdx,
	 ...
	 *@param outputPRMs
	 */
	void output_csv(unsigned int specIdx, ostream &output, float tolerance,
			char separator, short outputPRMs = 0) { }
	 ...
	 */
	bool output_midx(const char *filename) { return false; }
	 ...
	 *@return True if the file saved correctly, false otherwise
	 */
	bool output_mp(const char *filename) { return false; }
};
...
void scoreOverlapAMM(Spectrum &spec, Spectrum &dbSpec, short maxNumMods,
		AMM_match_spec &topKmatches, AMM_match &curMatch, float pmTolerance,
		float tolerance, float minSpecDist, float maxDbSpecMod,
		float minDbSpecMod, bool enforceEndpeaks = false);
...
void scoreOverlapAMMme(Spectrum &spec, Spectrum &dbSpec, short maxNumMods, int minMatchedPeaks,
		...
		float pmTolerance, float tolerance, float minSpecDist, float maxDbSpecMod,
		float minDbSpecMod, bool enforceEndpeaks = false);
