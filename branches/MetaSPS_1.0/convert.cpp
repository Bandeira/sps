...
int main(int argc, char **argv){
	...
	if(argc<3) {
		cerr<<"Syntax: "<<argv[0]<<" <file_type> <input_file_name> <output_file_name_prefix>\n";
		cerr<<"        file_type: mgf, ms2, pkl (multiple spectra per file), prms (pepnovo_prm output format)\n";
		return -1;
	}
	for(unsigned int index=0; argv[1][index]!=(char)0; index++) argv[1][index]=tolower(argv[1][index]);

	char *outFN; unsigned int fnPos;
	if(argc<=3) {
		outFN = (char *)malloc(strlen(argv[2])+1);   strcpy(outFN,argv[2]);
		for(fnPos=strlen(outFN)-1; fnPos>0 and outFN[fnPos]!='.'; fnPos--);
		if(fnPos>0)	outFN[fnPos] = 0;
	} else {
		outFN = (char *)malloc(strlen(argv[3])+1);   strcpy(outFN,argv[3]);
	}

	bool loadOk = false;
	if(strcmp(argv[1],"mgf")==0) {
		if(specs.LoadSpecSet_mgf(argv[2])<=0) { cerr<<"ERROR loading "<<argv[2]<<"!\n"; return -1; }
		specsInfo.resize(specs.size()); for(unsigned int i=0; i<specsInfo.size(); i++) { specsInfo[i][0]=specs[i].scan; specsInfo[i][1]=2; }
		loadOk=true;
	}
	...
	if(strcmp(argv[1],"prmsv3")==0) {
		if(specs.LoadSpecSet_prmsv3(argv[2])<=0) { cerr<<"ERROR loading "<<argv[2]<<"!\n"; return -1; }
		specsInfo.resize(specs.size()); for(unsigned int i=0; i<specsInfo.size(); i++) { specsInfo[i][0]=specs[i].scan; specsInfo[i][1]=2; }
		loadOk=true;
	}
	...
	if(strcmp(argv[1],"mzxml")==0) {
		if( LoadMzxml(argv[2], specs, &msLevel, 2) == 0 ) { cerr<<"ERROR loading "<<argv[2]<<"!\n"; return -1; }
		specsInfo.resize(specs.size()); for(unsigned int i=0; i<specsInfo.size(); i++) { specsInfo[i][0]=specs[i].scan; specsInfo[i][1]=msLevel[i]; }
		loadOk=true;
...
*/
	...
	if(loadOk) {
		...
		specs.SaveSpecSet_pklbin(outName.c_str());
