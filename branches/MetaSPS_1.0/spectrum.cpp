                                    ...
                                    map<string, unsigned short>& versions)
{
  ...
  {
    float rt;
    count = fread(&rt, sizeof(float), 1, fp);
    if (count == 0)
    {
      return false;
    }
...
//
// -------------------------------------------------------------------------
short SpecSet::SaveSpecSet_pklbin(const char *filename, const char *binFilename)
{
  if (binFilename)
  {
    cerr << "Bin filename [" << binFilename
        << "] passed to SaveSpecSet_pklbin\n";
    cerr << "Bin file [" << binFilename << "] will NOT be written.\n";
  }
                           ...
                           char subversion)
{
  ...
  if (!oldVersion)
  {
    ...
    {
      cerr << "Not enough memory for " << numSpecs << " scan numbers\n";
      fclose(fp);
      return false;
    }
    ...
    if (msLevels == (short *)0)
    {
      cerr << "Not enough memory for " << numSpecs << " MS Levels.\n";
      fclose(fp);
      return false;
    }
  ...
  {
    cerr << "Not enough memory for " << numSpecs << " spectra\n";
    fclose(fp);
    return false;
  }
  ...
  if (data == (float *)0)
  {
    cerr << "Not enough memory for " << dataSize << " floats\n";
    fclose(fp);
    return false;
  }
  ...
  for (i = 0; i < numSpecs; i++)
  {
    ...
    if (count != numValues)
    {
      cerr << "Not enough memory for " << numSpecs << " numValues\n";
      free(numPeaks);
      free(data);
      fclose(fp);
      return false;
    ...
    if (specs[i].parentCharge > 0)
    {
      specs[i].parentMZ = (specs[i].parentMass + ((specs[i].parentCharge - 1.0)
          * AAJumps::massHion)) / specs[i].parentCharge;
    }
    else
    {
...
bool SpecSet::loadPklBin_2(FILE* fp, int numSpecs, char subversion)
{
  ...
  {
    cerr << "Not enough memory for " << numSpecs << " MS frag types.\n";
    goto load_fail;
  }
  ...
  for (unsigned int i = 0; i < numSpecs; i++)
  {
    ...
    {
      cerr << "Found unsupported fragmentation ID " << fragTypes[i] << "\n";
      free(fragTypes);
      goto load_fail;
    }
  }
  ...
  if (PMs == (float *)0)
  {
    cerr << "Not enough memory for " << numSpecs << " parent masses.\n";
    goto load_fail;
  }
  ...
  {
    cerr << "Not enough memory for " << numSpecs << " MS Charges.\n";
    goto load_fail;
  }
  ...
  if (PMTols == (float *)0)
  {
    cerr << "Not enough memory for " << numSpecs
...
bool SpecSet::loadPklBin_3(FILE* fp, int numSpecs, char subversion)
{
  ...
  {
    cerr << "Error reading version info\n";
    fclose(fp);
    return false;
  }
  ...
  for (unsigned int i = 0; i < numSpecs; i++)
  {
    ...
    {
      cerr << "Failed to load spectrum " << i << "\n";
      fclose(fp);
      return false;
    }
  }
                        ...
                        const char * peaksFilename /* = 0x0 */)
{
  ...
  if (count != 1)
  {
    cerr << "Reading number of spectra from " << filename << "\n";
    fclose(fp);
    return 0;
  }
  ...
  if (numSpecs != 0)
  {
    cerr << "PKLBIN file non-zero first value.\n";
    cerr << "Assuming first value is number of spectra [" << numSpecs << "]\n";
  }
  else
  {
    oldVersion = false;
    ...
    if (count != 1)
    {
      cerr << "Reading version number from " << filename << "\n";
      fclose(fp);
      return 0;
    }
    subversion = 0;
    ...
    if (count != 1)
    {
      cerr << "Reading sub-version number from " << filename << "\n";
      fclose(fp);
      return 0;
    }
    count = fread(&numSpecs, sizeof(unsigned int), 1, fp); // Number of spectra in the file
  ...
  {
    cerr << "Found unsupported version " << (int)version << "\n";
    cerr << "Reading " << filename << "\n";
    return 0;
  }
  /*
  ...
  if (peaksFilename)
  {
    /*
     ...
     cerr << "Problem loading matched peaks from [" << peaksFilename
     << "]\n";
     return 0;
     }
     ...
     specs[i].psmList.front()->m_matchedPeaks.resize(
     tempMatchedPeaks[i].size());
     for (int j = 0; j < tempMatchedPeaks[i].size(); j++) {
     specs[i].psmList.front()->m_matchedPeaks[j].set(
     tempMatchedPeaks[i][j][0], tempMatchedPeaks[i][j][1]);
     }
     }
     DEBUG_TRACE;
     */
                                         ...
                                         bool countSpectraOnly)
{
  ...
  {
    cout << "countSpectraOnly is no longer supported\n";
  }
