...
InspectResultsLine* InspectResultsSet::getScan(int scan)
{
  map<int, InspectResultsLine>::iterator it;
  it = results.find(scan);

  if (it == results.end())
  {
    return (InspectResultsLine*)NULL;
  }
  else
  {
    return &(it->second);
...
{
  //open Inspect file
  ifstream inspect_results_handle(inspect_results_file);
  ...
  int count_it = 0;
  while (!inspect_results_handle.eof())
  {
	  count_it ++;
    ...
    {
      inspect_results_handle.getline(line_buff, 1024);
      if (inspect_results_handle.gcount() == 0)
        break;
                                         ...
                                         const vector<string>& field_names)
{
  ...
  for (int i=0; i < field_names.size(); i++)
  {
    ...
    {
      if (sscanf(fields[i].c_str(), "%d", &Charge) != 1 || Charge < 0 || Charge
          > 20)
      {
    ...
    {
      if (sscanf(fields[i].c_str(), "%d", &Length) != 1 || Length < 1 || Length
          > 999999999)
      {
    ...
    {
      if (sscanf(fields[i].c_str(), "%f", &TotalPRMScore) != 1 || TotalPRMScore
          < -999999999 || TotalPRMScore > 999999999)
      {
