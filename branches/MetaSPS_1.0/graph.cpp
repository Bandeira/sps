...
*/
				if(pathPredIdx>0 and pathPredIdx%1000==0) cout<<"Generated vSets["<<pathPredIdx<<"]...\n";
				pathPredIdx++;
			}
	}
}
...
void MSGraph::output_graphviz(char *filename){
	char *sBuf = (char *)malloc(1024);   if(sBuf==(char *)0) { cerr<<"ERROR: Out of memory (MSGraph::output_graphviz)\n"; return; }
	ofstream out(filename);
	if(!out) { cerr<<"Error opening "<<filename<<"!\n"; return; }
	out << "digraph G {\n";
