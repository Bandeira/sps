...
//	paramStrs[3] = "INPUT_MATCHES2";
	...
	vector<list<int> > simpleProteinHits, curProteinHits;
...
*/
	...
	if(blr.Load(params.getValue("INPUT_MATCHES_LIST"))<=0) { cerr<<"ERROR reading "<<params.getValue("INPUT_MATCHES_LIST")<<"!\n"; return -1; }
	if(Load_binListArray<int,list<int>,list<int>::iterator>(blr.getline(0), simpleProteinHits)<0) { cerr<<"Error reading "<<blr.getline(0)<<"!\n"; return -1; }
	for(lineIdx=1; lineIdx<blr.size(); lineIdx++)
		if (strlen(blr.getline(lineIdx))>0) {
			if(Load_binListArray<int,list<int>,list<int>::iterator>(blr.getline(lineIdx), curProteinHits)<0) { cerr<<"Error reading "<<blr.getline(lineIdx)<<"!\n"; return -1; }
			pivot1=simpleProteinHits.size();   simpleProteinHits.resize(simpleProteinHits.size()+curProteinHits.size());
			for(pivot2=0; pivot2<curProteinHits.size(); pivot2++, pivot1++)
				simpleProteinHits[pivot1].swap(curProteinHits[pivot2]);
		}
	...
	for(pivot1=0; pivot1<simpleProteinHits.size(); pivot1++) 
		if(not simpleProteinHits[pivot1].empty()) matchedProts[simpleProteinHits[pivot1].front()]=true;
	
	FILE *protsFID = (FILE *)fopen(params.getValue("OUTPUT_FASTA"), "w");
