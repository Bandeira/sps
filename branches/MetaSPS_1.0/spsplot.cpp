...
//      if (spattern.substr(nslash + 1, nstar[0] - nslash - 1) + ssni + spattern.substr(nstar[1] + 1) == pe->d_name)
  ...
  if(sort)
    std::sort(centry.begin(), centry.end());
  ...
  if (! m.sopt[m.ep].empty())
    for (unsigned i = 0; i < 2; ++ i)
    {
      ...
      if (cparams.paramPresent("FILE_CLUSTERMS"))
        m.copt[m.eclusterms] = entries(cparams.getValue("FILE_CLUSTERMS"), false);
      if (cparams.paramPresent("FILE_CLUSTERSCAN"))
        m.copt[m.eclusterscan] = entries(cparams.getValue("FILE_CLUSTERSCAN"), false);
...
#if defined(__linux__)
    const unsigned ncpu = get_nprocs() * 2;
#endif
...
int SpsPlot::genhtml()
{
  ...
  for (abinfo_t::iterator i = cdata.cabinfo.begin(), j = i; i != cdata.cabinfo.end(); i = j)
  {
    ...
    shtml << "    <h3 align=center><img src=\"../../images/pagelogo_ccms_left.jpg\" border=\"0\"/></h3>\n";
    shtml << "  </div>\n";
    shtml << "<body>\n";
  ...
  if (! sopt[efasta].empty())
  {
    ...
    for (map< int, set<unsigned> >::iterator ip = cfastamap.begin(); ip != cfastamap.end(); ++ ip)
    {
      ...
      shtml << "    </tr>\n";
      shtml << "  </table>\n";
      shtml << "  <a href=\"protein_details." << ssfilename << ".html\">Details</a>\n";
      ...
      // details
      {
        ofstream shtml((sopt[eoutdir] + "/protein_details." + ssfilename + ".html").c_str());
...
{
#if ! defined(SPECPLOT)
  sout << "spsplot 1.0-" << XSTR(SPS_VERSION) << "\n\n";
#else
  sout << "specplot 1.0-" << XSTR(SPS_VERSION) << "\n\n";
#endif

  sout << "Copyright (c) 2009 The Regents of the University of California\n";
  sout << "All Rights Reserved\n";
  sout << '\n';
  sout << "Written by Phil Bouchard <pbouchard@cs.ucsd.edu>\n";
...
{
  cerr << "spsplot: invalid or missing option " << a << endl;

  cerr << "Try `spsplot --help' for more information." << endl;
