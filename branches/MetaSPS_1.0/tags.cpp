...
unsigned int ExtractTags(Spectrum &spec, list<Tag> &tags, float peakTol,
                           unsigned int tagLen, unsigned int maxNumJumps, unsigned int maxNumTags) {
	...
	if(maxNumTags>0 and tags.size()>maxNumTags) {
		for(unsigned int count=0;count<maxNumTags;count++) iter++;
...
unsigned int ExtractTags(char *sequence, vector<Tag> &tags, unsigned int tagLen) {
	tags.resize(0); if(!sequence or strlen(sequence)<tagLen) return 0;
...
*/
void MatchTagsToSpectra(SpecSet &specs, list<Tag> &tags, float peakTol, float maxCharge,
                     float missingPeakPenalty, float noisePeakPenalty, unsigned int maxNumMissedPeaks, 
                     vector<list<unsigned int> > &matchedTagsIdx, vector<list<float> > &matchedTagsScores, vector<list<unsigned int> > &matchedTagsPos) {
	matchedTagsIdx.resize(specs.size());   matchedTagsScores.resize(specs.size());   matchedTagsPos.resize(specs.size());
	for(unsigned int i=0; i<specs.size(); i++) { matchedTagsIdx[i].clear(); matchedTagsScores[i].clear(); matchedTagsPos[i].clear(); } 
	if(specs.size()==0 or tags.size()==0) return;
...
//	for(tagsIdx=0; tagsIdx<tags.size(); tagsIdx++) getMasses(tags[tagsIdx].sequence, tagMasses[tagsIdx]);
	...
	for(tagsIdx=0; tagsIter!=tags.end(); tagsIter++, tagsIdx++) getMasses(tagsIter->sequence, tagMasses[tagsIdx]);
	
	for(specIdx=0; specIdx<specs.size(); specIdx++)
		for(float charge=1.0; charge<maxCharge+0.0001; charge=(float)round(charge+1.0))
			for(tagsIdx=0; tagsIdx<tagMasses.size(); tagsIdx++) {
//			for(tagsIdx=6; tagsIdx<7; tagsIdx++) {
				curTagMasses.resize(tagMasses[tagsIdx].size()+1);   curTagMasses[0]=0;
				for(tagMassIdx=0; tagMassIdx<tagMasses[tagsIdx].size(); tagMassIdx++) curTagMasses[tagMassIdx+1]=tagMasses[tagsIdx][tagMassIdx]/charge;
				
				for(peakIdx=0; peakIdx<specs[specIdx].size(); peakIdx++) {
					vector<int> matchesIdx;   unsigned int pivot, lastMatchedPeak = peakIdx, missedPeaks = 0;
					cumTagMass = specs[specIdx][peakIdx][0];   tagMatchScore = 0;
					for(tagMassIdx=0; tagMassIdx<curTagMasses.size() and missedPeaks<=maxNumMissedPeaks; tagMassIdx++) {
						cumTagMass += curTagMasses[tagMassIdx];

						// Look for a matching tag peak
						if(specs[specIdx].findMatches(cumTagMass, peakTol, matchesIdx, lastMatchedPeak)) {
							for(pivot=0; pivot<matchesIdx.size(); pivot++) tagMatchScore+=specs[specIdx][matchesIdx[pivot]][1];
							if(tagMassIdx==0) { lastMatchedPeak = matchesIdx[matchesIdx.size()-1]; continue; }
						} else { tagMatchScore+=missingPeakPenalty; missedPeaks++; 
//cerr<<"["<<specIdx<<","<<peakIdx<<","<<charge<<"]: missed cumTagMass = "<<cumTagMass<<", tagMassIdx = "<<tagMassIdx<<endl; cerr.flush();
						}
						
						// Penalize for noise peaks
						for(pivot=lastMatchedPeak+1; pivot<specs[specIdx].size() and specs[specIdx][pivot][0]<cumTagMass-peakTol-0.0001; pivot++) 
							tagMatchScore += (noisePeakPenalty * specs[specIdx][pivot][1]);
						if(matchesIdx.size()) lastMatchedPeak = matchesIdx[matchesIdx.size()-1];
						else {
							for(lastMatchedPeak=pivot-1; lastMatchedPeak<specs[specIdx].size() and specs[specIdx][lastMatchedPeak][0]<cumTagMass+peakTol+0.0001; lastMatchedPeak++);
							lastMatchedPeak--;
						}
					}
