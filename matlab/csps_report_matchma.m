function [report, matchScores, results] = csps_report_matchma(specs, matchmaData, vSets, addModMasses, peakTol, numModStates, htmlFN)
% function [report, matchScores, results] = csps_report_matchma(specs, matchmaData, vSets, addModMasses, peakTol, numModStates, htmlFN)
%
...
%  results = as returned by load_matchma
%

% NB-10/02/18 global AAmasses AAletters;
...
% NB-10/02/18    denovoPep = denovoPeptide(specs{i,idxSpec},1,AAmasses,AAletters,min(peakTol,0.47));  %  0.47 max peakTol avoids Q/K/E confusions
    denovoPep = '';
...
% NB-10/02/18            bypercs = getBYPercs(curSpec,'prm',peakTol,'',[],addModMasses,0,0);
...
%
%  Converts a matchma peptide match string to a peptide annotation string (i.e. suitable for getsnr2)
%  {} -> [], (ABC) -> ABC, [A,B,C] -> [A]
