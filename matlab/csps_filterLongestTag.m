function tags = csps_filterLongestTag(report, minTagSize, maxJump, maxMod)
% function tags = csps_filterLongestTag(report, minTagSize, maxJump, maxMod)
%
...
function [tag, massPrefix, massSuffix] = lcl_findLongestTag(annotation, minTagSize, maxJump, maxMod)
...
% Remove spaces
idxNoSpaces = find(tag~=' ');   tag = tag(idxNoSpaces);   szTag = size(tag,2);
...
if isempty(idxS) tag=''; massPrefix=0; massSuffix=0; return; else
    bestIdx = min(find((idxE-idxS)==max(idxE-idxS)));
    tag = tag(idxS(bestIdx):idxE(bestIdx));
end;

% Find prefix/suffix masses - convert irrelevant text to zero-mass characters such as ' ', '(', ')'
% Remove mods
idxJumpS=find(annotation=='[');   idxJumpE=find(annotation==']');
for i=1:length(idxJumpS)
    idx = idxJumpS(i)+min(find(annotation(idxJumpS(i)+1:idxJumpE(i))==','));
    annotation(idx:idxJumpE(i)-1)=' ';
end

% Convert start/end jumps to regular offsets
annotation(find(annotation=='{'))='[';
annotation(find(annotation=='}'))=']';

massPrefix = sum(sn_getmasses(annotation(1:idxNoSpaces(idxS(bestIdx))-1),'',[],0));
massSuffix = sum(sn_getmasses(annotation(idxNoSpaces(idxE(bestIdx))+1:length(annotation)),'',[],0));
