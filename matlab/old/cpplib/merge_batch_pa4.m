function [pairs, aligns, ratios, means, stddevs, matchedPeaks] = merge_batch_pa4(resultFiles, ratioFiles, meanFiles, varsFiles, matchedPeaksFiles)
% function [pairs, aligns, ratios, means, stddevs, matchedPeaks] = merge_batch_pa4(resultFiles, ratioFiles, meanFiles, varsFiles, matchedPeaksFiles)
%
%  Same as merge_batch_pa3 but also loads _matchedPeaks files.
%
...
pairs = []; aligns = []; ratios = []; matchedPeaks = []; meansTmp = cell(1,numFiles);   meansCorrections = [];   varsTmp = cell(1,numFiles);   varsCorrections = [];
for i=1:numFiles
    ...
    if isempty(common)
        if ~isempty(pairsCur) 
            pairs = [pairs; pairsCur];   aligns = [aligns; alignsCur];   ratios = [ratios; load(ratioFiles{i})]; 
            if exist(matchedPeaksFiles{i})==2 matchedPeaks = [matchedPeaks; load(matchedPeaksFiles{i})]; end;
        end;
    else
        ratiosCur = load(ratioFiles{i});   matchedPeaksCur = load(matchedPeaksFiles{i});   meansTmp{i} = load(meanFiles{i});   varsTmp{i} = load(varsFiles{i});
        idxNew = setdiff([1:size(pairsCur,1)]',idxCur);
        pairs = [pairs; pairsCur(idxNew,:)];   aligns = [aligns; alignsCur(idxNew,:)];   ratios = [ratios; ratiosCur(idxNew,:)];   matchedPeaks = [matchedPeaks; matchedPeaksCur(idxNew,:)];
        
        % Select between repeated pairs and correct means
        for j=1:size(common,1)
            if (double(aligns(idxOld(j),3)) + double(aligns(idxOld(j),4))) >= (double(alignsCur(idxCur(j),3)) + double(alignsCur(idxCur(j),4)))
                scoresToRemove = double(alignsCur(idxCur(j),3:4));
            else
                scoresToRemove = double(aligns(idxOld(j),3:4));
                aligns(idxOld(j),:) = alignsCur(idxCur(j),:);
                ratios(idxOld(j),:) = ratiosCur(idxCur(j),:);
                matchedPeaks(idxOld(j),:) = matchedPeaksCur(idxCur(j),:);
            end
